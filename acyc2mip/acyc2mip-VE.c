    ...
    if(option_mip) {
	  ...
	  fputs("Bounds\n", out);
		for (int i=0; i<nsources; i++){
			fprintf(out, "  0 <= edge_%i_%i <= 1\n", sources[i], targets[i]);
		}	  
      fputs("BINARY\n", out);
      tr_atoms(STYLE_BIN, out, table, occtable);	  
      //fputs("Integer\n", out);
		//for (int i=0; i<nsources; i++){
			//fprintf(out, "  edge_%i_%i\n", sources[i], targets[i]);
		//}
      fputs("GENERAL\n", out);		
...
void tr_acyc_interface(int style, FILE *out, GRAPH *graphs)
{
		  ...
		  int *idfound;
		  int *node2id;
  sources=(int *)malloc((1000000) * sizeof(int));
  targets=(int *)malloc((1000000) * sizeof(int));
  ...
  while(sg) {
	...
	if(style == STYLE_LP){
		  ...
		  idfound=(int *)malloc((100000000) * sizeof(int));
		  node2id=(int *)malloc((nNodes) * sizeof(int));
		  //addedby = (int ***)malloc(nNodes * sizeof(int **));
          ...
          for(int i=0;i<nNodes;i++){
			...
			//addedby[i]=(int **)malloc((nNodes) * sizeof(int *));
		  ...
		  while(node2) {
			  ...
			  while(edge) {
				  ...
				  int t = (node2->node)-bias;
				  int s = (edge->node)-bias;
				  if (idfound[t]==-1){
					  idfound[t]=nnode2id;
					  node2id[nnode2id]=t;					  
					  nnode2id++;
				  ...
				  /*if ((s<0) || (s>=nNodes) || (t<0) || (t>=nNodes)){
					  printf("\n\n INCONSISTECY IN GRAPH \n\n");
					  printf("\n\n node2->node = %d    bias = %d    nNodes = %d  edge->node = %d\n\n",node2->node, bias,nNodes, edge->node);
					  exit(0);
				  }	*/			  

					fprintf(out, "  edge_%i_%i - var_%i >= 0\n", node2id[s+bias], node2id[t+bias], edge->atom);
					if ((graph[s][t]==-1) && (graph[t][s]==-1)){
						sources[nsources]=node2id[s+bias];
						targets[nsources]=node2id[t+bias];
						nsources++;
					}
          ...
          for (int c=0; c<nNodes; c++){
            ...
            for (int a=0; a<ngraphAdjrev[i]; a++){
              ...
              for (int b=0; b<ngraphAdj[i]; b++){
                ...
                if ((graph[j][k]==-1) && (graph[k][j]==-1)){

 
							//fprintf(out, "  edge_%i_%i >= -1\n", j+bias, k+bias);
							//fprintf(out, "  edge_%i_%i <= 1\n", j+bias, k+bias);
							
							sources[nsources]=node2id[j+bias];
							targets[nsources]=node2id[k+bias];
							nsources++;
				}
				...
				if ((graph[j][k]==-1)){	  
					...
					fprintf(out, "  edge_%i_%i + edge_%i_%i - edge_%i_%i <= 1\n",  node2id[j+bias], node2id[i+bias], node2id[i+bias], node2id[k+bias], node2id[j+bias], node2id[k+bias]);
	...
	for(int i=0;i<nNodes;i++){
		...
		//for(int j=i+1;j<nNodes;j++){
			...
			if (graph[i][i]==-1)
				continue;
			fprintf(out, "  edge_%i_%i = 0\n",  node2id[i+bias], node2id[i+bias]);
