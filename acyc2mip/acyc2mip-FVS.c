...
void tr_acyc_interface(int style, FILE *out, GRAPH *graphs)
{
	...
	bounds=(int *)malloc(100000000 * sizeof(int));
	boundsv=(int *)malloc(100000000 * sizeof(int));
	nbounds=0;
  GRAPH *sg = graphs;
  int bits = 0, i = 0, w = 0;
  ...
  while(sg) {
    ...
    while(node) {
			bounds[nbounds]=node->node;
			boundsv[nbounds]=nloops;
			nbounds++;
			int j=0;
			int nNodes = (sg->node_cnt) + 1;

			for(int i=0;i<nNodes;i++){
					if (node2id[i]==node->node)
					{
						j=i;
						break;
					}
