...
typedef lvector<mapping> value_map;
...
class partial_state : public pair_set
{
 public:
  ...
  partial_state(const partial_state& s, const value_map& map);
  ...
  bool implies(const index_pair& a) const;
...
class PartialStateEnumerator : public IterativeEnumerator {
 ...
 public:
  ...
  PartialStateEnumerator(const index_vec& s, const partial_state& c);
...
class SASInstance {
 ...
 public:
  ...
  // as a projection of a SAS instance; reduce_map maps from the original
  ...
  // construct abstract SAS instance by projection on subset of vars
  SASInstance(const SASInstance& ins, const index_set& vars);
  // construct abstract SAS instance from value map
  SASInstance(const SASInstance& ins, const value_map& vm);
  // copy
  ...
  void compute_plan_causal_graph(const index_vec& p, graph& g) const;
  // void compute_domain_transitions(const ACF& cost);
  void compute_DTG(index_type v, index_set_graph& g,
		   index_type& ni, index_set& gn);
  void compute_action_interaction_graph(graph& g);
  void compressed_action_interaction_graph(index_set_graph& g);
  ...
  bool make_state_complete(const partial_state& p, partial_state& s) const;
  ...
  // construct abstraction from value_map
  void construct(const SASInstance& ins, const value_map& vm);
  ...
  void actions_with_effect_on(index_type v, index_set& acts);
  void actions_with_effect(index_pair c, index_set& acts);
  void precondition_variable_set(const index_set& acts, index_set& vars);
  ...
  void write_automata(std::ostream& s) const;
...
class SpanningVariableSet : public index_set {
  SASInstance&  instance;

  class SetOrder : public index_vec::order {
    cost_vec bit_ratio;
    const index_vec& cover_size;
    const index_vec& largest_add_set;
   public:
    SetOrder(const index_set_vec& dset,
    ...
    virtual bool operator()(const index_type& v0,
			    const index_type& v1) const;
  };

  void rank_sets(const index_set& sel,
		 const index_set_vec& dset,
		 const index_set_vec& dby,
		 index_vec& o);
  void compute_approximate();

 public:
  SpanningVariableSet(SASInstance& ins);
  ~SpanningVariableSet();
};
