...
//   void compute_with_precedence(const graph& pg);
  ...
  virtual void abstract_plan(const partial_state& s,
			     PlanSet& ps,
			     index_type max);
...
inline std::ostream& operator<<(::std::ostream& s, const HValue& v)
{
  s << v.val;
  if (v.opt) s << '*';
  return s;
...
typedef __gnu_cxx::hash_map<partial_state, HValue,
  StateHashFunction, std::equal_to<partial_state> > partial_state_map;

//typedef std::unordered_map<partial_state, HValue,
//  StateHashFunction, std::equal_to<partial_state> > partial_state_map;
