	     ...
	     Statistics& stats)
{
  ...
  if (time_limit > 0) search_stats.enable_time_out(time_limit);
  index_type m = h_m + 1;
  ...
  if (new_cost <= est_cost) {
    ...
    search_stats.enable_iteration_limit(1);
...
int main(int argc, char *argv[]) {
  ...
  for (int k = 1; k < argc; k++) {
    if ((strcmp(argv[k],"-v") == 0) && (k < argc - 1)) {
      ...
      improve_time_limit = atoi(argv[++k]);
    }
      ...
      Instance::write_atom_set_with_symbolic_names = false;
	      ...
	      (strcmp(argv[k],"-r") == 0)) &&
	     (k < argc - 1)) {
      unsigned long random_seed = atoi(argv[++k]);
  ...
  for (index_type k = 0; k < n_plans; k++) {
    ...
    while ((s < plan.plan_steps().length()) && ok) {
      ...
      cs.advance(dt, errs);
      if (!cs.applicable(act.index, errs, s)) {
