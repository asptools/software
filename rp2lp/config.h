...
// not seem to be too expensive).
#define RATIONAL_ARITHMETIC_CHECK_OVERFLOW

// Print warnings when overflow occurs in rational arithmetic
// operations? (no effect unless overflow checking is enabled).
#define RATIONAL_ARITHMETIC_PRINT_OVERFLOW_WARNINGS

// Abort (dumping core) when overflow occurs in rational arithmetic
// operations? (no effect unless overflow checking is enabled).
...
// the rational::round method) and this is currently only used in
// a few places. Setting this value to the (signed) maximum that can be
// represented using half the bits of a (signed) long int should be safe,
// in the sense that no operation (+, -, *, /, <, >) on two rationals that
// have been rounded should ever cause an overflow
...
// Extra heuristic evaluation of action precondition in sequential
// regression -- useless when using A*, and probably not worth the
// effort when using IDA* either.
// #define EVAL_ACTION_PRECONDITION_IN_SEQUENTIAL_REGRESSION
...
#define PERIODIC_CHECK_INTERVAL 1.0
...
// Malte: This shaves off another 10% or so in my experiments.
#ifdef __GNUC__
