  ...
  if (!stats.break_signal_raised()) {
	...
	if (opt_cd) {
	  if (opt_no_cost_limit) {
	    list = cost_tab.boost_cd(list, *b_root, 0, cd_limit, EMPTYSET,
				     POS_INF, EMPTYSET,
	  ...
	  if (opt_no_cost_limit) {
	    list = cost_tab.boost(list, *b_root, 0, POS_INF, EMPTYSET,
				  boost_wps_limit, false);
	  }
	  else if (opt_dynamic_limit) {
	    list = cost_tab.boost(list, *b_root, 0, root_est_cost,
				  instance.goal_atoms,
				  boost_wps_limit, false);
	  }
	  else {
	    list = cost_tab.boost(list, *b_root, 0, root_est_cost, EMPTYSET,
				  boost_wps_limit, false);
	  }
	}
	...
	if (opt_tt) {
	  ...
	  search = new IDA(search_stats, result, tt);
	  search->set_cycle_check(opt_cc);
	}
	else {
	  std::cerr << "using IDA*..." << std::endl;
	  search = new IDA(search_stats, result);
	  search->set_cycle_check(opt_cc);
	}
