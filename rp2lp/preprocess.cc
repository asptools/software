...
bool Preprocessor::prep_remove_static_atoms = true;
bool Preprocessor::prep_remove_useless_actions = true;

Preprocessor::Preprocessor(Instance& ins, Stopwatch& s)
  : instance(ins),
...
StaticMutex* Preprocessor::inconsistency()
{
  if (inc_relation != 0) {
  ...
  if (inc_relation == 0) {
    ...
    if (instance.conditional_effects()) {
      Instance irce(instance);
      irce.relax_conditional_effects(false);
      irce.cross_reference();
      StaticMutex mx(irce);
      inc_relation = new StaticMutex(instance, mx);
    }
    else {
      inc_relation = new StaticMutex(instance, instance.init_atoms);
    }
    if (trace_level > 0) {
...
graph* Preprocessor::landmark_graph(bool opt_H2)
{
  if (lm_graph == 0) {
    lm_graph = new graph(instance.n_atoms());
    causal_landmark_graph(*lm_graph, opt_H2);
    add_non_causal_landmarks(*lm_graph);
  }
  return lm_graph;
}

graph* Preprocessor::necessary_sb_graph(bool opt_H2)
{
  if (nsb_graph == NULL) {
    nsb_graph = new graph(instance.n_atoms());
    causal_landmark_graph(*nsb_graph, opt_H2);
    graph g1;
    nc_landmark_graph(g1, opt_H2);
    for (index_type i = 0; i < instance.n_atoms(); i++)
      for (index_type j = 0; j < instance.n_atoms(); j++) {
	if (g1.adjacent(i, j)) {
	  if (!instance.atoms[i].add_by.
	      have_common_element(instance.atoms[j].add_by))
	    nsb_graph->add_edge(i, j);
	}
...
graph* Preprocessor::never_after_graph(bool opt_H2)
{
  inconsistency();
  if (na_graph == 0) {
    na_graph = new graph(instance.n_atoms());
    compute_never_after_graph(*na_graph, 0, 0, opt_H2);
  }
  return na_graph;
}

void Preprocessor::compute_reachability
(bool_vec& reachable_atoms, bool_vec& reachable_actions,
 const bool_vec* aa, bool opt_H2)
{
  Heuristic* h0 = 0;
  CostTable* h0x = new CostTable(instance, stats);
  if (opt_H2) {
    h0 = new StaticMutex(instance, instance.init_atoms, aa);
    //h0x->compute_H2(ZeroACF());
  }
  else {
    h0 = new Reachability(instance, instance.init_atoms, aa);
    //CostTable* ht = new CostTable(instance, stats);
    //bool_vec init(instance.init_atoms, instance.n_atoms());
    //ht->compute_H1(ZeroACF(), init, aa);
    //h0 = ht;
    //h0x->compute_H1(ZeroACF());
  }
  reachable_atoms.assign_value(true, instance.atoms.length());
  // bool error = false;
  for (index_type k = 0; k < instance.atoms.length(); k++) {
    NTYPE c = h0->eval(k);
    if (INFINITE(c)) reachable_atoms[k] = false;
    // if (c != (h0x->eval(k))) {
    //   std::cerr << "atom " << k << ": " << c << " / " << h0x->eval(k)
    // 		<< std::endl;
    //   error = true;
    // }
  }
  // if (error) exit(1);
  reachable_actions.assign_value(true, instance.actions.length());
  for (index_type k = 0; k < instance.actions.length(); k++) {
    NTYPE c = h0->eval(instance.actions[k].pre);
    if (INFINITE(c)) reachable_actions[k] = false;
  }
  delete h0;
}
...
void Preprocessor::compute_relevance_with_ce
(const index_set& check, bool_vec& r_atm, bool_vec& r_act)
{
  for (index_type k = 0; k < check.length(); k++)
    if (!r_atm[check[k]]) {
	...
	if (!r_act[i]) {
	  if (instance.actions[i].add.contains(check[k])) {
	    r_act[i] = true;
	    compute_relevance_with_ce(instance.actions[i].pre, r_atm, r_act);
	  }
	  else {
	    bool ce_rel = false;
	    for (index_type j = 0; j < instance.actions[i].cadd.size(); j++)
	      if (instance.actions[i].cadd[j].consequent == check[k]) {
		compute_relevance_with_ce
		  (instance.actions[i].cadd[j].antecedent, r_atm, r_act);
		ce_rel = true;
	      }
	    if (ce_rel) {
	      r_act[i] = true;
...
void Preprocessor::remove_useless_actions()
{
  ...
  for (index_type k = 0; k < instance.n_actions(); k++)
	...
	instance.actions[k].pre.contains(instance.actions[k].add)) {
      if (trace_level > 1) {
	std::cerr << "action " << instance.actions[k].name
		  << " is useless" << std::endl;
      }
...
///
// Version 1 (apparently not correct, though I can't figure out why)
///
// void Preprocessor::preprocess_with_ce(bool opt_H2, bool remove_irrelevant)
// {
//   stats.start();
//   if (trace_level > 0)
//     std::cerr << "relaxing conditional effects..." << std::endl;
//   Instance irce(instance);
//   index_type first_rce_atom = irce.n_atoms();
//   index_type first_rce_action = irce.n_actions();
//   irce.relax_conditional_effects(true);
//   index_type n_rce = 0;
//   for (index_type k = 0; k < instance.n_actions(); k++)
//     n_rce += (instance.actions[k].cadd.size() + 
// 	      instance.actions[k].cdel.size());
//   // std::cerr << "instance.n_act = " << instance.n_actions()
//   // 	    << ", n_rce = " << n_rce
//   // 	    << ", irce.n_act = " << irce.n_actions()
//   // 	    << std::endl;
//   assert(irce.n_actions() == (instance.n_actions() + n_rce));
//   // irce.print(std::cerr);
//   Preprocessor prce(irce, stats);
//   prce.trace_level = trace_level;
//   prce.preprocess(opt_H2);
//   if (remove_irrelevant) {
//     prce.compute_irrelevant_atoms();
//     prce.remove_irrelevant_atoms();
//   }
//   assert(prce.action_map.size() == (instance.n_actions() + n_rce));
//   // irce.print(std::cerr);
//   // std::cerr << "atom map: " << prce.atom_map << std::endl;
//   // std::cerr << "action map: " << prce.action_map << std::endl;
//   // std::cerr << "inv map: " << prce.inv_map << std::endl;
//   bool_vec atoms_to_remove(false, instance.n_atoms());
//   bool_vec actions_to_remove(false, instance.n_actions());
//   bool_vec invs_to_remove(false, instance.n_invariants());
//   for (index_type k = 0; k < instance.n_atoms(); k++)
//     if (prce.atom_map[k] == no_such_index)
//       atoms_to_remove[k] = true;
//   index_type next_rce = instance.n_actions();
//   for (index_type k = 0; k < instance.n_actions(); k++)
//     if (prce.action_map[k] == no_such_index) {
//       actions_to_remove[k] = true;
//       next_rce += instance.actions[k].cadd.size();
//       next_rce += instance.actions[k].cdel.size();
//     }
//     else {
//       // if the action stays, we need to figure out which of
//       // its ce's to remove:
//       if (!instance.actions[k].cadd.empty()) {
//       	bool_vec cadd_to_remove(false, instance.actions[k].cadd.size());
//       	for (index_type i = 0; i < instance.actions[k].cadd.size(); i++)
//       	  if (prce.action_map[next_rce + i] == no_such_index)
//       	    cadd_to_remove[i] = true;
// 	next_rce += instance.actions[k].cadd.size();
//       	instance.actions[k].cadd.remove(cadd_to_remove);
//       }
//       if (!instance.actions[k].cdel.empty()) {
//       	bool_vec cdel_to_remove(false, instance.actions[k].cdel.size());
//       	for (index_type i = 0; i < instance.actions[k].cdel.size(); i++)
//       	  if (prce.action_map[next_rce + i] == no_such_index)
//       	    cdel_to_remove[i] = true;
// 	next_rce += instance.actions[k].cdel.size();
// 	instance.actions[k].cdel.remove(cdel_to_remove);
//       }
//     }
//   // std::cerr << "next rce = " << next_rce << std::endl;
//   assert(next_rce == prce.action_map.size());
//   for (index_type k = 0; k < instance.n_invariants(); k++)
//     if (prce.inv_map[k] == no_such_index)
//       invs_to_remove[k] = true;
//   instance.remove_actions(actions_to_remove, action_map);
//   instance.remove_invariants(invs_to_remove, inv_map);
...
void Preprocessor::preprocess_with_ce(bool opt_H2, bool remove_irrelevant)
{
  ...
  index_type first_rce_action = irce.n_actions();
  irce.relax_conditional_effects(false);
  Reachability reach(irce);
  if (trace_level > 0)
    std::cerr << "removing unreachable actions..." << std::endl;
  bool_vec actions_to_remove(false, instance.n_actions());
  for (index_type k = 0; k < instance.n_actions(); k++) {
    if (reach.unreachable(instance.actions[k].pre)) {
      actions_to_remove[k] = true;
    }
    else {
      if (!instance.actions[k].cadd.empty()) {
	bool_vec cadd_to_remove(false, instance.actions[k].cadd.size());
	for (index_type j = 0; j < instance.actions[k].cadd.size(); j++)
	  if (reach.unreachable(instance.actions[k].cadd[j].antecedent))
	    cadd_to_remove[j] = true;
	instance.actions[k].cadd.remove(cadd_to_remove);
      }
      if (!instance.actions[k].cdel.empty()) {
	bool_vec cdel_to_remove(false, instance.actions[k].cdel.size());
	for (index_type j = 0; j < instance.actions[k].cdel.size(); j++)
	  if (reach.unreachable(instance.actions[k].cdel[j].antecedent))
	    cdel_to_remove[j] = true;
	instance.actions[k].cdel.remove(cdel_to_remove);
      }
    }
  }
  instance.remove_actions(actions_to_remove, action_map);
  bool_vec static_true(false, instance.n_atoms());
  bool_vec static_false(false, instance.n_atoms());
  for (index_type i = 0; i < instance.n_atoms(); i++)
    if (instance.atoms[i].init)
      ...
      static_false[i] = true;
  for (index_type k = 0; k < instance.n_actions(); k++) {
    static_true.subtract(instance.actions[k].del);
    for (index_type i = 0; i < instance.actions[k].cdel.size(); i++)
      static_true[instance.actions[k].cdel[i].consequent] = false;
    static_false.subtract(instance.actions[k].add);
    for (index_type i = 0; i < instance.actions[k].cadd.size(); i++)
      static_false[instance.actions[k].cadd[i].consequent] = false;
  }
  if (trace_level > 0)
    std::cerr << static_true.count(true) << " statically true and "
	      << static_false.count(true) << " statically false atoms"
	      << std::endl;
  // make sure we don't remove statically false goal atoms
  for (index_type i = 0; i < instance.n_atoms(); i++)
    if (instance.atoms[i].goal)
      static_false[i] = false;
  static_true.insert(static_false);
  instance.remove_atoms(static_true, atom_map);
  if (trace_level > 0)
    std::cerr << "re-cross referencing..." << std::endl;
  instance.clear_cross_reference();
  instance.cross_reference();
...
void Preprocessor::preprocess(bool opt_H2) {
  // this method applies only to problems without conditional effects!
  assert(!instance.conditional_effects());
  ...
  bool_vec useless(false, instance.n_actions());
  for (index_type k = 0; k < instance.n_actions(); k++) {
    ...
    if (prep_remove_useless_actions) {
      for (index_type i = 0; i < instance.actions[k].add.length(); i++)
	if (instance.actions[k].sel &&
	    instance.actions[k].pre.contains(instance.actions[k].add))
	  useless[k] = false;
  ...
  compute_reachability(reachable_atoms, reachable_actions, 0, opt_H2);
  if (trace_level > 1) {
    std::cerr << reachable_atoms.count(true) << " of " << instance.n_atoms()
	      << " atoms and " << reachable_actions.count(true) << " of "
	      << instance.n_actions() << " reachable" << std::endl;
  }
  if (trace_level > 0) std::cerr << "computing static atoms..." << std::endl;
  bool_vec atoms_to_remove(false, instance.n_atoms());
  compute_static_atoms(reachable_actions, atoms_to_remove);
  if (trace_level > 1) {
    std::cerr << atoms_to_remove.count(true) << " of " << instance.n_atoms()
	      << " atoms are statically true" << std::endl;
  }
...
(index_type init_atom, index_set& inv_set, bool& exact, index_set& branch_set)
{
  ...
  while (!converged && !inv_set.empty()) {
	...
	index_type n = act.add.count_common(inv_set);
	if (n > (act.add.contains(p[0]) ? 1 : 0)) {
	  if (trace_level > 2) {
...
void Preprocessor::add_mutex_invariants(graph& inc)
{
  if (trace_level > 0) {
    std::cerr << "adding mutex invariants..." << std::endl;
  }
  stats.start();
  ...
  for (index_type i = 0; i < instance.n_atoms(); i++)
    for (index_type j = i + 1; j < instance.n_atoms(); j++)
      if (inc.adjacent(i, j)) {
	bool covered = false;
	for (index_type k = 0; (k < n) && !covered; k++)
	  if (instance.invariants[k].set.contains(i) &&
	      instance.invariants[k].set.contains(j))
	    covered = true;
	if (!covered) {
	  Instance::Constraint& c = instance.new_invariant();
	  c.set.insert(i);
	  c.set.insert(j);
	  c.lim = 1;
	  c.exact = false;
...
(const index_set& init, const index_set& goal, index_type p,
 const bool_vec* aa, bool opt_H2)
{
  ...
  Heuristic* h0 = 0;
  if (opt_H2) {
    h0 = new StaticMutex(*test_instance, test_instance->init_atoms, aa);
  }
  else {
    h0 = new Reachability(*test_instance, test_instance->init_atoms, aa);
  }
  NTYPE val = h0->eval(goal);
  delete h0;
...
void Preprocessor::compute_landmark_graph
(graph& g, const bool_vec* aa, bool opt_H2)
{
  stats.start();
  ...
  for (index_type i = 0; i < instance.n_atoms(); i++) {
    ...
    for (index_type j = 0; j < instance.n_atoms(); j++) if (j != i) {
      if (landmark_test(instance.init_atoms, goal_i, j, aa, opt_H2))
	g.add_edge(j, i);
    }
  }
  stats.stop();
...
// quick_lmg returns a graph with only "causal" landmarks
void Preprocessor::causal_landmark_graph
(graph& g, bool opt_H2)
{
  stats.start();
  g.init(instance.n_atoms());
  if (opt_H2) {
    StaticMutex reach(instance, instance.init_atoms);
    bool_vec r0(false, instance.n_atoms());
    for (index_type i = 0; i < instance.n_atoms(); i++)
      r0[i] = !reach.unreachable(i);
    for (index_type i = 0; i < instance.n_atoms(); i++)
      if (r0[i]) {
	bool_vec in(true, instance.n_actions());
	in.subtract(instance.atoms[i].req_by);
	index_set s0(instance.init_atoms);
	s0.subtract(i);
	reach.recompute(s0, in);
	for (index_type j = 0; j < instance.n_atoms(); j++)
	  if ((i != j) && r0[j] && reach.unreachable(j)) {
	    g.add_edge(i, j);
	  }
      }
  }
  else {
    Reachability reach(instance, instance.init_atoms);
    bool_vec r0(reach.reachable());
    for (index_type i = 0; i < instance.n_atoms(); i++)
      if (r0[i]) {
	bool_vec in(true, instance.n_actions());
	in.subtract(instance.atoms[i].req_by);
...
void Preprocessor::add_non_causal_landmarks(graph& g)
{
  assert(g.size() == instance.n_atoms());
  for (index_type i = 0; i < instance.n_atoms(); i++)
    for (index_type j = 0; j < instance.n_atoms(); j++)
      if ((i != j) && !instance.atoms[i].init && g.adjacent(i, j)) {
	// std::cerr << instance.atoms[i].name << " -> "
	// 	  << instance.atoms[j].name << " (causal)" << std::endl;
	assert(instance.atoms[i].add_by.size() > 0);
	// std::cerr << "adders = ";
	// instance.write_action_set(std::cerr, instance.atoms[i].add_by);
	// std::cerr << std::endl;
	bool_vec co_add(true, instance.n_atoms());
	for (index_type k = 0; k < instance.atoms[i].add_by.size(); k++)
	  co_add.intersect(instance.actions[instance.atoms[i].add_by[k]].add);
	// std::cerr << "co-add = ";
	// instance.write_atom_set(std::cerr, co_add);
	// std::cerr << std::endl;
	for (index_type l = 0; l < instance.n_atoms(); l++)
	  if ((i != l) && (j != l) && co_add[l] && !g.adjacent(l, j))
	    g.add_edge(l, j);
      }
}

void Preprocessor::nc_landmark_graph
(graph& g, bool opt_H2)
{
  stats.start();
  g.init(instance.n_atoms());
  if (opt_H2) {
    StaticMutex reach(instance, instance.init_atoms);
    bool_vec r0(true, instance.n_atoms());
    for (index_type i = 0; i < instance.n_atoms(); i++)
      r0[i] = !reach.unreachable(i);
    for (index_type i = 0; i < instance.n_atoms(); i++)
      if (r0[i] && !instance.atoms[i].init) {
	bool_vec in(true, instance.n_actions());
	in.subtract(instance.atoms[i].add_by);
	reach.recompute(instance.init_atoms, in);
	for (index_type j = 0; j < instance.n_atoms(); j++)
	  if ((i != j) && r0[j] && reach.unreachable(j))
	    g.add_edge(i, j);
      }
...
// void Preprocessor::quick_landmark_graph
// (graph& g, const bool_vec* atms, const bool_vec* aa, bool opt_H2)
// {
//   read_only_vector_with_default<bool> atms1(atms, true);
//
//   stats.start();
//   g.init(instance.n_atoms());
//
//   for (index_type i = 0; i < instance.n_atoms(); i++) if (atms1[i]) {
//     Instance* test_instance = new Instance(instance);
...
void Preprocessor::conditional_landmark_graph
(const index_set& p, graph& g, bool_vec& u,
 const bool_vec* aa, bool opt_H2)
{
  stats.start();
  g.init(instance.n_atoms());
  bool_vec v(true, instance.n_atoms());
  for (index_type k = 0; k < instance.n_atoms(); k++)
    if (inconsistent(p, k))
      v[k] = false;

#ifdef TRACE_PRINT_LOTS
  std::cerr << "computing conditional landmark graph for p = ";
  instance.write_atom_set(std::cerr, p);
  std::cerr << std::endl;
  std::cerr << "init vector: " << v << std::endl;
#endif

  u.assign_value(false, instance.n_atoms());
  Instance* test_instance = new Instance(instance);
  for (index_type k = 0; k < instance.n_atoms(); k++)
    test_instance->atoms[k].init = v[k];
  test_instance->cross_reference();
  Heuristic* h0 = 0;
  if (opt_H2) {
    h0 = new StaticMutex(*test_instance, test_instance->init_atoms, aa);
  ...
  for (index_type i = 0; i < instance.n_atoms(); i++)
    if (INFINITE(h0->eval(i)))
      u[i] = true;
  delete h0;
  delete test_instance;

  for (index_type i = 0; i < instance.n_atoms(); i++) if (!u[i]) {
#ifdef TRACE_PRINT_LOTS
    std::cerr << "testing over atom " << i << ". "
...
void Preprocessor::compute_never_after_graph
(graph& g, const bool_vec* atms, const bool_vec* aa, bool opt_H2)
{
  inconsistency();
  read_only_vector_with_default<bool> atms1(atms, true);
  ...
  for (index_type i = 0; i < instance.n_atoms(); i++)
	...
	if (!inconsistent(i, k))
      ...
      Heuristic* h0 = 0;
      if (opt_H2) {
	h0 = new StaticMutex(instance, i_init, aa);
      }
      else {
	h0 = new Reachability(instance, i_init, aa);
      }
      for (index_type k = 0; k < instance.n_atoms(); k++)
	if (!inconsistency()->unreachable(k))
	  if (INFINITE(h0->eval(k)))
	    g.add_edge(i, k);
    }
  stats.stop();
}
...
ReducedActionSet::ReducedActionSet(Instance& ins)
  : bool_vec(true, ins.n_actions()), instance(ins)
{
  // done
}

ReducedActionSet::~ReducedActionSet()
{
  // done
}

void ReducedActionSet::compute(const bool_vec& s)
{
  // does nothing
}

ForwardRelevantActionSet::ForwardRelevantActionSet(Instance& ins)
  : ReducedActionSet(ins)
{
  // ...
}

ForwardRelevantActionSet::~ForwardRelevantActionSet()
{
  // ...
}

void ForwardRelevantActionSet::prepare(Preprocessor& prep)
{
}

void ForwardRelevantActionSet::compute(const bool_vec& s)
{
}
