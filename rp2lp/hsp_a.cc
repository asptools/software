...
int main(int argc, char *argv[])
{
  ...
  // apply node and iteration limits to relaxed search only
  if (node_limit > 0) a_stats.enable_node_expansion_limit(node_limit);
  if (iteration_limit > 0) a_stats.enable_iteration_limit(iteration_limit);
  ...
  if (!stats.break_signal_raised()) {
    CostTable cost_tab(instance, h_stats);
	...
	CostTable* rce = new CostTable(instance, h_stats);
	...
	if (opt_mm) {
	  ...
	  for (index_type d = 1; d <= mm_depth; d++) {
	    std::cerr << "depth = " << d << ": ";
	    MiniMax mm_search(a_stats, d);
	    new_cost = mm_search.new_state(*apx_root, POS_INF);
	    std::cerr << "val = " << new_cost << " (" << a_stats << ")"
		      << std::endl;
	  }
	  a_stats.stop();
	  std::cerr << "minimax search complete in " << a_stats.total_time()
		    << " seconds" << std::endl;
	}
	  ...
	  ITest apx_search(a_stats, apx_res, sol_tab);
	  ...
	  ApxResult apx_res;
	  HashTable sol_tab(opt_st_size);
	  LDFS apx_search(a_stats, apx_res, sol_tab);
	  new_cost = apx_search.start(*apx_root);
	  ...
	  IDAO* apx_search = 0;
	  if (max_k > 0) {
	    apx_search = new kIDAO(max_k, a_stats, apx_res, &sol_tab);
	  }
	  else if (opt_no) {
	    apx_search = new kIDAO(a_stats, apx_res, &sol_tab);
	  }
	  else {
	    apx_search = new IDAO(a_stats, apx_res, &sol_tab);
	  }
	  apx_search->set_cycle_check(opt_cc);
	  ...
	  if (!stats.break_signal_raised()) {
	    ...
	    else if (a_stats.break_signal_raised()) {
	      std::cerr << "node/iteration limit reached, no "
			<< current_apx_limit
...
// 		    << std::endl;
	...
	h_file.close();
      ...
      if (opt_load_compare) {
	CostTable* lc = new CostTable(instance, h_stats);
	...
	if (solved && opt_temporal && opt_post_op) {
	  ...
	  if (!stats.break_signal_raised()) {
	    ...
	    std::cerr << "searching..." << std::endl;
	    DFS_BB op_search(search_stats, filter);
  ...
  if (opt_pop) {
    ...
    std::cout << "relaxed search: " << a_stats << std::endl;
    std::cout << "main search: " << search_stats << std::endl;
