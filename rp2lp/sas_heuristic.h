...
class SASH1 : public SASHeuristic {
  const SASInstance& instance;
  index_vec idx;
  index_set_vec req_by;
  cost_vec val;

  typedef std::pair<index_pair, NTYPE> queue_type;
  struct queue_greater_than {
  public:
    bool operator()(const queue_type& e1, const queue_type& e2) const
    {
  ...
  cost_vec qval;
  index_vec rem;
  std::priority_queue<queue_type, std::vector<queue_type>, queue_greater_than>
    queue;

  index_type n_atoms();
  index_type index(const index_pair& p);
  void enqueue(const partial_state& ps, NTYPE c);

  friend class SASLMCutBase;

 public:
  SASH1(const SASInstance& i);

  void compute(const partial_state& s0, const ACF& cost);
  void compute(const partial_state& s0, const ACF& cost, const bool_vec& aa);
  void compute1(const partial_state& s0, const ACF& cost);

  // eval estimates the cost of reaching goal state s from the
  // initial state s0 given to compute
  virtual NTYPE eval(const partial_state& s);
  virtual NTYPE eval(const index_pair& p);
};

class SASLMCutBase {
  const SASInstance& instance;
  SASH1 h1;
  index_set_vec add_by;

  void extend_goal_set(const partial_state& sgs,
		       const AnyACF& rcost,
 ...
 public:
  SASLMCutBase(const SASInstance& i);

  NTYPE compute(const partial_state& s0,
