...
class SeqRegState : public AtomSetState, public StateFactory {
 protected:
  ...
  class SeqRegTrans : public RegressionPath, SequentialTransition {
  public:
    SeqRegTrans(index_type a, NTYPE d) : SequentialTransition(a, d) { };
    virtual int compare(const Transition& t) {
      return SequentialTransition::compare((const SeqRegTrans&)t);
    };
    virtual Transition* copy() {
      return new SeqRegTrans(act, delta);
    };
