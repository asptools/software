...
partial_state::partial_state
(const partial_state& s, const value_map& map)
  : pair_set()
{
  for (index_type k = 0; k < s.length(); k++) {
    assert(s[k].first < map.size());
    if (map[s[k].first](s[k].second) != no_such_index)
      insert(index_pair(s[k].first, map[s[k].first](s[k].second)));
  }
}
...
bool partial_state::implies(const index_pair& a) const
{
  return contains(a);
}
...
PartialStateEnumerator::PartialStateEnumerator
(const index_vec& s, const partial_state& c)
  : signature(s), v_free(true, s.size())
{
  variables.fill(signature.size());
  for (index_type k = 0; k < variables.length(); k++) {
    index_type l = c.value_of(variables[k]);
    if (l != no_such_index) {
      v_free[k] = false;
...
char SASInstance::STRIPS_eq_sign = '=';
...
SASInstance::SASInstance
(const SASInstance& ins, const value_map& vm)
  : atom_map(0),
    atom_vars(0),
    n_atoms(0),
    name(ins.name),
...
bool SASInstance::construct_actions(const Instance& ins)
{
  ...
  for (index_type k = 0; k < ins.n_actions(); k++) {
    ...
    if (pc && !b.post.consistent()) {
      ...
      write_partial_state(std::cerr, b.pre);
      std::cerr << ")" << std::endl;
      ok = false;
    }
...
bool SASInstance::construct_safe_actions(const Instance& ins)
{
  ...
  for (index_type k = 0; k < ins.n_actions(); k++) {
    ...
    if (pc && !b_post.consistent()) {
      ...
      std::cerr << std::endl << " (pre = ";
      write_partial_state(std::cerr, b_pre);
      std::cerr << ", safe)" << std::endl;
      std::cerr << "STRIPS pre = ";
      ins.write_atom_set(std::cerr, a.pre);
      std::cerr << std::endl << "STRIPS add = ";
      ins.write_atom_set(std::cerr, a.add);
      std::cerr << std::endl << "STRIPS del = ";
      ins.write_atom_set(std::cerr, a.del);
      std::cerr << std::endl;
      ok = false;
    }
...
bool SASInstance::construct_init_and_goal_state(const Instance& ins)
{
  ...
  partial_state m;
  map_to_partial_state(ins.init_atoms, m);
  ok = make_state_complete(m, init_state);
  if (!ok) {
    std::cerr << "warning (construction): initial state ";
    write_partial_state(std::cerr, init_state);
    std::cerr << " is not complete" << std::endl;
  }
...
void SASInstance::compute_plan_causal_graph
(const index_vec& p, graph& g) const
{
  g.init(n_variables());
  for (HSPS::index_type k = 0; k < p.length(); k++) {
    for (index_type i = 0; i < actions[p[k]].post.length(); i++) {
      for (index_type j = i + 1; j < actions[p[k]].post.length(); j++) {
	g.add_edge(actions[p[k]].post[i].first, actions[p[k]].post[j].first);
	g.add_edge(actions[p[k]].post[j].first, actions[p[k]].post[i].first);
      }
      for (index_type j = 0; j < actions[p[k]].prv.length(); j++) {
	g.add_edge(actions[p[k]].post[i].first, actions[p[k]].prv[j].first);
      }
    }
  }
}

void SASInstance::compute_DTG
(index_type v, index_set_graph& g, index_type& ni, index_set& gn)
{
  ...
  for (index_type k = 0; k < n_actions(); k++) {
	...
	for (index_type i = 0; i < variables[v].n_values(); i++)
	  g.add_edge(i, v_post, k);
      }
      else {
	g.add_edge(v_pre, v_post, k);
      }
    }
  }
  ni = init_state.value_of(v);
  index_type j = goal_state.value_of(v);
  if (j != no_such_index)
    gn.assign_singleton(j);
  else
    gn.fill(variables[v].n_values());
}
...
void SASInstance::compute_action_interaction_graph(graph& g)
{
  g.init(n_actions());
  for (index_type i = 1; i < n_actions(); i++) {
    index_set pvi;
    actions[i].post.defined_set(pvi);
    index_set rvi(pvi);
    actions[i].prv.defined_set(rvi);
    for (index_type j = i + 1; j < n_actions(); j++)
      if (actions[j].post.defines_any(rvi) ||
	  actions[j].prv.defines_any(pvi))
	g.add_undirected_edge(i, j);
  }
}

void SASInstance::compressed_action_interaction_graph(index_set_graph& g)
{
  svector< std::pair<index_set, index_set> > sigs;
  index_set_vec acts;
  for (index_type k = 1; k < n_actions(); k++) {
    index_set ev;
    actions[k].post.defined_set(ev);
    index_set dv;
    actions[k].prv.defined_set(dv);
    index_type i = sigs.first(std::pair<index_set, index_set>(ev, dv));
    if (i < sigs.size()) {
      assert(i < acts.size());
      acts[i].insert(k);
    }
    else {
      sigs.append(std::pair<index_set, index_set>(ev, dv));
      acts.append(EMPTYSET);
      assert(sigs.size() == acts.size());
      acts[sigs.size() - 1].insert(k);
    }
  }

  g.init(sigs.size());
  for (index_type i = 0; i < sigs.size(); i++) {
    g.node_label(i).assign_copy(acts[i]);
    for (index_type j = i + 1; j < sigs.size(); j++)
      if (sigs[i].first.have_common_element(sigs[j].first) ||
	  sigs[i].first.have_common_element(sigs[j].second) ||
	  sigs[j].first.have_common_element(sigs[i].second)) {
	index_set cev(sigs[i].first);
	cev.intersect(sigs[j].first);
	index_set dev1(sigs[i].second);
	dev1.intersect(sigs[j].first);
	index_set dev2(sigs[j].second);
	dev1.intersect(sigs[i].first);
	g.add_undirected_edge(i, j);
	g.edge_label(i, j).insert(cev);
	g.edge_label(i, j).insert(dev1);
	g.edge_label(i, j).insert(dev2);
      }
  }
}
...
void SASInstance::construct(const SASInstance& ins, const value_map& vm)
{
  variable_reduce_map.clear();
  variable_expand_map.clear();
  action_reduce_map.assign_value(no_such_index, ins.n_actions());

  // construct internal value map
  assert(vm.size() == ins.n_variables());
  value_map ivm(mapping(), vm.size());
  for (index_type k = 0; k < ins.n_variables(); k++)
    mapping::compact(vm[k], ivm[k]);

  // copy variables
  signature.clear();
  for (index_type k = 0; k < ins.n_variables(); k++) {
    Variable& v = new_variable(ins.variables[k].name);
    v.s_index = ins.variables[k].s_index;
    for (index_type i = 0; i < ivm[k].range(); i++) {
      index_set reps;
      ivm[k].inverse(i, reps);
      assert(!reps.empty());
      assert(reps[0] < ins.variables[k].domain.size());
      v.domain.append(ins.variables[k].domain[reps[0]]);
    }
    assert(v.domain.size() == ivm[k].range());
    if (ins.variables[k].default_val != no_such_index)
      v.default_val = ivm[k](ins.variables[k].default_val);
    else
      v.default_val = no_such_index;
    signature.append(v.domain.size());
  }
  ...
  for (index_type k = 0; k < ins.n_actions(); k++) {
    partial_state pre(ins.actions[k].pre, ivm);
    partial_state post(ins.actions[k].post, ivm);
    if (!pre.implies(post)) {
      partial_state nochange(pre);
      nochange.intersect(post);
      pre.subtract(nochange);
      post.subtract(nochange);
      assert(!post.empty());
      partial_state prv(ins.actions[k].prv, ivm);
      prv.insert(nochange);

      pre.restrict_to(real_vars);
      post.restrict_to(real_vars);
      prv.restrict_to(real_vars);
	...
	// there is already an abstract action with the same pre/post/prv:
	// if the new abstract action has lower cost, remove earlier
	// (higher-cost) actions from the action_reduce_map; if the new
	// abstract action has higher cost, ignore it.
	if (ins.actions[k].cost <= actions[i].cost) {
	  if (ins.actions[k].cost < actions[i].cost) {
	    actions[i].cost = ins.actions[k].cost;
	    for (index_type j = 0; j < k; j++)
  ...
  init_state = partial_state(ins.init_state, ivm);
  goal_state = partial_state(ins.goal_state, ivm);
}
...
Instance* SASInstance::convert_to_STRIPS()
{
  ...
  for (index_type k = 0; k < n_variables(); k++) {
    ...
    for (index_type v = 0; v < variables[k].n_values(); v++) {
      Name* n =	new ConcatenatedName(variables[k].name,
				     variables[k].domain[v],
				     STRIPS_eq_sign);
...
bool SASInstance::make_state_complete
(const partial_state& p, partial_state& s) const
{
  s.clear();
  bool ok = true;
  index_type i = 0;
  for (index_type k = 0; k < n_variables(); k++) {
	...
	if (variables[k].default_val == no_such_index)
	  ...
	  s.append(index_pair(k, variables[k].default_val));
      }
    }
    else {
      if (variables[k].default_val == no_such_index)
 ...
 RNG& rnd)
{
  ...
  count_type mixn = rational::floor_to(n_val * n_val * mixf, 1).numerator();
  if (trace_level > 0) {
...
void SASInstance::actions_with_effect_on(index_type v, index_set& acts)
{
  acts.clear();
  for (index_type k = 0; k < n_actions(); k++)
...
void SASInstance::actions_with_effect(index_pair c, index_set& acts)
{
  acts.clear();
  for (index_type k = 0; k < n_actions(); k++)
    if (actions[k].post.contains(c)) acts.insert(k);
}

void SASInstance::precondition_variable_set
(const index_set& acts, index_set& vars)
{
  vars.clear();
  for (index_type k = 0; k < acts.length(); k++) {
    actions[acts[k]].pre.defined_set(vars);
    actions[acts[k]].prv.defined_set(vars);
  }
}
...
(index_type var0, index_type val0, index_type var1, index_type val1)
{
  ...
  rational r = rational(co_app0 + co_app1, app0 + app1).reduce();
  assert(0 <= r);
  assert(r <= 1);
  return 1 - r;
}
...
void SASInstance::write_automata(std::ostream& s) const
{
  for (index_type i = 0; i < n_variables(); i++) {
    s << "automaton ";
    variables[i].name->write(s, Name::NC_INSTANCE);
    s << " :" << std::endl;
    index_vec q;
    index_type vi = init_state.value_of(i);
    assert(vi < variables[i].n_values());
    q.append(vi);
    index_type p = 0;
    while (p < q.length()) {
      for (index_type k = 0; k < n_actions(); k++) {
	if (actions[k].pre.value_of(i) == q[p]) {
	  index_type vpost = actions[k].post.value_of(i);
	  assert(vpost < variables[i].n_values());
	  s << q[p] << " " << vpost << " ";
	  actions[k].name->write(s, Name::NC_INSTANCE);
	  s << " " << PRINT_NTYPE(actions[k].cost / actions[k].post.length())
	    << std::endl;
	  if (!q.contains(vpost))
	    q.append(vpost);
	}
	else if (actions[k].prv.value_of(i) == q[p]) {
	  s << q[p] << " " << q[p] << " ";
	  actions[k].name->write(s, Name::NC_INSTANCE);
	  s << " 0" << std::endl;
	}
    ...
    index_type gi = goal_state.value_of(i);
    if (gi != no_such_index) {
      s << gi << " 0" << std::endl;
    }
    else {
      for (index_type j = 0; j < q.length(); j++)
	s << q[j] << " 0" << std::endl;
    }
  }
}
...
// void write_invariants_for_downward
// (Instance& ins, SASInstance& sas_ins, ::std::ostream& s)
// {
//   assert(sas_ins.atom_map_defined() &&
// 	 (sas_ins.atom_map_n() == ins.n_atoms()));
//   bool_vec cov(false, ins.n_atoms());
//   for (index_type k = 0; k < ins.n_invariants(); k++)
//     if (ins.invariants[k].lim == 1)
//       cov.insert(ins.invariants[k].set);
//   s << "begin_groups" << std::endl;
//   s << ins.n_invariants() + cov.count(false) << std::endl;
//   for (index_type k = 0; k < ins.n_invariants(); k++)
//     if (ins.invariants[k].lim == 1) {
//       s << "group" << std::endl;
//       s << ins.invariants[k].set.size() << std::endl;
//       for (index_type i = 0; i < ins.invariants[k].set.size(); i++) {
// 	index_type ith_atom = ins.invariants[k].set[i];
// 	partial_state ith_ps(sas_ins.atom_map_defined()[ith_atom]);
// 	assert(ith_ps.size() == 1);
// 	s << ith_ps[0].first << ' ' << ith_ps[0].second << ' ';
// 	//PDDL_Name* atm_pddl_name =
// 	//  (PDDL_Name*)ins.atoms[ith_atom].name->cast_to("PDDL_Name");
// 	//if (atm_pddl_name) {
// 	//  atm_pddl_name->write_for_downward(s);
// 	//  s << std::endl;
// 	//}
// 	//else {
// 	//  ins.atoms[ith_atom].name->write(s, Name::NC_INSTANCE);
// 	//  s << ' ' << 0 << std::endl;
// 	//}
// 	ins.atoms[ith_atom].name->write(s, Name::NC_INSTANCE);
// 	s << ' ' << 0 << std::endl;
...
//       s << "group" << std::endl;
//       s << 1 << std::endl;
//       partial_state ps(sas_ins.atom_map_defined()[k]);
//       assert(ps.size() == 1);
//       s << ps[0].first << ' ' << ps[0].second << ' ';
//       //PDDL_Name* atm_pddl_name =
//       //	(PDDL_Name*)ins.atoms[k].name->cast_to("PDDL_Name");
//       //if (atm_pddl_name) {
//       //	atm_pddl_name->write_for_downward(s);
//       //	s << std::endl;
//       //}
//       //else {
//       //	ins.atoms[k].name->write(s, Name::NC_INSTANCE);
//       //	s << ' ' << 0 << std::endl;
//       //}
//       ins.atoms[k].name->write(s, Name::NC_INSTANCE);
//       s << ' ' << 0 << std::endl;
...
(Instance& ins, SASInstance& sas_ins, ::std::ostream& s)
{
  assert(sas_ins.atom_map_defined() &&
	 (sas_ins.atom_map_n() == ins.n_atoms()));
  s << "begin_groups" << std::endl;
  s << sas_ins.n_variables() << std::endl;
  for (index_type k = 0; k < sas_ins.n_variables(); k++) {
    s << "group" << std::endl;
    s << sas_ins.variables[k].n_values() << std::endl;
    for (index_type i = 0; i < sas_ins.variables[k].n_values(); i++) {
      s << k << " " << i << " ";
      sas_ins.variables[k].domain[i]->write(s, Name::NC_INSTANCE);
      s << " 0" << std::endl;
    }
  }
  s << "end_groups" << std::endl;  
...
void write_sasv2_for_downward
(SASInstance& sas_ins, ::std::ostream& s)
{
  // metric spec?
  s << "begin_metric" << std::endl;
  s << 1 << std::endl;
  s << "end_metric" << std::endl;
  // variables section
  s << "begin_variables" << std::endl;
  s << sas_ins.n_variables() << std::endl;
  for (index_type k = 0; k < sas_ins.n_variables(); k++) {
    //sas_ins.variables[k].name->write(s, Name::NC_INSTANCE);
    s << "var" << k;
    s << " " << sas_ins.variables[k].n_values() << " -1" << std::endl;
  }
  s << "end_variables" << std::endl;
  // initial state
  s << "begin_state" << std::endl;
  assert(sas_ins.init_state.size() == sas_ins.n_variables());
  for (index_type k = 0; k < sas_ins.n_variables(); k++) {
    assert(sas_ins.init_state[k].first == k);
    index_type val_k = sas_ins.init_state[k].second;
    assert(val_k < sas_ins.variables[k].n_values());
    s << val_k << std::endl;
  }
  // for (index_type k = 0; k < sas_ins.n_variables(); k++) {
  //   index_type val_k = sas_ins.init_state.value_of(k);
  //   assert(val_k < sas_ins.variables[k].n_values());
  //   s << val_k << std::endl;
  // }
  s << "end_state" << std::endl;
  // goal
  s << "begin_goal" << std::endl;
  s << sas_ins.goal_state.size() << std::endl;
  for (index_type k = 0; k < sas_ins.goal_state.size(); k++) {
    s << sas_ins.goal_state[k].first
      << ' ' << sas_ins.goal_state[k].second << std::endl;
  }
  s << "end_goal" << std::endl;
  s << sas_ins.n_actions() << std::endl;
  for (index_type k = 0; k < sas_ins.n_actions(); k++) {
    s << "begin_operator" << std::endl;
    PDDL_Name* act_pddl_name =
      (PDDL_Name*)sas_ins.actions[k].name->cast_to("PDDL_Name");
    if (act_pddl_name) {
      act_pddl_name->write_without_parentheses(s);
      s << std::endl;
    }
    else {
      sas_ins.actions[k].name->write(s, Name::NC_INSTANCE);
      s << std::endl;
    }
    s << sas_ins.actions[k].prv.size() << std::endl;
    for (index_type i = 0; i < sas_ins.actions[k].prv.size(); i++)
      s << sas_ins.actions[k].prv[i].first
	<< ' ' << sas_ins.actions[k].prv[i].second << std::endl;
    s << sas_ins.actions[k].post.size() << std::endl;
    for (index_type i = 0; i < sas_ins.actions[k].post.size(); i++) {
      index_type var_i = sas_ins.actions[k].post[i].first;
      index_type post_val = sas_ins.actions[k].post[i].second;
      index_type pre_val = sas_ins.actions[k].pre.value_of(var_i);
      s << 0 << ' ' << var_i << ' ';
      if (pre_val == no_such_index)
	s << -1;
      else
    ...
    //s << sas_ins.actions[k].cost.numerator() << std::endl;
    s << FLOOR_TO_INT(sas_ins.actions[k].cost) << std::endl;
    s << "end_operator" << std::endl;
  }
  // no axioms
  s << 0 << std::endl;
}

void write_sasv3_for_downward
(SASInstance& sas_ins, bool downward_naming, bool metric,
 Heuristic* inc, ::std::ostream& s)
{
  // version section
  s << "begin_version" << std::endl
    << 3 << std::endl
  ...
  for (index_type k = 0; k < sas_ins.n_variables(); k++) {
    ...
    s << "-1" << std::endl;
    // The fourth line specifies the variable's range
    s << sas_ins.variables[k].n_values() << std::endl;
    // The following range lines specify the symoblic names for each
    // of the range values the variable can take
    for (index_type i = 0; i < sas_ins.variables[k].n_values(); i++) {
      if (downward_naming) {
	if ((sas_ins.variables[k].n_values() == 2) &
	    (sas_ins.variables[k].domain[i] == SASInstance::val_true)) {
	  PDDL_Name* pn =
	    (PDDL_Name*)sas_ins.variables[k].name->cast_to("PDDL_Name");
	  assert(pn);
	  s << "Atom ";
	  pn->write_FOL(s);
	  s << std::endl;
	}
	else if ((sas_ins.variables[k].n_values() == 2) &
		 (sas_ins.variables[k].domain[i] == SASInstance::val_false)) {
	  s << "<none of those>" << std::endl;
	}
	else if ((sas_ins.variables[k].domain[i] == SASInstance::val_nil)) {
	  s << "<none of those>" << std::endl;
	}
	else {
	  PDDL_Name* pn =
	    (PDDL_Name*)sas_ins.variables[k].domain[i]->cast_to("PDDL_Name");
	  assert(pn);
	  s << "Atom ";
	  pn->write_FOL(s);
	  s << std::endl;
    ...
    s << "end_variable" << std::endl;
  }
  // mutex group section
  // because we (normally) use all invariants to define variables,
  // each (non-binary) mutex is represented by some variable; but
  // this does not explicitly show the cross-variable mutexes.
  lvector<pair_set> groups;
  const partial_state* am = sas_ins.atom_map_defined();
  if (am != 0) {
    for (index_type k = 0; k < sas_ins.n_variables(); k++) {
      for (index_type i = 0; i < sas_ins.variables[k].s_index.size(); i++) {
    	partial_state m(am[sas_ins.variables[k].s_index[i]]);
    	m.undefine(k);
    	for (index_type l = 0; l < m.size(); l++) {
    	  pair_set g;
    	  g.insert(m[l]);
    	  for (index_type j = 0; j < sas_ins.variables[k].n_values(); j++)
    	    if (j != i)
    	      g.insert(index_pair(k, j));
    	  groups.append(g);
    	}
      }
    }
    index_type n_groups = groups.size();
    if (inc != 0) {
      for (index_type i = 0; i < sas_ins.atom_map_n(); i++)
	for (index_type j = i + 1; j < sas_ins.atom_map_n(); j++)
	  for (index_type ii = 0; ii < am[i].size(); ii++)
	    for (index_type jj = 0; jj < am[j].size(); jj++)
	      if (am[i][ii].first != am[j][jj].first) {
		bool found = false;
		for (index_type k = 0; (k < n_groups) && !found; k++)
		  if (groups[k].contains(am[i][ii]) &&
		      groups[k].contains(am[j][jj]))
		    found = true;
		if (!found) {
		  pair_set m;
		  m.insert(am[i][ii]);
  ...
  // The mutex section begins with a line containing a single number,
  // the number of mutex groups in the task.
  s << groups.size() << std::endl;
  for (index_type k = 0; k < groups.size(); k++) {
    // The first line is "begin_mutex_group".
    s << "begin_mutex_group" << std::endl;
    // The second line contains a single number, denoting the number
    // of facts in the mutex group.
    s << groups[k].size() << std::endl;
    // The following lines describe the facts in the group, one line for
    // each fact. Each fact is is given by two numbers separated by a
    // space, denoting the variable (indexing into the variable section
    // above, counting from 0) and a value for that variable (indexing
    // into the list of value names for that variable, counting from 0).
    for (index_type i = 0; i < groups[k].size(); i++) {
      s << groups[k][i].first << ' ' << groups[k][i].second << std::endl;
    }
    // The final line is "end_mutex_group". 
    s << "end_mutex_group" << std::endl;
  }
  ...
  for (index_type k = 0; k < sas_ins.n_actions(); k++) {
    ...
    // The third line contains a single number, denoting the number
    ...
    for (index_type i = 0; i < sas_ins.actions[k].post.size(); i++) {
      ...
      index_type post_val = sas_ins.actions[k].post[i].second;
      index_type pre_val = sas_ins.actions[k].pre.value_of(var_i);
      // An effect is always given in a single line, with the individual
      // parts separated by spaces. It is structured as follows:
      // First comes the number of effect conditions.
      // This is followed by one variable/value pair for each effect condition.
      ...
      // particular value that the variable must have.
      if (pre_val == no_such_index)
	s << -1;
      else
	s << pre_val;
    ...
    // The line before last gives the operator cost.
    if (metric)
      s << FLOOR_TO_INT(sas_ins.actions[k].cost) << std::endl;
    else
      s << 1 << std::endl;
  ...
  // axiom section
  // Similar to the operator section, the axiom section begins with a
  // line containing the number of axiom rules.
  s << 0 << std::endl;
}
...
SpanningVariableSet::SpanningVariableSet(SASInstance& ins)
  : instance(ins)
{
  compute_approximate();
}

SpanningVariableSet::~SpanningVariableSet()
{
  // done
}

SpanningVariableSet::SetOrder::SetOrder
(const index_set_vec& dset,
 const index_vec& dset_bits,
 const index_vec& dby_bits,
 const index_vec& cs,
 const index_vec& las)
  : bit_ratio(0, dset.length()), cover_size(cs), largest_add_set(las)
{
  for (index_type k = 0; k < dset.length(); k++)
    bit_ratio[k] = R_TO_N(dset_bits[k] + dby_bits[k], dset_bits[k]);
}
...
bool SpanningVariableSet::SetOrder::operator()
(const index_type& v0, const index_type& v1) const
{
  assert(v0 < bit_ratio.length());
  assert(v1 < bit_ratio.length());
  if (bit_ratio[v0] > bit_ratio[v1])
    return true;
  if ((bit_ratio[v0] == bit_ratio[v1]) &&
      (cover_size[v0] < cover_size[v1]))
    return true;
  if ((bit_ratio[v0] == bit_ratio[v1]) &&
      (cover_size[v0] == cover_size[v1]) &&
      (largest_add_set[v0] > largest_add_set[v1]))
    return true;
  return false;
}

void SpanningVariableSet::rank_sets
(const index_set& sel,
 const index_set_vec& dset,
 const index_set_vec& dby,
 index_vec& o)
{
  index_vec dset_bits(0, dset.length());
  index_vec dby_bits(0, dset.length());
  index_vec cover_size(0, dset.length());
  index_vec largest_add(0, dset.length());
  for (index_type i = 0; i < dset.length(); i++) {
    for (index_type k = 0; k < dset[i].length(); k++)
      dset_bits[i] += ilog(instance.variables[dset[i][k]].n_values());
    for (index_type k = 0; k < dby[i].length(); k++)
      dby_bits[i] += ilog(instance.variables[dby[i][k]].n_values());
  }
  for (index_type i = 0; i < dset.length(); i++) {
    index_set e(sel);
    e.insert(dset[i]);
    graph g(instance.interference_graph, e);
    index_set_vec c;
    g.apx_independent_set_cover(c);
    cover_size[i] = c.length();
    for (index_type k = 0; k < c.length(); k++)
      if (c[k].length() > largest_add[i])
	largest_add[i] = c[k].length();
  }
  o.clear();
  SetOrder so(dset, dset_bits, dby_bits, cover_size, largest_add);
  for (index_type k = 0; k < dset.length(); k++)
    o.insert_ordered(k, so);
  if (instance.trace_level > 1) {
    std::cerr << "selected: ";
    instance.write_variable_set(std::cerr, sel);
    std::cerr << std::endl << "ranking:" << std::endl;
    for (index_type k = 0; k < o.length(); k++) {
      std::cerr << o[k] << ". ";
      instance.write_variable_set(std::cerr, dset[o[k]]);
      std::cerr << ": bit ratio = " << dset_bits[o[k]] << " + "
		<< dby_bits[o[k]] << " / " << dset_bits[o[k]] << " = "
		<< PRINT_NTYPE(R_TO_N(dset_bits[o[k]] + dby_bits[o[k]],
				      dset_bits[o[k]]))
		<< ", cover size = " << cover_size[o[k]]
		<< std::endl;
    }
  }
}

void SpanningVariableSet::compute_approximate()
{
  index_set::clear();
  index_set_vec dset;
  index_set_vec dby;
  for (index_type k = 0; k < instance.n_variables(); k++) {
    SASInstance::additional_strictness_check = false;
    index_set s;
    SASInstance::determined_check_result r =
      instance.determining_set(k, s);
    bool ok = false;
    if ((r == SASInstance::determined_goal_preserving) ||
	(r == SASInstance::determined_strict))
      ok = true;
    if ((r == SASInstance::determined_yes) &&
	!instance.goal_state.defines(k))
      ok = true;
    if (ok) {
      index_type i = dset.first(s);
      if (i != no_such_index) {
	dby[i].insert(k);
      }
      else {
  ...
  assert(dset.length() == dby.length());
  for (index_type i = 0; i < dset.length(); i++)
    for (index_type j = 0; j < dset.length(); j++)
      if ((i != j) && dset[i].contains(dset[j]))
	dby[i].insert(dby[j]);
  std::cerr << "determining sets / determined:" << std::endl;
  for (index_type k = 0; k < dset.length(); k++)
    std::cerr << dset[k] << " / " << dby[k] << std::endl;
  std::cerr << "not determined: " << *this << std::endl;
  index_set cov;
  while ((index_set::length() + cov.length()) < instance.n_variables()) {
    assert(dset.length() > 0);
    index_vec ranking;
    rank_sets(*this, dset, dby, ranking);
    assert(ranking.length() == dset.length());
    index_type i_best = ranking[0];
    std::cerr << "best ranked: " << dset[i_best] << " / " << dby[i_best]
	      << std::endl;
    //assert(!index_set::have_common_element(dset[i_best]));
    index_set::insert(dset[i_best]);
    cov.insert(dby[i_best]);
    if ((index_set::length() + cov.length()) < instance.n_variables()) {
      bool_vec rm(false, dset.length());
      rm[i_best] = true;
      for (index_type k = 0; k < dset.length(); k++) if (!rm[k]) {
	dset[k].subtract(*this);
	if (dset[k].empty())
	  rm[k] = true;
	dby[k].subtract(*this);
	dby[k].subtract(cov);
