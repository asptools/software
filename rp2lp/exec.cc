...
Transition* ExecState::transition()
{
  return new EST(*this);
}
...
void ExecState::write(::std::ostream& s)
{
  ...
  for (index_type k = 0; k < actions.length(); k++) {
    s << " ;; " << actions[k].act << ". "
      << instance.actions[actions[k].act].name << ", "
...
(index_type a, ExecErrorSet* errors, index_type step)
{
  bool pre_ok = true;
  bool ext_ok = true;
  bool compat_ok = true;
  bool res_ok = true;
  bool cons_ok = true;
  for (index_type k = 0; k < instance.actions[a].pre.length(); k++) {
    bool is_sat = atoms[instance.actions[a].pre[k]];
    if (!is_sat) {
      if (trace_level > 0) {
	::std::cerr << "action " << instance.actions[a].name
		    << ": precondition "
		    << instance.atoms[instance.actions[a].pre[k]].name
		    << " not satisfied"
		    << ::std::endl;
      }
      if (errors) {
	...
	UnsatisfiedPreconditionError* upe =
	  new UnsatisfiedPreconditionError(instance, a,
					   instance.actions[a].pre[k], s,
					   abs_t, step);
  ...
  ext_ok = check_extended_applicability(instance.actions[a], errors, step);
  amt_vec in_use(0, instance.n_resources());
  for (index_type k = 0; k < actions.length(); k++) {
    if (!instance.non_interfering(a, actions[k].act) ||
	!instance.lock_compatible(a, actions[k].act)) {
      if (trace_level > 0) {
	::std::cerr << "action " << instance.actions[a].name
		    << ": conflict with "
	...
	IncompatibleActionError* iae =
	  new IncompatibleActionError(instance, a, k, abs_t, step);
  ...
  for (index_type r = 0; r < instance.n_resources(); r++) {
    if (instance.actions[a].cons[r] > res[r]) {
      if (trace_level > 0) {
	::std::cerr << "action " << instance.actions[a].name
	...
	ResourceShortageError* rse = 
	  new ResourceShortageError(instance, r, res[r], a, abs_t, step);
	errors->new_error(rse);
      }
      cons_ok = false;
    }
    if ((in_use[r] + instance.actions[a].req(r)) > res[r]) {
      if (trace_level > 0) {
	::std::cerr << "action " << instance.actions[a].name
		    << ": not enough of resource "
  ...
  return (pre_ok && ext_ok && compat_ok && res_ok && cons_ok);
}

void ExecState::apply
(index_type a, ExecErrorSet* errors, index_type step)
{
  exec_act e(a, instance.actions[a].dur, step);
  save_extended_state_info(e);
  for (index_type k = 0; k < instance.actions[a].del.length(); k++)
    atoms[instance.actions[a].del[k]] = false;
  apply_extended_action_start_effects(instance.actions[a], e, atoms);
  actions.append(e);
}
...
void ExecState::advance(NTYPE dt, ExecErrorSet* errors)
{
  ...
  for (index_type k = 0; k < actions.length(); k++) {
	...
	::std::cerr << "action " << a.name << " started at "
      ...
      added.insert(a.add);
      apply_extended_action_end_effects(a, actions[k], added);
      free_extended_state_info(actions[k]);
      bool is_red = true;
	...
	if (errors) {
	  RedundantActionWarning* raw =
	    new RedundantActionWarning(instance, a.index,
				       abs_t+dt, actions[k].step);
...
bool ExecState::check_extended_applicability
(Instance::Action& a, ExecErrorSet* errors, index_type step)
{
  return true;
}

void ExecState::apply_extended_action_start_effects
(Instance::Action& a, exec_act& e, bool_vec& s)
{
  // does nothing
}

void ExecState::apply_extended_action_end_effects
(Instance::Action& a, exec_act& e, bool_vec& s)
{
  // does nothing
}

void ExecState::save_extended_state_info(exec_act& e)
{
  // does nothing
}

void ExecState::free_extended_state_info(exec_act& e)
{
  // does nothing
}
...
const ExecState::exec_act_vec& ExecState::current_actions() const
{
  return actions;
}
...
bool ExecState::is_stable() const
{
...
bool ExecState::test_conjunction(const index_set& set) const
{
...
bool ExecState::test_disjunction(const index_set& set) const
{
  for (index_type k = 0; k < set.length(); k++)
    if (atoms[set[k]]) return true;
  return false;
}
...
ExecTrace* ExecTrace::stable_trace()
{
  ExecTrace* t = new ExecTrace(instance);
  for (index_type k = 0; k < length(); k++)
    if ((*this)[k]->is_stable())
...
bool ExecTrace::test_always_conjunction(const index_set& c)
{
  for (index_type k = 0; k < length(); k++) {
    if (!(*this)[k]->test_conjunction(c)) return false;
  }
  return true;
}

bool ExecTrace::test_always_disjunction(const index_set& d)
{
  int trace_level = (length() > 0 ? (*this)[0]->trace_level : 0);
  if (trace_level > 1) {
    std::cerr << "evaluating ALWAYS DISJUNCTION ";
    instance.write_atom_set(std::cerr, d);
    std::cerr << "..." << std::endl;
  }
  for (index_type k = 0; k < length(); k++) {
    if (trace_level > 1) {
      std::cerr << "evaluating disjunction ";
      instance.write_atom_set(std::cerr, d);
      std::cerr << " in state " << *((*this)[k]) << "...";
    }
    if (!(*this)[k]->test_disjunction(d)) {
      if (trace_level > 1) {
	std::cerr << "false - ALWAYS CONSTRAINT FALSIFIED" << std::endl;
      }
      return false;
    }
    if (trace_level > 1) {
  ...
  if (trace_level > 1) {
    std::cerr << "ALWAYS CONSTRAINT SATISFIED" << std::endl;
  }
  return true;
}
...
bool ExecTrace::test_sometime_conjunction(const index_set& c)
{
  int trace_level = (length() > 0 ? (*this)[0]->trace_level : 0);
  if (trace_level > 1) {
    std::cerr << "evaluating SOMETIME CONJUNCTION ";
    instance.write_atom_set(std::cerr, c);
    std::cerr << "..." << std::endl;
  }
  for (index_type k = 0; k < length(); k++) {
    if (trace_level > 1) {
      std::cerr << "evaluating conjunction ";
      instance.write_atom_set(std::cerr, c);
      std::cerr << " in state " << *((*this)[k]) << "...";
    }
    if ((*this)[k]->test_conjunction(c)) {
      if (trace_level > 1) {
	std::cerr << "true - SOMETIME CONSTRAINT SATISFIED" << std::endl;
      }
      return true;
    }
    if (trace_level > 1) {
      std::cerr << "not yet true" << std::endl;
    }
  }
  if (trace_level > 1) {
    std::cerr << "SOMETIME CONSTRAINT FALSIFIED" << std::endl;
  }
  return false;
}

bool ExecTrace::test_sometime_disjunction(const index_set& d)
{
  for (index_type k = 0; k < length(); k++) {
    if ((*this)[k]->test_disjunction(d)) return true;
  }
  return false;
}
...
{
  for (index_type k = 0; k < length(); k++) {
    if ((*this)[k]->current_atoms()[q] && !(*this)[k]->current_atoms()[p]) {
      return true;
    }
...
bool ExecTrace::test_at_most_once_conjunction(const index_set& c)
{
  bool f = false;
  bool g = false;
  for (index_type k = 0; k < length(); k++) {
    if (g) {
      ...
      if (!(*this)[k]->test_conjunction(c)) g = true;
    }
    else {
      if ((*this)[k]->test_conjunction(c)) f = true;
    }
  }
  return true;
}

bool ExecTrace::test_at_most_once_disjunction(const index_set& d)
{
  bool f = false;
  bool g = false;
  for (index_type k = 0; k < length(); k++) {
    if (g) {
      if ((*this)[k]->test_disjunction(d)) return false;
    }
    else if (f) {
      if (!(*this)[k]->test_disjunction(d)) g = true;
    }
    else {
      if ((*this)[k]->test_disjunction(d)) f = true;
    }
  }
  return true;
}

bool ExecTrace::find_always
(index_type k, bool_vec& p, Heuristic* inc, index_type lim, index_set_vec& c)
{
  if (k >= length()) {
    if (c.first_subset(p) != no_such_index)
      return true;
    bool_vec sups(false, c.length());
    for (index_type i = 0; i < c.length(); i++)
      if (c[i].contains(p)) sups[i] = true;
    c.remove(sups);
    index_set& s = c.append();
    s.insert(p);
    return true;
  }
  else {
    bool holds = false;
    for (index_type i = 0; (i < instance.n_atoms()) && !holds; i++)
      if (p[i] && (*this)[k]->current_atoms()[i])
	holds = true;
    if (holds) {
      return find_always(k + 1, p, inc, lim, c);
    }
    else if (p.count(true) < lim) {
      bool found = false;
      for (index_type i = 0; i < instance.n_atoms(); i++)
	if (!p[i] && (*this)[k]->current_atoms()[i]) {
	  bool ok = true;
	  if (instance.atoms[i].neg != no_such_index) {
	    if (p[instance.atoms[i].neg])
	      ok = false;
	    if (ok && inc)
	      for (index_type j = 0; (j < instance.n_atoms()) && ok; j++)
		if (p[j] && (instance.atoms[j].neg != no_such_index))
		  if (INFINITE(inc->eval(instance.atoms[i].neg,
					 instance.atoms[j].neg)))
		    ok = false;
	  }
	  if (ok) {
	    p[i] = true;
	    if (find_always(k + 1, p, inc, lim, c))
	      found = true;
	    p[i] = false;
	  }
	}
...
void ExecTrace::find_always
(Heuristic* inc, index_type lim, index_set_vec& c_out)
{
  c_out.clear();
  bool_vec p(false, instance.n_atoms());
  find_always(0, p, inc, lim, c_out);
}

void ExecTrace::find_always
(Heuristic* inc, index_set_vec& c_out)
{
  c_out.clear();
  index_type n = 0;
  index_type l = 1;
  while (l < instance.n_atoms()) {
    bool_vec p(false, instance.n_atoms());
    find_always(0, p, inc, l, c_out);
    if ((n > 0) && (c_out.length() == n))
      return;
    n = c_out.length();
    l += 1;
  }
...
void ExecTrace::find_always
(Heuristic* inc, index_type lim, const index_set_vec& c_in,
 index_set_vec& c_out)
{
  c_out.clear();
  for (index_type k = 0; k < c_in.length(); k++)
    if (c_in[k].length() <= lim) {
      bool_vec p(c_in[k], instance.n_atoms());
      find_always(0, p, inc, lim, c_out);
    }
}
...
void ExecTrace::find_order_constraints
(graph* lms, index_set& e, pair_set& sb, pair_set& sa)
{
  e.clear();
  sb.clear();
  for (index_type k = 0; k < length(); k++) {
    bool_vec new_atoms((*this)[k]->current_atoms());
    new_atoms.subtract(e);
    for (index_type i = 0; i < instance.n_atoms(); i++)
      if (new_atoms[i]) {
	for (index_type j = 0; j < e.size(); j++) {
	  bool keep = !instance.atoms[e[j]].init;
	  if ((lms != NULL) && keep) {
	    if (lms->adjacent(e[j], i)) keep = false;
	  }
	  if (keep)
	    sb.insert(index_pair(i, e[j]));
	}
      }
    e.insert(new_atoms);
  }
  bool_vec r(false, instance.n_atoms());
  sa.clear();
  for (index_type k = length(); k > 0; k--) {
    bool_vec new_atoms((*this)[k-1]->current_atoms());
    new_atoms.subtract(r);
    for (index_type i = 0; i < instance.n_atoms(); i++)
      if (new_atoms[i] && !instance.atoms[i].init) {
	for (index_type j = 0; j < instance.n_atoms(); j++)
	  if (r[j])
	    sa.insert(index_pair(i, j));
      }
    r.insert(new_atoms);
  }
}

void ExecTrace::true_at_any_time(bool_vec& atoms)
{
  atoms.assign_value(false, instance.n_atoms());
  for (index_type k = 0; k < length(); k++)
    atoms.insert((*this)[k]->current_atoms());
}
...
void Schedule::output(Plan& plan, bool mark_end) const
{
  NTYPE t = 0;
  for (index_type k = 0; k < steps.length(); k++) {
...
(Plan& plan, const index_vec& act_map, bool mark_end) const
{
  NTYPE t = 0;
  for (index_type k = 0; k < steps.length(); k++) {
    NTYPE dt = steps[k].at - t;
...
(ExecState* cs, ExecTrace* trace, ExecErrorSet* errors, bool finish) const
{
  if (trace_level > 0) {
  ...
  while (t < POS_INF) {
	...
	if (!cs->applicable(steps[k].act, errors, k)) {
	  if (trace_level > 0) {
	    ::std::cerr << steps[k].at << ": "
...
{
  ExecState cs(instance, instance.init_atoms);
  return simulate(&cs, trace, errors, finish);
}
...
void Schedule::causal_links(index_set_graph& cl) const
{
  cl.init(steps.length() + 1);
  index_vec source(no_such_index, instance.n_atoms());
  for (index_type i = 0; i < instance.n_atoms(); i++)
    if (instance.atoms[i].init)
      source[i] = steps.length() + 1;
  NTYPE next_t = MIN(next_start_time(NEG_INF), next_finish_time(NEG_INF));
  while (next_t < POS_INF) {
    // actions finishing at next_t
    for (index_type k = 0; k < steps.length(); k++)
      if ((steps[k].at + instance.actions[steps[k].act].dur) == next_t) {
	const Instance::Action& act = instance.actions[steps[k].act];
	for (index_type i = 0; i < act.add.length(); i++)
	  source[act.add[i]] = k;
      }
    // actions starting at next_t
    for (index_type k = 0; k < steps.length(); k++)
      if (steps[k].at == next_t) {
	const Instance::Action& act = instance.actions[steps[k].act];
	for (index_type i = 0; i < act.pre.length(); i++) {
	  assert(source[act.pre[i]] != no_such_index);
	  if (source[act.pre[i]] < steps.length())
	    cl.add_edge(source[act.pre[i]], k, act.pre[i]);
	}
      }
    next_t = MIN(next_start_time(next_t), next_finish_time(next_t));
  }
  for (index_type i = 0; i < instance.goal_atoms.length(); i++) {
    assert(source[instance.goal_atoms[i]] != no_such_index);
    if (source[instance.goal_atoms[i]] < steps.length())
      cl.add_edge(source[instance.goal_atoms[i]], steps.length(),
		  instance.goal_atoms[i]);
  }
}

void Schedule::make_pop(index_set_graph& clt) const
{
  causal_links(clt);
  graph prec(clt);
  ...
  for (index_type i = 0; i < steps.length(); i++)
	...
	if (!prec.adjacent(i, j))
	  if (!instance.non_interfering(steps[i].act, steps[j].act))
	    clt.add_edge(i, j);
}
...
void Schedule::deorder_non_temporal(graph& prec) const
{
  prec.init(steps.length());
  for (index_type i = 0; i < steps.length(); i++)
    for (index_type j = 0; j < steps.length(); j++)
	...
	// else {
	//   std::cerr << "deordering steps " << i << " and " << j << std::endl;
	//   instance.print_action(std::cerr, instance.actions[steps[i].act]);
	//   instance.print_action(std::cerr, instance.actions[steps[j].act]);
	// }
}
...
index_type Schedule::action_multiset_difference(const Schedule& s) const
{
  index_vec c(0, instance.n_actions());
  for (index_type k = 0; k < steps.size(); k++)
    c[steps[k].act] += 1;
  for (index_type k = 0; k < s.steps.size(); k++)
    if (c[s.steps[k].act] > 0)
      c[s.steps[k].act] -= 1;
    else
      c[s.steps[k].act] += 1;
  index_type d = 0;
 ...
 ExecTrace* trace, RNG& rnd)
{
  ...
  while (true) {
    ...
    for (index_type k = 0; k < instance.n_actions(); k++)
      app[k] = cs.applicable(k, 0, no_such_index);
    app.copy_to(s_app);
    if (s_app.empty()) {
      if (trace_level > 0) {
    ...
    cs.apply(s_app[sel], 0, no_such_index);
...
TraceSet::TraceSet(Instance& ins)
  : lvector< std::pair<ExecTrace*, Schedule*> >(std::pair<ExecTrace*, Schedule*>(0, 0), 0), instance(ins)
{
  // done
}
...
void TraceSet::add_trace(ExecTrace* t, Schedule* s)
{
  append(std::pair<ExecTrace*, Schedule*>(t, s));
}

void TraceSet::add_plan(Schedule* s, bool untimed)
{
  cost_vec d;
  instance.save_durations(d);
  if (untimed)
    instance.assign_unit_durations();
  ExecTrace* t = new ExecTrace(instance);
  s->simulate(t, 0, true);
  if (untimed) {
    ExecTrace* st = t->stable_trace();
    delete t;
    t = st;
  }
  append(std::pair<ExecTrace*, Schedule*>(t, s));
  if (untimed)
    instance.set_durations(d);
}

void TraceSet::add_plan_if_executable(Schedule* s, bool untimed)
{
  cost_vec d;
  instance.save_durations(d);
  if (untimed)
    instance.assign_unit_durations();
  ExecTrace* t = new ExecTrace(instance);
  ExecErrorSet* e = new ExecErrorSet();
  s->simulate(t, e, false);
  if (e->executable()) {
    if (untimed) {
      ExecTrace* st = t->stable_trace();
      delete t;
...
void TraceSet::add_plan_if_valid(Schedule* s, bool untimed)
{
  cost_vec d;
  instance.save_durations(d);
  if (untimed)
...
void TraceSet::test_always_conjunction
(const index_set& s, bool_vec& v)
{
  v.set_length(length());
  for (index_type k = 0; k < length(); k++)
    v[k] = (*this)[k].first->test_always_conjunction(s);
}

void TraceSet::test_always_disjunction
(const index_set& s, bool_vec& v)
{
  v.set_length(length());
  for (index_type k = 0; k < length(); k++)
    v[k] = (*this)[k].first->test_always_disjunction(s);
}

void TraceSet::test_sometime_conjunction
(const index_set& s, bool_vec& v)
{
  v.set_length(length());
  for (index_type k = 0; k < length(); k++)
    v[k] = (*this)[k].first->test_sometime_conjunction(s);
}

void TraceSet::test_sometime_disjunction
(const index_set& s, bool_vec& v)
{
  v.set_length(length());
  for (index_type k = 0; k < length(); k++)
    v[k] = (*this)[k].first->test_sometime_disjunction(s);
}

void TraceSet::test_at_most_once_conjunction
(const index_set& s, bool_vec& v)
{
  v.set_length(length());
  for (index_type k = 0; k < length(); k++)
    v[k] = (*this)[k].first->test_at_most_once_conjunction(s);
}

void TraceSet::test_at_most_once_disjunction
(const index_set& s, bool_vec& v)
{
  v.set_length(length());
  for (index_type k = 0; k < length(); k++)
    v[k] = (*this)[k].first->test_at_most_once_disjunction(s);
}

void TraceSet::find_common_always
(Heuristic* inc, index_type lim, index_set_vec& c)
{
  c.clear();
  if (length() == 0) return;
  std::cerr << "checking first trajectory..." << std::endl;
  (*this)[0].first->find_always(inc, lim, c);
  for (index_type k = 1; (k < length()) && (c.length() > 0); k++) {
    std::cerr << c.length() << " constraints, checking trajectory "
	      << k + 1 << "..." << std::endl;
    index_set_vec c_tmp(c);
    (*this)[k].first->find_always(inc, lim, c_tmp, c);
  }
}

void TraceSet::find_common_order_constraints
(graph* lms, index_set& e, pair_set& sb, pair_set& sa)
{
  if (length() == 0) {
    e.clear();
  ...
  std::cerr << "checking first trajectory..." << std::endl;
  (*this)[0].first->find_order_constraints(lms, e, sb, sa);
  for (index_type k = 1; k < length(); k++) {
    std::cerr << e.length() << " / " << sb.length() << " / " << sa.length()
	      << " constraints, checking trajectory " << k + 1 << "..."
	      << std::endl;
    index_set e1;
    pair_set sb1;
    pair_set sa1;
    (*this)[k].first->find_order_constraints(lms, e1, sb1, sa1);
    e.intersect(e1);
    sb.intersect(sb1);
    sa.intersect(sa1);
  }
...
bool TraceSet::add_constraint_if_not_dominated
(const index_set& nc, bool nisx, const bool_vec& nex,
 index_set_vec& c, bool_vec& isx, bool_matrix& ex, index_type& max_ex) const
{
  bool non_dom = true;
  bool_vec dom(false, c.length());
  for (index_type k = 0; (k < c.length()) && non_dom; k++) {
    if ((isx[k] == nisx) && (c[k].length() < nc.length()) && nex.subset(ex[k]))
      non_dom = false;
    if ((isx[k] == nisx) && (c[k].length() == nc.length()) && nex.strict_subset(ex[k]))
      non_dom = false;
    if ((isx[k] == nisx) && (c[k].length() == nc.length()) && nex.strict_superset(ex[k]))
      dom[k] = true;
  }
  if (non_dom) {
    c.remove(dom);
    ...
    isx.append(nisx);
    ex.append(nex);
    if (nex.count(true) > max_ex)
      max_ex = nex.count(true);
  }
  return non_dom;
}

void TraceSet::find_separating_always
(index_type p, index_type lim,
 index_set_vec& c, bool_vec& is_neg, bool_matrix& ex)
{
  c.set_length(0);
  is_neg.set_length(0);
  ex.set_size(0, length());
  index_type l = 1;
  index_set rem;
  rem.fill(instance.n_atoms());
  index_type max_ex = 0;
  while ((l <= lim) && !rem.empty() && (max_ex < (length() - 1))) {
    std::cerr << "checking always constraints of size " << l
	      << "..." << std::endl;
    index_set used;
    c.union_set(used);
    rem.subtract(used);
    mSubsetEnumerator e(rem.length(), l);
    bool more = e.first();
    while (more) {
      index_set s;
      e.current_set(rem, s);
      bool f = false;
      if ((*this)[p].first->test_always_disjunction(s)) {
	bool_vec v;
	test_always_disjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  f = add_constraint_if_not_dominated(s, false, v, c,
					      is_neg, ex, max_ex);
      }
      if (!f && !((*this)[p].first->test_sometime_conjunction(s))) {
	bool_vec v;
	test_sometime_conjunction(s, v);
	if (v.count(true) > 0)
	  add_constraint_if_not_dominated(s, true, v, c, is_neg, ex, max_ex);
      }
      more = e.next();
    }
    std::cerr << c.length() << " constraints found, max # ex. = "
	      << max_ex << " of " << length() - 1 << std::endl;
    l += 1;
  }
}

void TraceSet::find_separating_sometime
(index_type p, index_type lim,
 index_set_vec& c, bool_vec& is_neg, bool_matrix& ex)
{
  c.set_length(0);
  ...
  index_type max_ex = 0;
  while ((l <= lim) && !rem.empty() && (max_ex < (length() - 1))) {
    std::cerr << "checking sometime constraints of size " << l
	      << "..." << std::endl;
    index_set used;
    c.union_set(used);
    rem.subtract(used);
    ...
    while (more) {
      ...
      e.current_set(rem, s);
      bool f = false;
      if ((*this)[p].first->test_sometime_conjunction(s)) {
	bool_vec v;
	test_sometime_conjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
      ...
      if (!f && !((*this)[p].first->test_always_disjunction(s))) {
	bool_vec v;
	test_always_disjunction(s, v);
	if (v.count(true) > 0)
	  f = add_constraint_if_not_dominated(s, true, v, c,
					      is_neg, ex, max_ex);
      }
      more = e.next();
    }
...
void TraceSet::find_separating_at_most_once
(index_type p, index_type lim,
 index_set_vec& c, bool_vec& is_d, bool_matrix& ex)
{
  c.set_length(0);
  is_d.set_length(0);
  ex.set_size(0, length());
  ...
  index_type max_ex = 0;
  while ((l <= lim) && !rem.empty() && (max_ex < (length() - 1))) {
    std::cerr << "checking at-most-once constraints of size " << l
	      << "..." << std::endl;
    index_set used;
    c.union_set(used);
    rem.subtract(used);
    ...
    while (more) {
      ...
      e.current_set(rem, s);
      bool f = false;
      if ((*this)[p].first->test_at_most_once_conjunction(s)) {
	bool_vec v;
	test_at_most_once_conjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  f = add_constraint_if_not_dominated(s, false, v, c,
					      is_d, ex, max_ex);
      }
      if (!f && (*this)[p].first->test_at_most_once_disjunction(s)) {
	bool_vec v;
	test_at_most_once_disjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  add_constraint_if_not_dominated(s, true, v, c, is_d, ex, max_ex);
      }
      more = e.next();
    }
    std::cerr << c.length() << " constraints found, max # ex. = "
...
index_type TraceSet::PlanConstraint::ltype() const
{
  // type 1: positive atomic A/E constraints
  if (((pc_type == pc_always) ||
       (pc_type == pc_always_disjunction) ||
       (pc_type == pc_sometime) ||
       (pc_type == pc_sometime_disjunction)) &&
      (c_atoms.length() == 1))
    return 1;
  // type 2: negated atomic A/E constraints
  if (((pc_type == pc_never) ||
       (pc_type == pc_negated_sometime)) &&
      (c_atoms.length() == 1))
    return 2;
  // type 3: positive atomic A/E or O constraints
  if (((pc_type == pc_always) ||
       (pc_type == pc_sometime) ||
       (pc_type == pc_at_most_once)) &&
      (c_atoms.length() == 1))
    return 3;
  // type 4: union of types 2 and 3
  // type 5: positive disjunctive A/conjunctive E constraints
  if ((pc_type == pc_always_disjunction) ||
      (pc_type == pc_sometime))
    return 5;
  // type 6: negated disjunctive A/conjunctive E constraints
  if ((pc_type == pc_never_disjunction) ||
      (pc_type == pc_negated_sometime))
    return 6;
  // type 7: positive disjunctive A/conjunctive E/disj. or conj. O
  if ((pc_type == pc_at_most_once) ||
      (pc_type == pc_at_most_once_disjunction))
    return 7;
  // type 8: union of types 6 and 7

  // other: outside lattice (undefined type)
  return index_type_max;
}

bool TraceSet::PlanConstraint::below(index_type lt) const
{
  index_type ltype0 = ltype();
  switch (lt) {
  case 2:
    return (ltype0 == 1);
  case 3:
  ...
  case 5:
    return (ltype0 == 1);
  case 6:
    return ((ltype0 <= 3) || (ltype0 == 5));
  case 7:
    return ((ltype0 == 1) || (ltype0 == 3) || (ltype0 == 5));
  case 8:
    return (ltype0 < 8);
  default:
    return false;
...
bool TraceSet::PlanConstraint::dominates(const PlanConstraint& pc) const
{
  if (ex.strict_superset(pc.ex)) {
    if ((below(pc.ltype()) || (pc_type == pc.pc_type)) &&
	(c_atoms.length() <= pc.c_atoms.length()))
      return true;
    else
      return false;
  }
    ...
    if (below(pc.ltype())) {
      if (c_atoms.length() <= pc.c_atoms.length())
	return true;
      else
	return false;
    }
    else if (pc_type == pc.pc_type) {
      if (c_atoms.length() < pc.c_atoms.length())
	return true;
      else
	return false;
...
bool TraceSet::PlanConstraint::is_dominated_by(const PlanConstraint& pc) const
{
  return pc.dominates(*this);
}

bool TraceSet::add_non_dominated
(const PlanConstraint& pc, plan_constraint_vec& c) const
{
  bool non_dom = true;
  bool_vec dom(false, c.length());
  for (index_type k = 0; (k < c.length()) && non_dom; k++) {
    if (pc.is_dominated_by(c[k]))
      non_dom = false;
    else if (pc.dominates(c[k]))
      dom[k] = true;
  }
  if (non_dom) {
    c.remove(dom);
...
void TraceSet::find_separating_constraints
(index_type p,
 index_type s_min,
 index_type s_max,
 plan_constraint_vec& pc)
{
  ...
  bool done = false;
  while ((l <= s_max) && !done) {
    std::cerr << "checking constraints of size " << l << "..." << std::endl;
    mSubsetEnumerator e(instance.n_atoms(), l);
    bool more = e.first();
    while (more) {
      index_set s;
      e.current_set(s);
      bool_vec v;
      // test as disjunctive always
      if ((*this)[p].first->test_always_disjunction(s)) {
	test_always_disjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  add_non_dominated(PlanConstraint(pc_always_disjunction, s, v), pc);
      }
      // test as negated conjunctive sometime - only if at least one atom
      // in s lacks a complementary atom (negation) in instance!
      else if (!instance.all_lack_negation(s)) {
	test_always_disjunction(s, v);
	if (v.count(true) > 0)
	  add_non_dominated(PlanConstraint(pc_negated_sometime, s, v), pc);
      }
      // test as conjunctive sometime
      if ((*this)[p].first->test_sometime_conjunction(s)) {
	test_sometime_conjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  add_non_dominated(PlanConstraint(pc_sometime, s, v), pc);
      }
      // test as negated disjunctive always - only if at least one atom
      // in s lacks a complementary atom (negation) in instance!
      else if (!instance.all_lack_negation(s)) {
	test_sometime_conjunction(s, v);
	if (v.count(true) > 0)
	  add_non_dominated(PlanConstraint(pc_never_disjunction, s, v), pc);
      }
      // test as conjunctive at_most_once
      bool acc = false;
      if ((*this)[p].first->test_at_most_once_conjunction(s)) {
	test_at_most_once_conjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  acc = add_non_dominated(PlanConstraint(pc_at_most_once, s, v), pc);
      }
      // test as disjunctive at_most_once
      if ((!acc || (s.length() > 1)) &&
	  (*this)[p].first->test_at_most_once_disjunction(s)) {
	test_at_most_once_disjunction(s, v);
	v.complement(); // now true iff not sat
	if (v.count(true) > 0)
	  add_non_dominated(PlanConstraint(pc_at_most_once_disjunction, s, v), pc);
      }
      more = e.next();
    }
    bool_vec ex_all(false, length());
    for (index_type k = 0; k < pc.length(); k++)
      ex_all.insert(pc[k].ex);
    std::cerr << pc.length() << " constraints found, total # ex. = "
	      << ex_all.count(true) << " of " << length() - 1 << std::endl;
    if ((ex_all.count(true) >= (length() - 1)) && (l >= s_min))
      done = true;
    l += 1;
  }
}

void TraceSet::extract_sc_candidates
(weighted_vec<index_type,NTYPE>& sc_always,
 weighted_vec<index_type,NTYPE>& sc_sometime,
 weighted_vec<index_type,NTYPE>& sc_once,
 weighted_vec<index_pair,NTYPE>& sc_before)
{
  bool_vec atoms_always(false, instance.n_atoms());
  bool_vec atoms_sometime(false, instance.n_atoms());
  bool_vec atoms_once(false, instance.n_atoms());

  index_vec count_always(0, instance.n_atoms());
  for (index_type p = 0; p < instance.n_atoms(); p++) {
    for (index_type k = 0; k < length(); k++) {
      if ((*this)[k].first->test_always(p)) {
	if (Instance::default_trace_level > 0) {
	  std::cerr << "trace #" << k;
	  if ((*this)[k].second)
	    if ((*this)[k].second->plan_name())
	      std::cerr << " (" << (*this)[k].second->plan_name() << ")";
	  std::cerr << " satisfies (always "
		    << instance.atoms[p].name << ")" << std::endl;
	}
	count_always[p] += 1;
      }
      else if (Instance::default_trace_level > 0) {
	std::cerr << "trace #" << k;
	if ((*this)[k].second)
	  if ((*this)[k].second->plan_name())
	    std::cerr << " (" << (*this)[k].second->plan_name() << ")";
	std::cerr << " does NOT satisfy (always "
		  << instance.atoms[p].name << ")" << std::endl;
      }
    }
    if ((0 < count_always[p]) && (count_always[p] < length()))
      atoms_always[p] = true;
  }

  index_vec count_sometime(0, instance.n_atoms());
  for (index_type p = 0; p < instance.n_atoms(); p++) if (!atoms_always[p]) {
    for (index_type k = 0; k < length(); k++) {
      if ((*this)[k].first->test_sometime(p)) {
	if (Instance::default_trace_level > 0) {
	  std::cerr << "trace #" << k;
	  if ((*this)[k].second)
	    if ((*this)[k].second->plan_name())
	      std::cerr << " (" << (*this)[k].second->plan_name() << ")";
	  std::cerr << " satisfies (sometime "
		    << instance.atoms[p].name << ")" << std::endl;
	}
	count_sometime[p] += 1;
      }
	...
	if ((*this)[k].second)
	  if ((*this)[k].second->plan_name())
	    std::cerr << " (" << (*this)[k].second->plan_name() << ")";
	std::cerr << " does NOT satisfy (sometime "
		  << instance.atoms[p].name << ")" << std::endl;
      }
    }
    if ((0 < count_sometime[p]) && (count_sometime[p] < length()))
      atoms_sometime[p] = true;
  }

  index_vec count_once(0, instance.n_atoms());
  index_vec count_more(0, instance.n_atoms());
  for (index_type p = 0; p < instance.n_atoms(); p++) if (!atoms_always[p]) {
    for (index_type k = 0; k < length(); k++) {
      if ((*this)[k].first->test_at_most_once(p)) {
	if ((*this)[k].first->test_sometime(p)) {
	  if (Instance::default_trace_level > 0) {
	    std::cerr << "trace #" << k;
	    if ((*this)[k].second)
	      if ((*this)[k].second->plan_name())
		std::cerr << " (" << (*this)[k].second->plan_name() << ")";
	    std::cerr << " satisfies (sometime " << instance.atoms[p].name
		      << ") AND (at-most-once "
		      << instance.atoms[p].name << ")" << std::endl;
	  }
	  count_once[p] += 1;
	...
	if (Instance::default_trace_level > 0) {
	  ...
	  if ((*this)[k].second)
	    if ((*this)[k].second->plan_name())
	      std::cerr << " (" << (*this)[k].second->plan_name() << ")";
	  std::cerr << " does NOT satisfy (at-most-once "
		    << instance.atoms[p].name << ")" << std::endl;
	}
	count_more[p] += 1;
      }
  ...
  for (index_type p = 0; p < instance.n_atoms(); p++) {
    if (atoms_always[p])
      sc_always.append(weighted<index_type,NTYPE>(p, count_always[p]));
    if (atoms_sometime[p])
      sc_sometime.append(weighted<index_type,NTYPE>(p, count_sometime[p]));
    if (atoms_once[p])
      sc_once.append(weighted<index_type,NTYPE>(p, count_once[p]));
  }

  for (index_type p = 0; p < instance.n_atoms(); p++)
    if (atoms_sometime[p] || atoms_once[p]) {
      for (index_type q = 0; q < instance.n_atoms(); q++)
	if ((count_always[q] == 0) &&
	    !instance.atoms[q].init &&
	    !instance.atoms[q].goal) {
	  index_type n_before = 0;
	  for (index_type k = 0; k < length(); k++) {
	    if ((*this)[k].first->test_sometime_before(p, q)) n_before += 1;
	  }
	  if ((0 < n_before) && (n_before < length())) {
	    sc_before.append(weighted<index_pair,NTYPE>(index_pair(p, q), n_before));
	  }
	}
    }
}

void TraceSet::calculate_sc_weights
(weighted_vec<index_type,NTYPE>& sc_always,
 weighted_vec<index_type,NTYPE>& sc_sometime,
 weighted_vec<index_type,NTYPE>& sc_once,
 weighted_vec<index_pair,NTYPE>& sc_before)
{
  index_type n_atom_constraints =
    (sc_always.length() + sc_sometime.length() + sc_once.length());
  index_type n_pair_constraints = sc_before.length();
  index_type n_constraints = n_atom_constraints + n_pair_constraints;

  index_vec max_sat_always(0, sc_always.length());
  index_vec max_sat_sometime(0, sc_sometime.length());
  index_vec max_sat_once(0, sc_once.length());
  index_vec max_sat_before(0, sc_before.length());

  for (index_type k = 0; k < length(); k++) {
    bool_vec sat_always(false, sc_always.length());
    bool_vec sat_sometime(false, sc_sometime.length());
    bool_vec sat_once(false, sc_once.length());
    bool_vec sat_before(false, sc_before.length());

    for (index_type i = 0; i < sc_always.length(); i++)
      sat_always[i] = (*this)[k].first->test_always(sc_always[i].value);
    for (index_type i = 0; i < sc_sometime.length(); i++)
      sat_sometime[i] = (*this)[k].first->test_sometime(sc_sometime[i].value);
    for (index_type i = 0; i < sc_once.length(); i++)
      sat_once[i] = ((*this)[k].first->test_sometime(sc_once[i].value) &&
		     (*this)[k].first->test_at_most_once(sc_once[i].value));
    for (index_type i = 0; i < sc_before.length(); i++)
      sat_before[i] =
	(*this)[k].first->test_sometime_before(sc_before[i].value.first,
					       sc_before[i].value.second);

    index_type n_always = sat_always.count(true);
    index_type n_sometime = sat_sometime.count(true);
    index_type n_once = sat_once.count(true);
    index_type n_before = sat_before.count(true);
    index_type n_sat = n_always + n_sometime + n_once + n_before;

#ifdef UNECESSARILY_VERBOSE
...
#ifdef UNECESSARILY_VERBOSE
	std::cout << ";;  (always "
		  << instance.atoms[sc_always[i].value].name
		  << ")" << std::endl;
#endif
...
#ifdef UNECESSARILY_VERBOSE
	std::cout << ";;  (sometime "
		  << instance.atoms[sc_sometime[i].value].name
		  << ")" << std::endl;
#endif
	...
	std::cout << ";;  (and (sometime "
		  << instance.atoms[sc_once[i].value].name
		  << ") (at-most-once "
		  << instance.atoms[sc_once[i].value].name
		  << "))" << std::endl;
#endif
	...
	std::cout << ";;  (sometime-before "
		  << instance.atoms[sc_before[i].value.first].name << " "
		  << instance.atoms[sc_before[i].value.second].name
		  << ")" << std::endl;
#endif
  ...
  for (index_type k = 0; k < sc_always.length(); k++)
    sc_always[k].weight = n_constraints - max_sat_always[k];
  for (index_type k = 0; k < sc_sometime.length(); k++)
    sc_sometime[k].weight = n_constraints - max_sat_sometime[k];
  for (index_type k = 0; k < sc_once.length(); k++)
    sc_once[k].weight = n_constraints - max_sat_once[k];
  for (index_type k = 0; k < sc_before.length(); k++)
    sc_before[k].weight = n_constraints - max_sat_before[k];
}
...
void ScheduleSet::clear_no_delete()
{
  set_length(0);
  props.set_length(0);
}

TraceSet* ScheduleSet::trace_set(bool valid_only, bool exec_only, bool untimed)
{
  TraceSet* ts = new TraceSet(instance);
  for (index_type k = 0; k < length(); k++) {
    if (valid_only) ts->add_plan_if_valid((*this)[k], untimed);
    else if (exec_only) ts->add_plan_if_executable((*this)[k], untimed);
    else ts->add_plan((*this)[k], untimed);
  }
  return ts;
}
