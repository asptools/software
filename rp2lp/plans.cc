...
void ActionSequence::output(Plan& to, bool mark_end)
{
...
bool ActionSequence::simulate
(Instance& ins, ExecState& cs,
 ExecTrace* trace, ExecErrorSet* errors, bool finish) const
{
  ...
  while (p < length()) {
    if (!cs.applicable((*this)[p], errors, p)) {
      ::std::cerr << p << ". " << ins.actions[(*this)[p]].name
		  << " NOT applicable" << ::std::endl;
      executable = false;
      if (!finish) return false;
    }
    cs.apply((*this)[p], errors, p);
    cs.finish(errors);
    if (trace) {
...
void ApxResult::solution(State& s, Transition* p, NTYPE cost)
{
  std::cerr << "error: ApxResult::solution called with non-state path"
	    << std::endl;
  exit(255);
}
...
void PrintActions::insert(index_type act)
{
  ...
  if (print_action_cost) {
    to << " (" << PRINT_NTYPE(instance.actions[act].cost) << ")";
  }
  first_action = false;
}
...
SaveToFile::SaveToFile(Instance& i, Stopwatch* s, const std::string& name)
  : printer(0), planfile(name.c_str())
{
  //printer = new PrintIPC(i, planfile);
  printer = new PrintActions(i, planfile);
  if (s != NULL) {
    planfile << ";; " << *s << std::endl;
  }
}
...
void SaveToFile::protect(index_type atom)
{
  printer->protect(atom);
}

void SaveToFile::insert(index_type act)
{
  printer->insert(act);
}

void SaveToFile::advance(NTYPE delta)
{
  printer->advance(delta);
}
...
SaveAllToFile::SaveAllToFile(Instance& i, Stopwatch* s, const std::string& b)
  : instance(i), stats(s), basename(b), next(0)
{
  // done
}

SaveAllToFile::~SaveAllToFile()
{
  // done
}

Plan* SaveAllToFile::new_plan()
{
  std::ostringstream fname;
  fname << basename << "." << next++ << ".soln";
  Plan* p = new SaveToFile(instance, stats, fname.str());
  return p;
}

Store::Store(Instance& i)
  : ScheduleSet(i), copy_to(0)
{
  // done
}
...
void Store::solution(State& s, Transition* t, NTYPE cost)
{
  // std::cerr << "storing solution..." << std::endl;
  Result::solution(s, t, cost);
  solutions.append(s.copy());
  Schedule* new_plan = new Schedule(instance);
  if (t != NULL)
    t->insert_path(*new_plan);
  new_plan->end();
  if (copy_to != 0) {
    Plan* cc = copy_to->new_plan();
    if (cc != 0)
      new_plan->output(*cc);
  }
  append(new_plan);
...
void StoreFilter::lower_bound(NTYPE cost)
{
  store.lower_bound(cost);
}
...
void StoreFilter::write(std::ostream& to)
{
  store.write(to);
}
...
Conflicts::Conflicts(const Instance& ins)
  : instance(ins),
    nec_del(EMPTYSET),
    pos_del(EMPTYSET),
    min_peak(0, 0),
...
void Conflicts::solution(State& s, NTYPE cost)
{
  ...
  Schedule* plan = new Schedule((Instance&)instance);
  plan->trace_level = 0;
  s.insert_path(*plan);
  plan->end();
  ...
  ExecTrace* trace = new ExecTrace((Instance&)instance);
  plan->simulate(trace);
