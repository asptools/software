...
bool PDDL_Base::use_default_function_value = false;
NTYPE PDDL_Base::default_function_value(0);
bool PDDL_Base::assume_sequential = true;
...
bool PDDL_Base::compile_away_disjunctive_goals = true;
bool PDDL_Base::compile_away_object_functions = true;
// NOTE: make_types is buggy, so off by default
bool PDDL_Base::make_types_from_static_predicates = false;
...
PDDL_Base::Symbol* PDDL_Base::TypeSet::get_element(index_type n)
{
  index_type k = 0;
  while (k < length()) {
...
NTYPE PDDL_Base::find_function_value(const char* fname, const symbol_vec& arg)
{
  ...
  FInitAtom* a = find_function_init(fname, arg);
  if (a) {
    return a->val;
  }
...
PDDL_Base::FInitAtom*
PDDL_Base::find_function_init(const char* fname, const symbol_vec& arg)
{
  FunctionSymbol* fun = find_function(fname);
  if (fun == 0) {
    std::cerr << "error: no function named " << fname << std::endl;
    exit(255);
  }
...
PDDL_Base::signed_atom PDDL_Base::goal_to_atom(Goal* g)
{
  if (g->g_class == goal_pos_atom)
    return signed_atom(false, ((AtomicGoal*)g)->atom);
  else if (g->g_class == goal_neg_atom)
    return signed_atom(true, ((AtomicGoal*)g)->atom);
  else
    return signed_atom(false, 0);
}

bool PDDL_Base::quantified_atom_to_atom_vec
(bool is_dis,
 variable_vec& param,
 index_type p,
 atom_vec& pos_con,
...
{
  if (p < param.size()) {
    // std::cerr << "param[" << p << "] = " << param[p]->print_name
    // 	      << " has " << param[p]->sym_types.n_elements() << " values"
    // 	      << std::endl;
    for (index_type k = 0; k < param[p]->sym_types.n_elements(); k++) {
      param[p]->value = param[p]->sym_types.get_element(k);
      // std::cerr << "param[" << p << "] = " << param[p]->print_name
      // 		<< " assigned " << param[p]->value->print_name << std::endl;
      bool proceed = true;
      for (index_type i = 0; i < pos_con.length(); i++) {
	assert(pos_con[i]->is_static());
	partial_value v = pos_con[i]->partial_eval();
	if (is_dis && (v == p_false)) proceed = false;
	if (!is_dis && (v == p_true)) {
	  // pos_con[i]->print(std::cerr);
	  // std::cerr << " is static true" << std::endl;
	  proceed = false;
	}
      }
      for (index_type i = 0; i < neg_con.length(); i++) {
	assert(neg_con[i]->is_static());
	partial_value v = neg_con[i]->partial_eval();
	if (is_dis && (v == p_true)) proceed = false;
	if (!is_dis && (v == p_false)) {
	  // pos_con[i]->print(std::cerr);
	  // std::cerr << " is negated and static false" << std::endl;
	  proceed = false;
	}
      }
      if (proceed) {
	quantified_atom_to_atom_vec
	  (is_dis, param, p + 1, pos_con, neg_con, ns_atom, is_neg, av);
      }
    }
    param[p]->value = 0;    
    return true;
    ...
    Atom* pi_atom = ns_atom->instantiate_partially();
    av.append(signed_atom(is_neg, pi_atom));
    return true;
  }
}

bool PDDL_Base::goal_to_atom_vec
(Goal* g, signed_atom_vec& av, bool& is_disjunction)
{
  // std::cerr << "extracting atom vec from goal ";
  // g->print(std::cerr);
  // std::cerr << std::endl;
  av.clear();
  if (g->g_class == goal_pos_atom) {
    is_disjunction = false;
    av.append(signed_atom(false, ((AtomicGoal*)g)->atom));
    return true;
  }
  else if (g->g_class == goal_neg_atom) {
    is_disjunction = false;
    av.append(signed_atom(true, ((AtomicGoal*)g)->atom));
    return true;
  }
    ...
    for (index_type k = 0; k < cg->goals.length(); k++) {
      if (cg->goals[k]->g_class == goal_pos_atom)
	av.append(signed_atom(false, ((AtomicGoal*)cg->goals[k])->atom));
      else if (cg->goals[k]->g_class == goal_neg_atom)
	av.append(signed_atom(true, ((AtomicGoal*)cg->goals[k])->atom));
      else
	return false;
    }
    return true;
  }
  else if (g->g_class == goal_disjunction) {
    is_disjunction = true;
    DisjunctiveGoal* dg = (DisjunctiveGoal*)g;
    for (index_type k = 0; k < dg->goals.length(); k++) {
      if (dg->goals[k]->g_class == goal_pos_atom)
	av.append(signed_atom(false, ((AtomicGoal*)dg->goals[k])->atom));
      else if (dg->goals[k]->g_class == goal_neg_atom)
	av.append(signed_atom(true, ((AtomicGoal*)dg->goals[k])->atom));
      else
	return false;
    }
    return true;
  }
  else if ((g->g_class == goal_exists) ||
	   (g->g_class == goal_forall)) {
    is_disjunction = (g->g_class == goal_exists);
    QuantifiedGoal* qg = (QuantifiedGoal*)g;
    if (((g->g_class == goal_exists) &&
	 (qg->goal->g_class == goal_conjunction)) ||
	((g->g_class == goal_forall) &&
	 (qg->goal->g_class == goal_disjunction))) {
      ConjunctiveGoal* cg = (ConjunctiveGoal*)qg->goal;
      atom_vec pos_con;
      atom_vec neg_con;
      Atom* non_static_atom = NULL;
      bool ns_atom_is_neg = false;
      for (index_type k = 0; k < cg->goals.size(); k++) {
	if (cg->goals[k]->g_class == goal_pos_atom) {
	  // ((AtomicGoal*)cg->goals[k])->atom->print(std::cerr, false);
	  // std::cerr << " is static = "
	  // 	    << ((AtomicGoal*)cg->goals[k])->atom->is_static()
	  // 	    << std::endl;
	  if (((AtomicGoal*)cg->goals[k])->atom->is_static())
	    pos_con.append(((AtomicGoal*)cg->goals[k])->atom);
	  else if (non_static_atom == NULL)
	    non_static_atom = ((AtomicGoal*)cg->goals[k])->atom;
	  else
	    return false;
	}
	else if (cg->goals[k]->g_class == goal_neg_atom) {
	  // ((AtomicGoal*)cg->goals[k])->atom->print(std::cerr, true);
	  // std::cerr << " is static = "
	  // 	    << ((AtomicGoal*)cg->goals[k])->atom->is_static()
	  // 	    << std::endl;
	  if (((AtomicGoal*)cg->goals[k])->atom->is_static())
	    neg_con.append(((AtomicGoal*)cg->goals[k])->atom);
	  else if (non_static_atom == NULL) {
	    non_static_atom = ((AtomicGoal*)cg->goals[k])->atom;
	    ns_atom_is_neg = true;
	  }
	  ...
	  std::cerr << "wtf = " << cg->goals[k]->g_class << std::endl;
	  return false;
	}
      }
      if (non_static_atom == NULL)
	return false;
      // std::cerr << "instantiating ";
      // AtomBase::print_bindings = true;
      // non_static_atom->print(std::cerr);
      // std::cerr << std::endl;
      return quantified_atom_to_atom_vec(is_disjunction,
					 qg->param, 0, pos_con, neg_con,
					 non_static_atom, ns_atom_is_neg,
					 av);
    }
    else if ((qg->goal->g_class == goal_pos_atom) ||
	     (qg->goal->g_class == goal_neg_atom)) {
      atom_vec pos_con;
      atom_vec neg_con;
      return quantified_atom_to_atom_vec(is_disjunction,
					 qg->param, 0, pos_con, neg_con,
					 ((AtomicGoal*)qg->goal)->atom,
					 (qg->goal->g_class == goal_neg_atom),
					 av);
    }
    else {
...
void PDDL_Base::set_type_type(simple_type_vec& vec, TypeSymbol* t)
{
  for (index_type k = vec.length(); k > 0; k--) {
    if (vec[k - 1]->sym_types.length() > 0) return;
    vec[k - 1]->sym_types.append(t);
...
void PDDL_Base::make_parameters
(variable_vec& p, const char* prefix, variable_vec& v)
{
  v.set_length(p.length());
  for (index_type k = 0; k < p.length(); k++) {
    EnumName v_name(prefix, k);
    v[k] = new VariableSymbol(v_name.to_cstring(Name::NC_INSTANCE));
    v[k]->sym_types = p[k]->sym_types;
  }
}

void PDDL_Base::add_init_atom(Atom* a)
{
  a->insert(a->pred->init);
  dom_init.append(a);
}

void PDDL_Base::add_init_fun(FInitAtom* a)
{
  a->insert(a->fun->init);
  dom_fun_init.append(a);
}

void PDDL_Base::add_goal_atom(Atom* a, bool neg)
{
  AtomicGoal* g = new AtomicGoal(a, neg);
  dom_goals.append(g);
  if (neg)
    a->insert(a->pred->neg_goal);
  else
    a->insert(a->pred->pos_goal);
}
...
bool PDDL_Base::Expression::is_constant()
{
  ...
  case exp_fun:
    {
      FunctionSymbol* fun = ((FunctionExpression*)this)->fun;
      if (!fun->is_static()) return false;
      ListExpression* args = ((FunctionExpression*)this)->args;
      while (args) {
	if (args->sym->sym_class != sym_object) return false;
	args = args->rest;
      }
      return true;
    }
...
bool PDDL_Base::Expression::is_defined()
{
  switch (exp_class) {
  case exp_fun:
    return (use_default_function_value ? true :
	    ((FunctionExpression*)this)->is_defined());
  case exp_const:
  case exp_list:
  case exp_preference:
    return true;
  case exp_time:
    return (((TimeExpression*)this)->time_exp ? 
	    ((TimeExpression*)this)->time_exp->is_defined() : false);
  case exp_add:
  case exp_sub:
  case exp_mul:
  case exp_div:
  case exp_min:
  case exp_max:
    return (((BinaryExpression*)this)->first->is_defined() &&
	    ((BinaryExpression*)this)->second->is_defined());
  }
  assert(0);
}
  ...
  case exp_min:
    {
      ...
      interval v1 = bexp->first->eval_bounds();
      interval v2 = bexp->second->eval_bounds();
      return interval(MIN(v1.first, v2.first), MIN(v1.second, v2.second));
    }
  case exp_max:
    {
      BinaryExpression* bexp = (BinaryExpression*)this;
      interval v1 = bexp->first->eval_bounds();
      interval v2 = bexp->second->eval_bounds();
      return interval(MAX(v1.first, v2.first), MAX(v1.second, v2.second));
    }
  case exp_preference:
    {
...
PDDL_Base::Expression* PDDL_Base::Expression::simplify()
{
  ...
  case exp_fun:
    {
      if (is_constant()) return new ConstantExpression(eval_static());
      else return this;
    }
  case exp_list:
  case exp_time:
  ...
  case exp_add:
	...
	if (b1->first->is_static()) {
      ...
      // stupid special case: if, after simplification, either term
      // is a constant zero, then the sum can be simplified to the
      // other term (note: potential memory leak here)
      if (b_exp->first->exp_class == exp_const) {
	ConstantExpression* c_exp = (ConstantExpression*)b_exp->first;
	if (c_exp->val == 0)
	  return b_exp->second;
      }
      else if (b_exp->second->exp_class == exp_const) {
	ConstantExpression* c_exp = (ConstantExpression*)b_exp->second;
	if (c_exp->val == 0)
	  return b_exp->first;
      }
      return this;
  ...
  case exp_min:
  case exp_max:
    return this; // simplification not implemented
  }
  assert(0);
}

PDDL_Base::Expression* PDDL_Base::Expression::instantiate_partially()
{
  switch (exp_class) {
  case exp_fun:
    {
      ...
      if (fexp->args) {
	Expression* pi_args = fexp->args->instantiate_partially();
	return new FunctionExpression(fexp->fun, (ListExpression*)pi_args);
      }
      else {
	return this;
      }
  ...
  case exp_list:
    {
      ...
      assert(lexp->sym);
      Expression* pi_rest = 0;
      if (lexp->rest) {
	pi_rest = lexp->rest->instantiate_partially();
      }
      if (lexp->sym->sym_class == sym_variable) {
	VariableSymbol* vsym = (VariableSymbol*)lexp->sym;
	if (vsym->value) {
	  return new ListExpression(vsym->value, (ListExpression*)pi_rest);
	}
	else {
	  return new ListExpression(lexp->sym, (ListExpression*)pi_rest);
	}
      }
      else {
	return new ListExpression(lexp->sym, (ListExpression*)pi_rest);
  ...
  case exp_max:
    {
      BinaryExpression* b_exp = (BinaryExpression*)this;
      Expression* exp1 = b_exp->first->instantiate_partially();
      Expression* exp2 = b_exp->second->instantiate_partially();
      return new BinaryExpression(exp_class, exp1, exp2);
    }
    return this; // simplification not implemented
  }
  assert(0);
...
void PDDL_Base::Expression::collect_functions(exp_vec& c)
{
  switch (exp_class) {
  case exp_fun:
    c.append(this);
  ...
  case exp_min:
  case exp_max:
    ((BinaryExpression*)this)->first->collect_functions(c);
    ((BinaryExpression*)this)->second->collect_functions(c);
    break;
  }
}

void PDDL_Base::Expression::collect_variables(variable_vec& v)
{
  switch (exp_class) {
  case exp_fun:
    ((FunctionExpression*)this)->args->collect_variables(v);
    break;
  case exp_list:
    {
      Symbol* s = ((ListExpression*)this)->sym;
      if (s->sym_class == sym_variable)
	if (!v.contains((VariableSymbol*)s))
	  v.append((VariableSymbol*)s);
      if (((ListExpression*)this)->rest)
	((ListExpression*)this)->rest->collect_variables(v);
    }
    break;
  case exp_add:
  case exp_sub:
  ...
  case exp_min:
  case exp_max:
    ((BinaryExpression*)this)->first->collect_variables(v);
    ((BinaryExpression*)this)->second->collect_variables(v);
    break;
  }
}

void PDDL_Base::Expression::collect_objects(symbol_vec& v)
{
  switch (exp_class) {
  case exp_fun:
    ((FunctionExpression*)this)->args->collect_objects(v);
    break;
  case exp_list:
    {
      Symbol* s = ((ListExpression*)this)->sym;
      if (s->sym_class == sym_object)
	if (!v.contains(s))
	  v.append(s);
      if (((ListExpression*)this)->rest)
	((ListExpression*)this)->rest->collect_objects(v);
    }
    break;
  case exp_add:
  case exp_sub:
  ...
  case exp_min:
  case exp_max:
    ((BinaryExpression*)this)->first->collect_objects(v);
    ((BinaryExpression*)this)->second->collect_objects(v);
    break;
  }
}
...
bool PDDL_Base::FunctionExpression::is_defined()
{
  ptr_table* r = &(fun->init);
  ListExpression* a = args;
  while (a && r) {
    if (a->sym->sym_class == sym_variable) {
      if (((VariableSymbol*)a->sym)->value == 0) {
	std::cerr << "error: unbound variable "
		  << a->sym->print_name << " in ";
	print(std::cerr, true);
	std::cerr << " - uncompiled object function?"
	...
	exit(255);
      ...
      r = r->find_next(((VariableSymbol*)a->sym)->value);
    }
    else {
      r = r->find_next(a->sym);
    }
...
void PDDL_Base::AtomBase::fill_in_args(variable_vec& v, bool as_value)
{
  param.set_length(v.length());
  for (index_type k = 0; k < v.length(); k++)
    param[k] = (as_value ? v[k]->value : v[k]);
}
...
void PDDL_Base::AtomBase::collect_objects(symbol_vec& v)
{
  for (index_type k = 0; k < param.length(); k++)
    if (param[k]->sym_class == sym_object)
      if (!v.contains(param[k]))
	v.append(param[k]);
}
...
Instance::atom_ref PDDL_Base::Atom::find_prop
(Instance& ins, bool neg, bool create)
{
  ...
  if (!r->val) {
    Instance::atom_ref null_ref;
    if (!create) return null_ref;
    ...
    Instance::atom_ref not_p = find_prop(ins, !neg, false);
    if (not_p) {
	...
	else if ((warning_level > 0) || !best_effort) {
  ...
  Instance::atom_ref b = *a;
  // std::cerr << "find_prop: ";
  // PDDL_Base::AtomBase::print_bindings = true;
  // print(std::cerr);
  // PDDL_Base::AtomBase::print_bindings = false;
  // std::cerr << ", " << neg << ", " << create
  // 	    << " returning " << b->index << "." << b->name
  // 	    << std::endl;
  return b;
}
...
bool PDDL_Base::FChangeAtom::is_defined()
{
  ptr_table* r = &(fun->init);
  for (index_type k = 0; k < param.length(); k++) {
    if (param[k]->sym_class == sym_variable) {
  ...
  if (use_default_function_value) return true;
  return false;
}

Instance::resource_ref PDDL_Base::FChangeAtom::find_resource(Instance& ins)
{
  ptr_table* r = &(fun->init);
  for (index_type k = 0; k < param.length(); k++) {
    if (param[k]->sym_class == sym_variable) {
  ...
  if (!r->val) {
    if (!use_default_function_value) return Instance::resource_ref();
...
void PDDL_Base::ActionSymbol::post_process()
{
  ...
  /* search for locks */
  bool_vec dels_to_remove(false, dels.length());
  for (index_type k = 0; k < dels.length(); k++)
	...
	dels_to_remove[k] = true;
	adds.remove(add_i);
      }
    }
  dels.remove(dels_to_remove);

  /* search for enables */
  bool_vec adds_to_remove(false, adds.length());
  for (index_type k = 0; k < adds.length(); k++)
	...
	adds_to_remove[k] = true;
	dels.remove(del_i);
      }
    }
  adds.remove(adds_to_remove);
...
void PDDL_Base::post_process()
{
  if (compile_away_object_functions) {
    if (dom_object_functions.length() > 0)
  ...
  if (make_types_from_static_predicates) {
...
void PDDL_Base::make_types()
{
  if (dom_types.size() > 0) {
    std::cerr << "make_types: domain is already typed!" << std::endl;
    return;
  }
  assert(dom_top_type->elements.size() == dom_constants.size());
  predicate_vec static_unary(0, 0);
  for (index_type i = 0; i < dom_predicates.size(); i++)
    if ((dom_predicates[i]->param.size() == 1) &&
	!dom_predicates[i]->modded)
      static_unary.append(dom_predicates[i]);
  if (static_unary.size() == 0) {
    std::cerr << "make_types: no static unary predicates" << std::endl;
    return;
  }
  index_set_vec combs;
  for (index_type k = 0; k < dom_constants.size(); k++) {
    index_set bto;
    for (index_type i = 0; i < static_unary.size(); i++) {
      Atom a(static_unary[i], dom_constants[k]);
      if (a.initial_value()) bto.insert(i);
    }
    index_type j = combs.first(bto);
    if (j == no_such_index) {
      TypeSymbol* new_type = (TypeSymbol*)gensym(sym_typename, "new_type", 0);
      new_type->is_base_type = true;
      dom_types.append(new_type);
      combs.append(bto);
      assert(combs.size() == dom_types.size());
      j = combs.size() - 1;
    }
    assert(j < dom_types.size());
    dom_types[j]->elements.append(dom_constants[k]);
    dom_constants[k]->sym_types.clear();
    dom_constants[k]->sym_types.append(dom_types[j]);
  }
  std::cerr << "make_types: created " << dom_types.size() << " types"
	    << std::endl;
  for (index_type i = 0; i < dom_types.size(); i++)
    for (index_type j = 0; j < dom_types.size(); j++)
      if (combs[j].contains(combs[i]) && (combs[j] != combs[i])) {
	// make j a subtype of i
	dom_types[j]->sym_types.append(dom_types[i]);
	dom_types[i]->is_base_type = false;
      }
  for (index_type i = 0; i < dom_types.size(); i++)
    if (dom_types[i]->is_base_type) {
      for (index_type k = 0; k < dom_types[i]->elements.size(); k++)
	// calling add_element will add the element to the type's
	// supertypes, if it's not already there; it won't add the
	// element to the set of elements of type i, since it's
	// (obviously) already in that set.
	dom_types[i]->add_element(dom_types[i]->elements[k]);
    }
  for (index_type k = 0; k < dom_actions.size(); k++) {
    bool_vec type_pre(false, dom_actions[k]->pos_pre.size());
    bool ok = true;
    for (index_type i = 0; i < dom_actions[k]->param.size(); i++) {
      index_set bto;
      for (index_type p = 0; p < dom_actions[k]->pos_pre.size(); p++)
	if ((dom_actions[k]->pos_pre[p]->param.size() == 1) &&
	    (dom_actions[k]->pos_pre[p]->param[0] == dom_actions[k]->param[i])) {
	  index_type q = static_unary.first(dom_actions[k]->pos_pre[p]->pred);
	  if (q != no_such_index) {
	    bto.insert(q);
	    type_pre[p] = true;
	  }
      ...
      if (!bto.empty()) {
	bool found = false;
	for (index_type j = 0; (j < combs.size()) && !found; j++)
	  if (combs[j].contains(bto)) {
	    bool leastspec = true;
	    for (index_type j2 = j + 1; j2 < combs.size(); j2++)
	      if (combs[j2].contains(bto))
		if (combs[j].contains(combs[j2]))
		  leastspec = false;
	    if (leastspec) {
	      dom_actions[k]->param[i]->sym_types.clear();
	      dom_actions[k]->param[i]->sym_types.append(dom_types[j]);
	      found = true;
	    }
	  }
	if (!found) {
	  if ((warning_level > 0) || !best_effort) {
	    std::cerr << "warning: could not identify type of paramter "
		      << dom_actions[k]->param[i]->print_name << " of action "
		      << dom_actions[k]->print_name << std::endl;
	    if (!best_effort) exit(1);
	  }
	  ok = false;
	}
    ...
    if (ok) {
      dom_actions[k]->pos_pre.remove(type_pre);
    }
  }
}
...
bool PDDL_Base::Goal::makeCPG(CPG& g)
{
  if ((g_class == goal_pos_atom) || (g_class == goal_neg_atom)) {
    ...
    for (index_type k = 0; k < a->atom->param.length(); k++) {
      ...
      g.arg_types.append(a->atom->param[k]->sym_types);
      g.param_types.append(a->atom->pred->param[k]->sym_types);
      g.arg_range_pred.append(0);
      symbol_set& s = g.arg_range_set.append();
      s.assign_singleton(a->atom->param[k]);
    }
    assert(g.args.length() == g.arg_types.length());
    assert(g.arg_types.length() == g.param_types.length());
    assert(g.args.length() == g.arg_range_pred.length());
    assert(g.args.length() == g.arg_range_set.length());
    return true;
  }
  ...
  if ((warning_level > 0)) {
    std::cerr << "warning: can't make CPG from goal ";
    print(std::cerr);
    std::cerr << std::endl;
  }
  return false;
...
bool PDDL_Base::Goal::make_negated_CPG(CPG& g)
{
  if ((g_class == goal_pos_atom) || (g_class == goal_neg_atom)) {
    AtomicGoal* a = (AtomicGoal*)this;
    g.atoms.append(a->atom);
  ...
  else if (g_class == goal_disjunction) {
    DisjunctiveGoal* c = (DisjunctiveGoal*)this;
    for (index_type i = 0; i < c->goals.length(); i++) {
      if (!c->goals[i]->make_negated_CPG(g)) return false;
    }
    return true;
  }
  if ((warning_level > 0)) {
    std::cerr << "warning: can't make negated CPG from goal ";
    print(std::cerr);
    std::cerr << std::endl;
  }
  return false;
...
bool PDDL_Base::Goal::makeCPG(CPG& g, index_vec& s)
{
  if ((g_class == goal_pos_atom) || (g_class == goal_neg_atom)) {
	...
	g.param_types.append(a->atom->pred->param[k]->sym_types);
	g.arg_range_pred.append(0);
	symbol_set& ars = g.arg_range_set.append();
	ars.assign_singleton(a->atom->param[k]);
      }
      assert(g.args.length() == g.arg_types.length());
      assert(g.arg_types.length() == g.param_types.length());
      assert(g.args.length() == g.arg_range_pred.length());
    ...
    ConjunctiveGoal* c = (ConjunctiveGoal*)this;
    for (index_type i = 0; i < c->goals.length(); i++) {
      if (!c->goals[i]->makeCPG(g, s)) return false;
    }
    return true;
  }
  if ((warning_level > 0)) {
...
PDDL_Base::CPG::CPG(CPG& g, index_vec& s)
  : atoms(0, 0), neg(false, 0), args(g.args), arg_types(g.arg_types),
    param_types(g.param_types), arg_range_pred(g.arg_range_pred),
    arg_range_set(g.arg_range_set)
{
...
void PDDL_Base::CPG::make_key_no_negation(ptr_table::key_vec& key)
{
  key.clear();
...
//     v->sym_types.assign_copy(arg_types[k]);
//     param[k] = v;
//   }
// }
...
void PDDL_Base::CPG::print(std::ostream& s)
{
  s << "CPG [";
  for (index_type k = 0; k < atoms.length(); k++) {
    s << " ";
...
bool PDDL_Base::CPG::initial_value()
{
  if (write_info) {
    std::cerr << "evaluating ";
    print(std::cerr);
    std::cerr << " in init state" << std::endl;
  }
  for (index_type k = 0; k < atoms.length(); k++) {
    bool atom_init_val = atoms[k]->initial_value();
    if (write_info) {
      atoms[k]->print(std::cerr);
      std::cerr << ": " << atom_init_val << std::endl;
    }
...
void PDDL_Base::CPG::make_effect_conditions
(Context* e,
 variable_vec& act_param,
 ParamSymbol* pf,
 bool_vec& sat,
 symbol_vec& subs,
 symbol_pair_vec& eq,
 ...
 atom_vec& ng)
{
  // effect (context) conditions consist of
  ...
  // 3. positive and negative guards
  for (index_type k = 0; k < pg.length(); k++) {
    e->add_parameters_for_atom(pg[k], act_param);
    e->pos_con.append(pg[k]);
  }
  for (index_type k = 0; k < ng.length(); k++) {
    e->add_parameters_for_atom(ng[k], act_param);
    e->pos_con.append(ng[k]);
  }
  // 4. atoms in the CPG not marked as "sat"
 ...
 bool strict)
{
  ...
  atom_vec ng;
  atom_set_vec p_eff;
  qc_numeric_effect_vec f_eff;
  compute_asserting_effects(0, act, 0, sat, p, p_val, g, f, f_val,
			    eq, neq, pg, ng, strict, p_eff, f_eff);
  for (index_type k = 0; k < p_eff.length(); k++) {
    if (p_eff[k]->context_is_static()) {
      act->set_eff.append(p_eff[k]);
      if (write_info) {
	std::cerr << "info: added set effect ";
	p_eff[k]->print(std::cerr);
	std::cerr << " to action " << act->print_name << std::endl;
      }
    }
    else {
      act->cond_eff.append(p_eff[k]);
      if (write_info) {
  ...
  for (index_type k = 0; k < f_eff.length(); k++) {
    act->qc_fass.append(f_eff[k]);
    if (write_info) {
      std::cerr << "info: added QCN effect ";
      f_eff[k]->print(std::cerr);
      std::cerr << " to action " << act->print_name << std::endl;
    }
  }
}

void PDDL_Base::CPG::asserting_effects_satisfying_case
(Atom* e_atom,
 index_type c_atom,
 ActionSymbol* act,
 index_type c_eff,
 ...
 bool strict,
 atom_set_vec& pe,
 qc_numeric_effect_vec& fe)
{
  ParamSymbol* pf = (p ? (ParamSymbol*)p : (ParamSymbol*)f);
  if (!sat[c_atom]) {
    bool poss = true;
    for (index_type k = 0; (k < e_atom->param.length()) && poss; k++) {
      for (index_type i = 0; (i < neq.length()) && poss; i++)
	if ((neq[i].first == pf->param[atom_first_arg[c_atom] + k]) &&
	    (neq[i].second == e_atom->param[k]))
	  poss = false;
      if (!pf->param[atom_first_arg[c_atom] + k]->
	  equality_type_check(e_atom->param[k]))
	poss = false;
    }
    if (poss) {
      for (index_type k = 0; k < e_atom->param.length(); k++)
	eq.append(symbol_pair(pf->param[atom_first_arg[c_atom] + k],
			      e_atom->param[k]));
      sat[c_atom] = true;
      compute_asserting_effects(c_atom, act, c_eff + 1, sat,
				p, p_val, g, f, f_val, eq, neq, pg, ng,
				strict, pe, fe);
      sat[c_atom] = false;
      eq.dec_length(e_atom->param.length());
    }
  }
  compute_asserting_effects(c_atom, act, c_eff + 1, sat,
			    p, p_val, g, f, f_val, eq, neq, pg, ng,
...
void PDDL_Base::CPG::asserting_effects_conditional_satisfying_case
(Atom* e_atom,
 index_type c_atom,
 ActionSymbol* act,
 index_type c_eff,
 ...
 qc_numeric_effect_vec& fe)
{
  ...
  if (!sat[c_atom]) {
	...
	eq.append(symbol_pair(pf->param[atom_first_arg[c_atom] + k],
      ...
      sat[c_atom] = false;
      eq.dec_length(e_atom->param.length());
      pg.dec_length(pos_con.length());
      ng.dec_length(neg_con.length());
    }
  }
  compute_asserting_effects(c_atom, act, c_eff + 1, sat,
			    p, p_val, g, f, f_val, eq, neq, pg, ng,
...
void PDDL_Base::CPG::asserting_effects_falsifying_case
(Atom* e_atom,
 index_type c_atom,
 ActionSymbol* act,
 index_type c_eff,
...
void PDDL_Base::CPG::asserting_effects_conditional_falsifying_case
(Atom* e_atom,
 index_type c_atom,
 ActionSymbol* act,
 index_type c_eff,
...
void PDDL_Base::CPG::compute_asserting_effects
(index_type c_atom,
 ActionSymbol* act,
 index_type c_eff,
 bool_vec& sat,
...
//       std::cerr << ", matched = " << matched << std::endl;
	...
	if (neg[c_atom]) { // falsifying case (adds negative atom)
	  asserting_effects_falsifying_case
	    (act->adds[c_eff], c_atom, act, c_eff, sat,
	     p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	}
	else { // satisfying case (adds positive atom)
	  asserting_effects_satisfying_case
	    (act->adds[c_eff], c_atom, act, c_eff, sat,
	     p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	}
      }
...
//       std::cerr << ", matched = " << matched << std::endl;
	...
	if (!neg[c_atom]) { // falsifying case (dels positive atom)
	  asserting_effects_falsifying_case
	    (act->dels[d_eff], c_atom, act, c_eff, sat,
	     p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	}
	...
	compute_asserting_effects(c_atom, act, c_eff + 1, sat,
		      ...
		      act->set_eff.length())) {
      index_type i_eff = c_eff - (act->adds.length() + act->dels.length());
      // set effect with one postive atom only
      if ((act->set_eff[i_eff]->pos_atoms.length() == 1) &&
	  (act->set_eff[i_eff]->neg_atoms.length() == 0)) {
	if (atoms[c_atom]->pred == act->set_eff[i_eff]->pos_atoms[0]->pred) {
	  if (neg[c_atom]) { // falsifying case (adds negative atom)
	    asserting_effects_conditional_falsifying_case
	      (act->set_eff[i_eff]->pos_atoms[0], c_atom, act, c_eff, sat,
	       act->set_eff[i_eff]->pos_con, act->set_eff[i_eff]->neg_con,
	       p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	  }
	  else { // satisfying case (adds positive atom)
	    asserting_effects_conditional_satisfying_case
	      (act->set_eff[i_eff]->pos_atoms[0], c_atom, act, c_eff, sat,
	       act->set_eff[i_eff]->pos_con, act->set_eff[i_eff]->neg_con,
	       p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	  }
      ...
      // set effect with one negative atom only
      else if ((act->set_eff[i_eff]->pos_atoms.length() == 0) &&
	       (act->set_eff[i_eff]->neg_atoms.length() == 1)) {
	if (atoms[c_atom]->pred == act->set_eff[i_eff]->neg_atoms[0]->pred) {
	  if (!neg[c_atom]) { // falsifying case (dels positive atom)
	    asserting_effects_conditional_falsifying_case
	      (act->set_eff[i_eff]->neg_atoms[0], c_atom, act, c_eff, sat,
	       act->set_eff[i_eff]->pos_con, act->set_eff[i_eff]->neg_con,
	       p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	  }
	  else { // satisfying case (dels negative atom)
	...
	std::cerr << "warning: quantified/conditional effect ";
	act->set_eff[i_eff]->print(std::cerr);
	std::cerr << " ignored in compilation" << std::endl;
	if (!best_effort) exit(1);
      }
    }
    else if (c_eff < (act->adds.length() + act->dels.length() +
		      act->set_eff.length() + act->cond_eff.length())) {
      index_type i_eff = c_eff -
	(act->adds.length() + act->dels.length() + act->set_eff.length());
      // cond effect with one postive atom only
      if ((act->cond_eff[i_eff]->pos_atoms.length() == 1) &&
	  (act->cond_eff[i_eff]->neg_atoms.length() == 0)) {
	if (atoms[c_atom]->pred == act->cond_eff[i_eff]->pos_atoms[0]->pred) {
	  if (neg[c_atom]) { // falsifying case (adds negative atom)
	    asserting_effects_conditional_falsifying_case
	      (act->cond_eff[i_eff]->pos_atoms[0], c_atom, act, c_eff, sat,
	       act->cond_eff[i_eff]->pos_con, act->cond_eff[i_eff]->neg_con,
	       p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	  }
	  else { // satisfying case (adds positive atom)
	    asserting_effects_conditional_satisfying_case
      ...
      // cond effect with one negative atom only
      else if ((act->cond_eff[i_eff]->pos_atoms.length() == 0) &&
	       (act->cond_eff[i_eff]->neg_atoms.length() == 1)) {
	if (atoms[c_atom]->pred == act->cond_eff[i_eff]->neg_atoms[0]->pred) {
	  if (!neg[c_atom]) { // falsifying case (dels positive atom)
	    asserting_effects_conditional_falsifying_case
	      (act->cond_eff[i_eff]->neg_atoms[0], c_atom, act, c_eff, sat,
	       act->cond_eff[i_eff]->pos_con, act->cond_eff[i_eff]->neg_con,
	       p, p_val, g, f, f_val, eq, neq, pg, ng, strict, pe, fe);
	  }
	  else { // satisfying case (dels negative atom)
	...
	std::cerr << "warning: quantified/conditional effect ";
	act->cond_eff[i_eff]->print(std::cerr);
	std::cerr << " ignored in compilation" << std::endl;
	if (!best_effort) exit(1);
      }
    }
    else {
      compute_asserting_effects(c_atom + 1, act, 0, sat,
				p, p_val, g, f, f_val, eq, neq, pg, ng,
				strict, pe, fe);
    }
  }
	...
	while (!done) {
	  ...
	  make_effect_conditions(e, act->param, p, sat,
				 subs, rem_eq, neq, pg, ng);
	  if (strict) {
...
void PDDL_Base::compile_preferences_1()
{
  ...
  for (index_type p = 0; p < dom_preferences.length(); p++) if (cpg[p]) {
	...
	cpg[p]->add_asserting_effects
	  (dom_actions[a], 0, false, 0, f_violated[p], exp0, false);
	cpg[p]->add_destroying_effects
	  (dom_actions[a], f_violated[p], 0, false, 0, f_violated[p], exp1,
	   false);
      }
...
void PDDL_Base::compile_preferences_2
(bool ignore_tc_fun, bool use_shared_pred)
{
  if (dom_preferences.length() == 0) return;
  current_eq_predicate = dom_eq_pred;

  CPG* cpg[dom_preferences.length()];
  PredicateSymbol* p_sat[dom_preferences.length()];
  FunctionSymbol* f_cost[dom_preferences.length()];
  ptr_table signatures;
  ptr_table* signature[dom_preferences.length()];
  bool_vec compilable(false, dom_preferences.length());
  index_type n_compilable = 0;
  ...
  if (write_info) {
    std::cerr << "info: " << n_compilable << " of " << dom_preferences.length()
	      << " preferences compilable" << std::endl;
  }
  ...
  // find or create a total-cost function
  bool have_tc_fun = false;
  FunctionSymbol* f_tc = 0;
  if (!ignore_tc_fun) {
    f_tc = find_function("total-cost");
    if (f_tc != 0) {
      have_tc_fun = true;
      ...
      f_tc->increased = true;
      if (write_info) {
	std::cerr << "info: using existing function ";
	f_tc->print(std::cerr);
	std::cerr << std::endl;
      }
    }
  ...
  if (f_tc == 0) {
    f_tc = new FunctionSymbol("total-cost");
    f_tc->modified = true;
    f_tc->increased = true;
    dom_functions.append(f_tc);
    FInitAtom* tc_init = new FInitAtom(f_tc);
    tc_init->val = 0;
    add_init_fun(tc_init);
  }
  ...
  for (index_type p = 0; p < dom_preferences.length(); p++) {
	...
	std::cerr << std::endl;
      ...
      if (p_sat[p] == 0) {
	EnumName p_name("pref_sat", n_signatures);
	p_sat[p] = new PredicateSymbol(p_name.to_cstring(Name::NC_INSTANCE));
	index_type p_arity = cpg[p]->args.length();
	type_vec p_types(cpg[p]->arg_types);
	if (use_shared_pred) {
	  for (index_type q = p + 1; q < dom_preferences.length(); q++)
	    if (signature[q] == signature[p])
	      if (merge_type_vectors(p_types, cpg[q]->arg_types)) {
		p_sat[q] = p_sat[p];
		for (index_type k = 0; k < cpg[p]->arg_range_set.length(); k++)
		  cpg[p]->arg_range_set[k].insert(cpg[q]->arg_range_set[k]);
	      }
	}
	make_parameters(p_types, "?arg", p_sat[p]->param);
	// cpg[p]->make_range_predicates(this);
	p_sat[p]->pos_pre = true;
	p_sat[p]->neg_pre = false;
	p_sat[p]->added = true;
	p_sat[p]->deleted = true;
	p_sat[p]->modded = true;
	dom_predicates.append(p_sat[p]);
	if (write_info) {
	  std::cerr << "info: created CPG predicate ";
	  p_sat[p]->print(std::cerr);
	}

	for (index_type a = 0; a < dom_actions.length(); a++) {
	  cpg[p]->add_asserting_effects(dom_actions[a], p_sat[p], true,
					0, 0, 0, false);
	  cpg[p]->add_destroying_effects(dom_actions[a], p_sat[p], p_sat[p],
					 false, 0, 0, 0, false);
	}

	if (use_shared_pred) {
	  EnumName f_name("pref_cost", n_signatures);
	  f_cost[p] = new FunctionSymbol(f_name.to_cstring(Name::NC_INSTANCE));
	  make_parameters(p_types, "?arg", f_cost[p]->param);
	  dom_functions.append(f_cost[p]);
	  for (index_type q = p + 1; q < dom_preferences.length(); q++)
	    if (p_sat[q] == p_sat[p])
	      f_cost[q] = f_cost[p];
	}

	EnumName a_name("pref_skip", n_signatures);
	ActionSymbol* a_skip =
	  new ActionSymbol(a_name.to_cstring(Name::NC_INSTANCE));
	make_parameters(p_types, "?arg", a_skip->param);
	if (use_shared_pred) {
	  Expression* d_exp =
	    new FunctionExpression(f_cost[p], a_skip->param_list());
	  FChangeAtom* d_atom = new FChangeAtom(f_tc);
	  d_atom->val = d_exp;
	  a_skip->incs.append(d_atom);
	}
	else {
	  Expression* d_exp =
	    new ConstantExpression(dom_preferences[p]->value(metric_type,
							     metric));
	  FChangeAtom* d_atom = new FChangeAtom(f_tc);
	  d_atom->val = d_exp;
	  a_skip->incs.append(d_atom);
	}
	Atom* p_add = new Atom(p_sat[p], a_skip->param, false);
	a_skip->adds.append(p_add);
	dom_actions.append(a_skip);

	n_signatures += 1;
      }

      Atom* p_ins = new Atom(p_sat[p]);
      p_ins->param = cpg[p]->args;
      if (cpg[p]->initial_value()) {
	add_init_atom(p_ins);
      }
      add_goal_atom(p_ins, false);

      if (use_shared_pred) {
	FInitAtom* c_ins = new FInitAtom(f_cost[p]);
	c_ins->param = cpg[p]->args;
	c_ins->val = dom_preferences[p]->value(metric_type, metric);
	add_init_fun(c_ins);
      }
    }
    else {
  ...
  // fix the metric
  if ((metric_type == metric_minimize) || (metric_type == metric_maximize)) {
    assert(metric != 0);
    Expression* e_zero = new ConstantExpression(0);
    for (index_type p = 0; p < dom_preferences.length(); p++)
      if (compilable[p])
	metric =
	  metric->substitute_for_preference(dom_preferences[p]->name, e_zero);
    if (!have_tc_fun) {
      if (metric_type == metric_minimize) {
	metric = new BinaryExpression(exp_add, metric,
				      new FunctionExpression(f_tc, 0));
      }
      else { // metric_type == metric_maximize
	metric = new BinaryExpression(exp_sub, metric,
				      new FunctionExpression(f_tc, 0));
      }
    }
  ...
  if (write_info) {
    std::cerr << "info: "
	      << n_compilable << " preferences compiled, "
	      << n_signatures << " CPG predicates created, "
	      << dom_preferences.length() << " preferences remain"
	      << std::endl;
  }
}
...
(SimpleSequenceGoal* g, const Symbol* n, ptr_table& cs)
{
  current_eq_predicate = dom_eq_pred;
  if (write_info) {
  ...
  if (!g->constraint->makeCPG(f)) {
    if (!g->constraint->make_negated_CPG(f)) {
      if ((warning_level > 0) || !best_effort) {
	std::cerr << "warning: can't compile goal ";
	g->print(std::cerr);
	std::cerr << std::endl;
  ...
  if (is_neg)
    ...
    sig.append((void*)0);
  ptr_table* p_ex = cs.insert(sig);

  if (p_ex->val != 0) {
    p_ok = (PredicateSymbol*)p_ex->val;
  }
  else {
    p_ok = (PredicateSymbol*)gensym_s(sym_predicate, "ok-", n, 0);
    make_parameters(f.param_types, "?arg", p_ok->param);
    p_ok->pos_pre = true;
    ...
    for (index_type k = 0; k < dom_actions.length(); k++) {
      if (is_neg) {
	// if the CPG is negated, any action that makes it true should
	// delete p_ok
	f.add_asserting_effects(dom_actions[k], p_ok, false, 0, 0, 0, false);
      }
      else {
	// else, any action that makes it false should delete p_ok
...
(SimpleSequenceGoal* g, const Symbol* n, ptr_table& cs)
{
  ...
  if (p_ex->val != 0) {
    ptr_pair* pp = (ptr_pair*)p_ex->val;
    p_once = (PredicateSymbol*)pp->first;
    p_ok = (PredicateSymbol*)pp->second;
  }
  else {
    p_once = (PredicateSymbol*)gensym_s(sym_predicate, "once-", n, 0);
    make_parameters(f.param_types, "?arg", p_once->param);
    p_once->pos_pre = true;
    p_once->added = true;
    p_once->modded = true;
    ...
    ptr_pair* pp = new ptr_pair(p_once, p_ok);
    p_ex->val = pp;
  }
...
(TriggeredSequenceGoal* g, const Symbol* n, ptr_table& cs)
{
  current_eq_predicate = dom_eq_pred;
  if (write_info) {
    std::cerr << "info: compiling constraint ";
  ...
  make_parameters(f_c.param_types, "?arg", p_not_safe->param);
  ...
  make_parameters(f_tr.param_types, "?arg", p_ok->param);
  p_ok->pos_pre = true;
  p_ok->deleted = true;
  p_ok->modded = true;
  dom_predicates.append(p_ok);
  ...
  // every action that adds f_tr when "not-safe" is true dels "ok"
  for (index_type k = 0; k < dom_actions.length(); k++) {
    bool_vec sat(false, f_c.atoms.length());
    symbol_pair_vec eq;
    symbol_pair_vec neq;
...
(PDDL_Base::Goal* g, const Symbol* n,
 ptr_table& acs, ptr_table& ecs, ptr_table& ocs, ptr_table& bcs)
{
  if (g->g_class == goal_always) {
    return compile_always_constraint((SimpleSequenceGoal*)g, n, acs);
  }
  else if (g->g_class == goal_sometime) {
    return compile_sometime_constraint((SimpleSequenceGoal*)g, n, ecs);
  }
  else if (g->g_class == goal_at_most_once) {
    return compile_at_most_once_constraint((SimpleSequenceGoal*)g, n, ocs);
  }
  else if (g->g_class == goal_sometime_before) {
    return compile_sometime_before_constraint((TriggeredSequenceGoal*)g, n, bcs);
  }
  else if (g->g_class == goal_sometime_after) {
    if ((warning_level > 0) || !best_effort) {
...
(ActionSymbol* act, variable_vec& i_param, variable_vec& d_param, index_type p)
{
  if (p < d_param.length()) {
    ...
    for (index_type k = 0; k < act->set_eff.length(); k++) {
      ...
      act->set_eff[k]->compile(new_act->adds, new_act->dels, 0);
    }
    ...
    if (act->dmin == act->dmax) {
      if (act->dmin) {
	Expression* tmp = act->dmin->instantiate_partially();
	new_act->dmin = tmp->simplify();
	new_act->dmax = new_act->dmin;
      }
    }
    else {
      if (act->dmin) {
	...
	new_act->dmin = 0;
      ...
      if (act->dmax) {
	Expression* tmp = act->dmax->instantiate_partially();
	new_act->dmax = tmp->simplify();
      }
      else {
	new_act->dmax = 0;
      }
...
void PDDL_Base::compile_set_conditions_and_effects(ActionSymbol* act)
{
  ...
  for (index_type k = 0; k < act->param.length(); k++) {
    ...
    for (index_type i = 0; (i < act->cond_eff.length()) && !found; i++)
      if (act->cond_eff[i]->occurs_in_context(act->param[k])) {
	dp.append(act->param[k]);
	found = true;
      }
    if (!found)
...
void PDDL_Base::make_complementary_init
(PredicateSymbol* pred, PredicateSymbol* cpred, Atom* a, index_type p)
{
  if (p < pred->param.length()) {
    for (index_type k = 0; k < pred->param[p]->sym_types.n_elements(); k++) {
      a->param[p] = pred->param[p]->sym_types.get_element(k);
      make_complementary_init(pred, cpred, a, p + 1);
    }
    a->param[p] = 0;
  }
  else {
    if (!a->initial_value()) {
      Atom* b = new Atom(cpred, a);
      b->insert(cpred->init);
      dom_init.append(b);
    }
  }
...
void PDDL_Base::compile_negations()
{
  predicate_vec npred;
  predicate_vec cpred;
  // identify negated predicates, and create their complements
  for (index_type k = 0; k < dom_predicates.size(); k++)
    if (dom_predicates[k]->neg_pre) {
      if (write_info) {
	std::cerr << "info: creating complementary predicate for "
		  << dom_predicates[k]->print_name << std::endl;
      }
      char* nn = new char[strlen(dom_predicates[k]->print_name) + 6];
      strcpy(nn, "not-");
      strcat(nn, dom_predicates[k]->print_name);
      PredicateSymbol* np = new PredicateSymbol(nn);
      np->param.append(dom_predicates[k]->param);
      np->pos_pre = true;
      if (dom_predicates[k]->added) np->deleted = true;
      if (dom_predicates[k]->deleted) np->added = true;
      np->modded = dom_predicates[k]->modded;
      npred.append(dom_predicates[k]);
      cpred.append(np);
    }
  if (dom_eq_pred->neg_pre) {
    if (write_info) {
      std::cerr << "info: creating predicate not-eq as complement of '='"
		<< std::endl;
    }
    char* nn = strdup("not-eq");
    PredicateSymbol* np = new PredicateSymbol(nn);
    np->param.append(dom_eq_pred->param);
    np->pos_pre = true;
    npred.append(dom_eq_pred);
    cpred.append(np);
  }
  assert(cpred.size() == npred.size());
  // substitute completements in action preconditions, and introduce
  // the complementary add/del effects
  for (index_type k = 0; k < dom_actions.size(); k++) {
    for (index_type i = 0; i < dom_actions[k]->neg_pre.size(); i++) {
      Atom* a = dom_actions[k]->neg_pre[i];
      index_type j = npred.first(a->pred);
      assert(j < cpred.size());
      a->pred = cpred[j];
      dom_actions[k]->pos_pre.append(a);
    }
    dom_actions[k]->neg_pre.clear();
    index_type n_dels = dom_actions[k]->dels.size();
    for (index_type i = 0; i < dom_actions[k]->adds.size(); i++) {
      Atom* a = dom_actions[k]->adds[i];
      index_type j = npred.first(a->pred);
      if (j < cpred.size()) {
	Atom* b = new Atom(cpred[j], a);
	dom_actions[k]->dels.append(b);
      }
    }
    for (index_type i = 0; i < n_dels; i++) {
      Atom* a = dom_actions[k]->dels[i];
      index_type j = npred.first(a->pred);
      if (j < cpred.size()) {
	Atom* b = new Atom(cpred[j], a);
	dom_actions[k]->adds.append(b);
      }
    }
  }
  // substitute in goals
  for (index_type k = 0; k < dom_goals.size(); k++)
    if (dom_goals[k]->g_class == goal_neg_atom) {
      AtomicGoal* g = (AtomicGoal*)dom_goals[k];
      Atom* a = g->atom;
      index_type j = npred.first(a->pred);
      assert(j < cpred.size());
      a->pred = cpred[j];
      dom_goals[k]->g_class = goal_pos_atom;
    }
  // add complement predicates to domain, and add initialisations
  for (index_type i = 0; i < cpred.size(); i++) {
    dom_predicates.append(cpred[i]);
    assert(i < npred.size());
    assert(npred[i] != 0);
    Atom* a = new Atom(npred[i]);
    a->param.assign_value(0, npred[i]->param.length());
    make_complementary_init(npred[i], cpred[i], a, 0);
    delete a;
  }
}
...
void PDDL_Base::compile_object_functions()
{
  ...
  // compile object functions in goals and preferences
  for (index_type k = 0;k < dom_goals.length(); k++) {
...
void PDDL_Base::detil(bool force, NTYPE dur, action_vec& ea, cost_vec& tp)
{
  cost_set tps;
  for (index_type k = 0; k < dom_pos_til.size(); k++)
    tps.insert(dom_pos_til[k]->at_time);
  for (index_type k = 0; k < dom_neg_til.size(); k++)
    tps.insert(dom_neg_til[k]->at_time);
  tp.assign_copy(tps);
  ea.assign_value(0, tp.size());
  // create atoms (0-ary predicates)
  predicate_vec at_tp;
  at_tp.append(new PredicateSymbol("at-til-tp0"));
  for (index_type k = 0; k < tps.size(); k++) {
    std::ostringstream tpname;
    tpname << "at-til-tp" << k + 1;
    at_tp.append(new PredicateSymbol(strdup(tpname.str().c_str())));
  }
  for (index_type k = 0; k < at_tp.size(); k++) {
    at_tp[k]->pos_pre = true;
    at_tp[k]->added = true;
    at_tp[k]->deleted = true;
    at_tp[k]->modded = true;
    dom_predicates.append(at_tp[k]);
  }
  Atom* init_tp0 = new Atom(at_tp[0]);
  init_tp0->insert(at_tp[0]->init);
  dom_init.append(init_tp0);
  if (force && (tps.size() > 0)) {
    Atom* goal_tp = new Atom(at_tp[at_tp.size() - 1]);
    dom_goals.append(new AtomicGoal(goal_tp, false));
  }
  // create actions
  for (index_type k = 0; k < tps.size(); k++) {
    std::ostringstream actname;
    actname << "advance-to-til-tp" << k + 1;
    ActionSymbol* act = new ActionSymbol(strdup(actname.str().c_str()));
    act->pos_pre.append(new Atom(at_tp[k]));
    act->dels.append(new Atom(at_tp[k]));
    act->adds.append(new Atom(at_tp[k + 1]));
    for (index_type i = 0; i < dom_pos_til.size(); i++)
      if (dom_pos_til[i]->at_time == tps[k]) {
	Atom* a = new Atom(dom_pos_til[i]->pred);
	a->param = dom_pos_til[i]->param;
	act->adds.append(a);
      }
    for (index_type i = 0; i < dom_neg_til.size(); i++)
      if (dom_neg_til[i]->at_time == tps[k]) {
	Atom* a = new Atom(dom_neg_til[i]->pred);
	a->param = dom_neg_til[i]->param;
	act->dels.append(a);
      }
    act->dmin = new ConstantExpression(dur);
    dom_actions.append(act);
    ea[k] = act;
  }
  // done!
}

void PDDL_Base::compile_fluent_defined_check()
{
  predicate_vec deffun(0, dom_functions.size());
  for (index_type k = 0; k < dom_functions.size(); k++) {
    deffun[k] = (PredicateSymbol*)
      gensym_s(sym_predicate, "defined_", dom_functions[k], 0);
    deffun[k]->param.assign_copy(dom_functions[k]->param);
    deffun[k]->pos_pre = true;
    ptr_table::value_vec inits;
    dom_functions[k]->init.dump_values(inits);
    for (index_type i = 0; i < inits.size(); i++) {
      Atom* a = new Atom(deffun[k], (AtomBase*)inits[i]);
      a->insert(deffun[k]->init);
      dom_init.append(a);
    }
  }
  dom_predicates.append(deffun);
  for (index_type k = 0; k < dom_actions.size(); k++) {
    // collect references fluents
    ch_atom_vec fch;
    exp_vec fexp;
    if (dom_actions[k]->dmin)
      dom_actions[k]->dmin->collect_functions(fexp);
    if (dom_actions[k]->dmax)
      dom_actions[k]->dmax->collect_functions(fexp);
    for (index_type i = 0; i < dom_actions[k]->reqs.size(); i++) {
      fch.append(dom_actions[k]->reqs[i]);
      dom_actions[k]->reqs[i]->val->collect_functions(fexp);
    }
    for (index_type i = 0; i < dom_actions[k]->incs.size(); i++) {
      fch.append(dom_actions[k]->incs[i]);
      dom_actions[k]->incs[i]->val->collect_functions(fexp);
    }
    for (index_type i = 0; i < dom_actions[k]->decs.size(); i++) {
      fch.append(dom_actions[k]->decs[i]);
      dom_actions[k]->decs[i]->val->collect_functions(fexp);
    }
    for (index_type i = 0; i < dom_actions[k]->fass.size(); i++) {
      fch.append(dom_actions[k]->fass[i]);
      dom_actions[k]->fass[i]->val->collect_functions(fexp);
    }
    // add defined precs
    for (index_type i = 0; i < fch.size(); i++) {
      index_type p = dom_functions.first(fch[i]->fun);
      if (p == no_such_index) {
	std::cerr << "error: function " << fch[i]->fun->print_name
		  << " not declared? (1)" << std::endl;
	exit(255);
      }
      assert(p != no_such_index);
      Atom* a = new Atom(deffun[p]);
      a->param.assign_copy(fch[i]->param);
      dom_actions[k]->pos_pre.append(a);
    }
    for (index_type i = 0; i < fexp.size(); i++) {
      FunctionExpression* fe = (FunctionExpression*)fexp[i];
      index_type p = dom_functions.first(fe->fun);
      if (p == no_such_index) {
	std::cerr << "error: function " << fe->fun->print_name
		  << " not declared? (2)" << std::endl;
	exit(255);
      }
      assert(p != no_such_index);
      Atom* a = new Atom(deffun[p]);
      arg_list_to_vec(fe->args, a->param);
      dom_actions[k]->pos_pre.append(a);
    }
  }
}

void PDDL_Base::init_static_fluent_expression
(variable_vec& vs, index_type p, Expression* exp, FunctionSymbol* f)
{
  if (p < vs.length()) {
    for (index_type k = 0; k < vs[p]->sym_types.n_elements(); k++) {
      vs[p]->value = vs[p]->sym_types.get_element(k);
      init_static_fluent_expression(vs, p + 1, exp, f);
    }
    vs[p]->value = 0;
  }
  else {
    if (exp->is_defined()) {
      NTYPE val = exp->eval_static();
      FInitAtom* fi = new FInitAtom(f, vs, true, val);
      fi->insert(f->init);
      dom_fun_init.append(fi);
    }
  }
}

PDDL_Base::Expression*
PDDL_Base::compile_static_fluent_expression(Expression* exp, index_type n)
{
  variable_vec vs(0, 0);
  exp->collect_variables(vs);
  FunctionSymbol* fe =
    (FunctionSymbol*)gensym_i(sym_function, "static_exp", n, 0);
  make_parameters(vs, "?arg", fe->param);
  dom_functions.append(fe);
  init_static_fluent_expression(vs, 0, exp, fe);
  ListExpression* arg = arg_vec_to_list(vs, false);
  return new FunctionExpression(fe, arg);
}

void PDDL_Base::compile_static_fluent_expressions()
{
  index_type n = 0;
  for (index_type k = 0; k < dom_actions.length(); k++) {
    for (index_type i = 0; i < dom_actions[k]->reqs.length(); i++)
      if (dom_actions[k]->reqs[i]->val->is_static() &&
	  (dom_actions[k]->reqs[i]->val->exp_class != exp_const) &&
	  (dom_actions[k]->reqs[i]->val->exp_class != exp_fun))
	dom_actions[k]->reqs[i]->val =
	  compile_static_fluent_expression(dom_actions[k]->reqs[i]->val, n++);
    for (index_type i = 0; i < dom_actions[k]->incs.length(); i++)
      if (dom_actions[k]->incs[i]->val->is_static() &&
	  (dom_actions[k]->incs[i]->val->exp_class != exp_const) &&
	  (dom_actions[k]->incs[i]->val->exp_class != exp_fun))
	dom_actions[k]->incs[i]->val =
	  compile_static_fluent_expression(dom_actions[k]->incs[i]->val, n++);
    for (index_type i = 0; i < dom_actions[k]->decs.length(); i++)
      if (dom_actions[k]->decs[i]->val->is_static() &&
	  (dom_actions[k]->decs[i]->val->exp_class != exp_const) &&
	  (dom_actions[k]->decs[i]->val->exp_class != exp_fun))
	dom_actions[k]->decs[i]->val =
	  compile_static_fluent_expression(dom_actions[k]->decs[i]->val, n++);
    bool same_exp = (dom_actions[k]->dmax == dom_actions[k]->dmin);
    if (dom_actions[k]->dmin) {
      if (dom_actions[k]->dmin->is_static() &&
	  (dom_actions[k]->dmin->exp_class != exp_const) &&
	  (dom_actions[k]->dmin->exp_class != exp_fun)) {
	dom_actions[k]->dmin =
	  compile_static_fluent_expression(dom_actions[k]->dmin, n++);
	if (same_exp) {
	  dom_actions[k]->dmax = dom_actions[k]->dmin;
	}
      }
    }
    if (dom_actions[k]->dmax && !same_exp) {
      if (dom_actions[k]->dmax->is_static() &&
	  (dom_actions[k]->dmax->exp_class != exp_const) &&
	  (dom_actions[k]->dmax->exp_class != exp_fun))
	dom_actions[k]->dmax =
	  compile_static_fluent_expression(dom_actions[k]->dmax, n++);
    }
  }
}

void PDDL_Base::arg_list_to_vec(ListExpression* l, symbol_vec& v)
{
  while (l != 0) {
    v.append(l->sym);
    l = l->rest;
...
PDDL_Base::ListExpression* PDDL_Base::arg_vec_to_list
(variable_vec& v, bool as_value)
{
  ListExpression* e = 0;
  for (index_type p = v.length(); p > 0; p--)
    e = new ListExpression(as_value ? v[p - 1]->value : v[p - 1], e);
  return e;
}
...
PDDL_Base::ListExpression* PDDL_Base::ParamSymbol::param_list()
{
  ListExpression* tail = 0;
  index_type k = param.length();
  while (k > 0) {
    ListExpression* e = new ListExpression(param[k - 1], tail);
    tail = e;
    k -= 1;
  }
  return tail;
...
void PDDL_Base::ParamSymbol::print(std::ostream& s)
{
  s << "(" << print_name;
  for (index_type k = 0; k < param.length(); k++) {
    s << " " << param[k]->print_name;
...
void PDDL_Base::ParamSymbol::print_with_bindings(std::ostream& s)
{
  s << "(" << print_name;
  for (index_type k = 0; k < param.length(); k++) {
    s << " " << param[k]->print_name << "=";
    if (param[k]->value) {
      s << param[k]->value->print_name;
    }
    else {
      s << "0";
    }
...
(Instance& ins, index_type p, Expression* cost_exp)
{
  if (p < param.length()) {
    ...
    for (index_type k = 0; k < reqs.length(); k++) {
      Instance::resource_ref rc = reqs[k]->find_resource(ins);
      if (!rc) {
	if ((warning_level > 0)) {
	  std::cerr << "warning: action instance ";
	  print_with_bindings(std::cerr);
	  std::cerr << " depends on undefined resource ";
	  reqs[k]->fun->print_with_bindings(std::cerr);
	  std::cerr << std::endl;
	}
	return;
      }
      if (!reqs[k]->val->is_defined()) {
	if ((warning_level > 0)) {
	  std::cerr << "warning: resource effect ";
	  reqs[k]->print(std::cerr);
	  std::cerr << " of action instance ";
	  print_with_bindings(std::cerr);
	  std::cerr << " is undefined" << std::endl;
	}
	return;
    ...
    for (index_type k = 0; k < decs.length(); k++) {
      Instance::resource_ref rc = decs[k]->find_resource(ins);
      if (!rc) {
	if ((warning_level > 0)) {
	  std::cerr << "warning: action instance ";
	  print_with_bindings(std::cerr);
	  std::cerr << " depends on undefined resource ";
	  decs[k]->fun->print_with_bindings(std::cerr);
	  std::cerr << std::endl;
	}
	return;
      }
      if (!decs[k]->val->is_defined()) {
	if ((warning_level > 0)) {
	  std::cerr << "warning: resource effect ";
	  decs[k]->print(std::cerr);
	  std::cerr << " of action instance ";
	...
	if ((warning_level > 0)) {
	  std::cerr << "warning: action instance ";
	  print_with_bindings(std::cerr);
	  std::cerr << " has undefined duration (min)" << std::endl;
	}
	return;
      }
      ac_dmin = dmin->eval_static();
      if ((ac_dmin <= 0) && (PDDL_Base::warning_level > 0)) {
	std::cerr << "warning: action instance ";
	print_with_bindings(std::cerr);
	std::cerr << " has (min) duration " << ac_dmin << " <= 0"
		  << std::endl;
      }
    }
    if (dmax) {
	...
	if ((warning_level > 0)) {
	  std::cerr << "warning: action instance ";
	  print_with_bindings(std::cerr);
	  std::cerr << " has undefined duration (max)" << std::endl;
	}
	return;
      }
      ac_dmax = dmax->eval_static();
      if ((ac_dmax <= 0) && (PDDL_Base::warning_level > 0)) {
	std::cerr << "warning: action instance ";
	print_with_bindings(std::cerr);
	std::cerr << " has (min) duration " << ac_dmin << " <= 0"
		  << std::endl;
    ...
    if (cost_exp) {
      // we've already checked decs are defined - need to check incs
      for (index_type k = 0; k < incs.length(); k++) {
	if (!incs[k]->is_defined()) {
	  if ((warning_level > 0)) {
	    std::cerr << "warning: action instance ";
	    print_with_bindings(std::cerr);
	    std::cerr << " has effect ";
	    incs[k]->print(std::cerr);
	    std::cerr << " on undefined function ";
	    incs[k]->fun->print_with_bindings(std::cerr);
	    std::cerr << std::endl;
	  }
	  return;
	}
	if (!incs[k]->val->is_defined()) {
	  if ((warning_level > 0)) {
	    std::cerr << "warning: numeric effect ";
	    incs[k]->print(std::cerr);
	    std::cerr << " of action instance ";
	    print_with_bindings(std::cerr);
	    std::cerr << " is undefined" << std::endl;
      ...
      // std::cerr << "evaluating cost exp: ";
      // cost_exp->print(std::cerr, true);
      // std::cerr << std::endl;
      ac_cost = cost_exp->eval_delta(incs, decs);
      if ((ac_cost <= 0) && (PDDL_Base::warning_level > 1)) {
	std::cerr << "warning: action instance ";
	print_with_bindings(std::cerr);
	std::cerr << " has cost " << ac_cost << " <= 0"
		  << std::endl;
      }
    }
	...
	Instance::atom_ref pp = pos_pre[k]->find_prop(ins, false, true);
	ac_pre.insert(pp->index);
      }
    for (index_type k = 0; k < neg_pre.length(); k++)
      if (!neg_pre[k]->is_static() ||
	  (create_all_actions && !neg_pre[k]->pred->is_equality())) {
	Instance::atom_ref np = neg_pre[k]->find_prop(ins, true, true);
    ...
    for (index_type k = 0; k < adds.length(); k++)  {
      Instance::atom_ref pp = adds[k]->find_prop(ins, false, true);
      ac_add.insert(pp->index);
      if (adds[k]->pred->neg_pre || create_all_atoms) {
	Instance::atom_ref np = adds[k]->find_prop(ins, true, true);
	ac_del.insert(np->index);
      }
    }
    for (index_type k = 0; k < dels.length(); k++)  {
      Instance::atom_ref pp = dels[k]->find_prop(ins, false, true);
      ac_del.insert(pp->index);
      if (dels[k]->pred->neg_pre || create_all_atoms) {
	Instance::atom_ref np = dels[k]->find_prop(ins, true, true);
    ...
    // instantiate locks
    for (index_type k = 0; k < locks.length(); k++) {
      Instance::atom_ref lp = locks[k]->find_prop(ins, false, true);
      ac_lck.insert(lp->index);
    }

    // warn about ignored parts
    bool conditioned_inc = false;
    for (index_type k = 0; (k < incs.length()) && !conditioned_inc; k++)
      if (incs[k]->fun->conditioned)
	conditioned_inc = true;
    if ((!enables.empty() || conditioned_inc || !fass.empty() ||
    ...
    // if we have disjunctive precondition
    if (ns_dis_pre.length() > 0) {
      if (compile_away_disjunctive_preconditions) {
    ...
    // sets of positive and negative condeffs (default empty)
    rule_set pce;
    rule_set nce;

    // if we have conditional effects...
	...
	for (index_type k = 0; k < nce.length(); k++)
      ...
      if (compile_away_conditional_effects) {
	...
	while (more) {
	  build_actions_with_dc_and_ce(ins, name, count_name, ac_pre, ac_add,
				       ...
				       ac_dmax, ac_dur, ac_cost, ac_dc,
				       pce, nce, se.current_set());
	  more = se.next();
	}
      }
      else {
	// not compiling away condeffs
	build_actions_with_dc(ins, name, count_name, ac_pre, ac_add, ac_del,
			      pce, nce, ac_lck, ac_use, ac_cons,
			      ac_dmin, ac_dmax, ac_dur, ac_cost, ac_dc);
      }
    }
    else { // no condeffs
      build_actions_with_dc(ins, name, count_name, ac_pre, ac_add, ac_del,
...
(Instance& ins, PDDL_Name* name, index_type& count,
 ...
 const rule_set& cadd, const rule_set& cdel, const index_set& lck,
 const index_cost_vec& r_use, const index_cost_vec& r_cons,
 NTYPE dmin, NTYPE dmax, NTYPE d, NTYPE c)
{
  ...
  for (index_type k = 0; k < refs.length(); k++) {
	...
	std::cerr << " matched to "
      ...
      if ((warning_level > 0) && !refs[k]->index.empty()) {
  ...
  //std::cerr << "created action: " << std::endl;
  //ins.print_action(std::cerr, act);
 ...
 const index_set_vec& dc, index_vec& s, index_type p)
{
  if (p < dc.length()) {
	...
	(dc[p].first_common_element(pre) != no_such_index)) {
      build_actions_with_dc(ins, name, count, pre, add, del, cadd, cdel,
			    lck, r_use, r_cons, dmin, dmax, d, c, dc, s,
			    p + 1);
    }
    else {
    ...
    Instance::Action& act =
      build_action(ins, name, count, pre, add, del, cadd, cdel, lck,
		   r_use, r_cons, dmin, dmax, d, c);
    for (index_type k = 0; k < s.length(); k++)
      act.pre.insert(s[k]);
  }
}
...
(Instance& ins, PDDL_Name* name, index_type& count,
 ...
 const rule_set& cadd, const rule_set& cdel, const index_set& lck,
 const index_cost_vec& r_use, const index_cost_vec& r_cons,
 NTYPE dmin, NTYPE dmax, NTYPE d, NTYPE c, const index_set_vec& dc)
{
  index_vec s(no_such_index, 0);
  build_actions_with_dc(ins, name, count, pre, add, del, cadd, cdel, lck,
			r_use, r_cons, dmin, dmax, d, c, dc, s, 0);
}
 ...
 const bool_vec& ece)
{
  ...
  rule_set no_ce;
  index_vec s(no_such_index, 0);
  build_actions_with_dc(ins, name, count, x_pre, x_add, x_del, no_ce, no_ce,
			lck, r_use, r_cons, dmin, dmax, d, c, x_dc, s, 0);
}
...
(HTNInstance& ins, HTNInstance::Task& tk, index_type p)
{
  if (p < param.length()) {
	...
	if (!pos_con[k]->is_static()) {
	  Instance::atom_ref pp = pos_con[k]->find_prop(ins, false, true);
	  met.pre.insert(pp->index);
	}
      }
      for (index_type k = 0; k < neg_con.length(); k++) {
	if (!neg_con[k]->is_static()) {
	  Instance::atom_ref np = neg_con[k]->find_prop(ins, true, true);
...
index_type PDDL_Base::Reference::find_atom()
{
  if (name->sym_class != sym_predicate) {
    std::cerr << "warning: ";
    name->print(std::cerr);
    std::cerr << " of ";
    print(std::cerr);
    std::cerr << " is not a predicate symbol" << std::endl;
    return no_such_index;
  }
  PredicateSymbol* pred = (PredicateSymbol*)name;
  ptr_table* r = &(pred->pos_prop);
  for (index_type k = 0; k < param.length(); k++) {
    if (param[k]->sym_class == sym_variable) {
      if (((VariableSymbol*)param[k])->value == 0) {
	std::cerr << "error: unbound variable "
...
void PDDL_Base::Context::add_parameters_for_atom
(Atom* a, variable_vec& p_context)
{
  for (index_type k = 0; k < a->param.length(); k++)
    if (a->param[k]->sym_class == sym_variable) {
      index_type i = param.first((VariableSymbol*)(a->param[k]));
      if (i == no_such_index) {
	i = p_context.first((VariableSymbol*)(a->param[k]));
	if (i == no_such_index) {
	  param.append((VariableSymbol*)(a->param[k]));
	}
      }
    }
}
...
(Instance& ins, index_set& s, index_type p)
{
  if (p < param.length()) {
	...
	Instance::atom_ref p = pos_atoms[k]->find_prop(ins, false, true);
	s.insert(p->index);
      }
    for (index_type k = 0; k < neg_atoms.length(); k++)
      if (!neg_atoms[k]->pred->is_equality()) {
	Instance::atom_ref p = neg_atoms[k]->find_prop(ins, true, true);
	s.insert(p->index);
      }
  }
}
...
(Instance& ins, index_set& s_add, index_set& s_del, index_type p)
{
  if (p < param.length()) {
	...
	Instance::atom_ref np = pos_atoms[k]->find_prop(ins, true, true);
	s_del.insert(np->index);
      }
    }
    for (index_type k = 0; k < neg_atoms.length(); k++) {
      Instance::atom_ref p = neg_atoms[k]->find_prop(ins, false, true);
      s_del.insert(p->index);
      if (neg_atoms[k]->pred->neg_pre) {
	Instance::atom_ref np = neg_atoms[k]->find_prop(ins, true, true);
...
(Instance& ins, rule_set& s_pos, rule_set& s_neg, index_type p)
{
  if (p < param.length()) {
    ...
    if (write_trace) {
      std::cerr << "build_conditional: ";
      print(std::cerr);
      std::cerr << " with";
      for (index_type k = 0; k < param.length(); k++)
	std::cerr << " " << param[k]->print_name << "="
		  << param[k]->value->print_name;
      std::cerr << std::endl;
    }
    index_set ant;
    for (index_type k = 0; k < pos_con.length(); k++)
      if (!pos_con[k]->is_static()) {
	Instance::atom_ref p = pos_con[k]->find_prop(ins, false, true);
	ant.insert(p->index);
	if (write_trace) {
	  std::cerr << "added " << p->index << "." << p->name
		    << " (+) to antecendent" << std::endl;
	}
	Instance::atom_ref neg_p = pos_con[k]->find_prop(ins, true, true);
      }
    for (index_type k = 0; k < neg_con.length(); k++)
      if (!neg_con[k]->is_static()) {
	Instance::atom_ref p = neg_con[k]->find_prop(ins, false, true);
	Instance::atom_ref neg_p = neg_con[k]->find_prop(ins, true, true);
	ant.insert(neg_p->index);
	if (write_trace) {
	  std::cerr << "added " << neg_p->index << "." << neg_p->name
		    << " (-) to antecendent" << std::endl;
	}
      }
    for (index_type k = 0; k < pos_atoms.length(); k++) {
      Instance::atom_ref q = pos_atoms[k]->find_prop(ins, false, true);
      rule& r = s_pos.append();
      r.antecedent = ant;
      r.consequent = q->index;
      if (pos_atoms[k]->pred->neg_pre) {
	Instance::atom_ref not_q = pos_atoms[k]->find_prop(ins, true, true);
	rule& r = s_neg.append();
	r.antecedent = ant;
	r.consequent = not_q->index;
      }
    }
    for (index_type k = 0; k < neg_atoms.length(); k++) {
      Instance::atom_ref q = neg_atoms[k]->find_prop(ins, false, true);
      rule& r = s_neg.append();
      r.antecedent = ant;
      r.consequent = q->index;
      if (neg_atoms[k]->pred->pos_pre) {
	Instance::atom_ref not_q = neg_atoms[k]->find_prop(ins, true, true);
	rule& r = s_pos.append();
	r.antecedent = ant;
	r.consequent = not_q->index;
      }
...
//   else if (is_true()) {
//     for (index_type k = 0; k < pos_atoms.length(); k++) {
//       Instance::atom_ref a = pos_atoms[k]->find_prop(ins, false, true);
//       s.insert(a->index);
//     }
//     for (index_type k = 0; k < neg_atoms.length(); k++) {
//       Instance::atom_ref a = neg_atoms[k]->find_prop(ins, true, true);
//       s.insert(a->index);
//     }
//   }
// }
...
bool PDDL_Base::instantiate_atom_set
(Instance& ins, atom_vec& a, index_set& s)
{
  for (index_type k = 0; k < a.length(); k++) {
    Instance::atom_ref p = a[k]->find_prop(ins, false, true);
    if (p)
      s.insert(p->index);
    else
      return false;
...
bool PDDL_Base::instantiate_atom_set
(Instance& ins, signed_atom_vec& a, index_set& s)
{
  for (index_type k = 0; k < a.length(); k++) {
    Instance::atom_ref p = a[k].second->find_prop(ins, a[k].first, true);
    if (p)
      s.insert(p->index);
    else
      return false;
...
void PDDL_Base::Goal::instantiate(Instance& ins, NTYPE deadline)
{
  if (g_class == goal_pos_atom) {
    Instance::atom_ref p = ((AtomicGoal*)this)->atom->find_prop(ins, false, true);
    p->goal = true;
    p->goal_t = deadline;
  }
  else if (g_class == goal_neg_atom) {
    Instance::atom_ref p = ((AtomicGoal*)this)->atom->find_prop(ins, true, true);
  ...
  else if ((g_class == goal_disjunction) && compile_away_disjunctive_goals) {
    index_set d_set;
    bool is_d = true;
    bool ok = instantiate_as_atom_set(ins, d_set, is_d);
    if (ok && is_d) {
      mapping map(ins.n_actions());
      ins.enforce_disjunctive_goal(d_set, NULL, map);
    }
    else if ((warning_level > 0) || !best_effort) {
      std::cerr << "warning: compilation of ";
      print(std::cerr);
      std::cerr << " not implemented - goal ignored" << std::endl;
      if (!best_effort) exit(1);
    }
  }
    ...
    index_set a;
    bool a_is_d = false;
    bool ok = ((SimpleSequenceGoal*)this)->constraint->instantiate_as_atom_set(ins, a, a_is_d);
    if (ok) {
      mapping map(ins.n_actions());
      if (a_is_d)
	ins.enforce_pc_always_disjunction(a, 0, map);
      else
	ins.enforce_pc_always_conjunction(a, 0, map);
    }
    else if ((warning_level > 0) || !best_effort) {
      std::cerr << "warning: compilation of ";
      print(std::cerr);
    ...
    if (ok) {
      if (a_is_d) {
	ins.enforce_pc_sometime_disjunction(a, 0);
      }
      else {
	mapping map(ins.n_actions());
	ins.enforce_pc_sometime_conjunction(a, 0, map);
      }
    }
    else if ((warning_level > 0) || !best_effort) {
      std::cerr << "warning: compilation of ";
    ...
    if (ok) {
      if (a_is_d) {
	index_type g = ins.compile_pc_at_most_once_disjunction(a, 0);
	ins.atoms[g].goal = true;
      }
      else {
	mapping map(ins.n_actions());
	ins.enforce_pc_at_most_once_conjunction(a, 0, map);
      }
    }
    else if ((warning_level > 0) || !best_effort) {
      std::cerr << "warning: compilation of ";
    ...
    index_set a;
    bool a_is_d = false;
    bool ok = ((TriggeredSequenceGoal*)this)->trigger->instantiate_as_atom_set(ins, a, a_is_d);
    index_set b;
    bool b_is_d = false;
    ok = (ok && ((TriggeredSequenceGoal*)this)->constraint->instantiate_as_atom_set(ins, b, b_is_d));
    if (ok && !a_is_d && !b_is_d) {
      ins.compile_pc_sometime_before_cc(a, b, 0);
    }
    else if ((warning_level > 0) || !best_effort) {
      std::cerr << "warning: compilation of ";
      print(std::cerr);
...
(Instance& ins, index_set& set, Symbol* p, index_type i)
{
  if (g_class == goal_pos_atom) {
    ...
    Name* n = (p ? (i != no_such_index ?
	       ...
	       (Name*)0);
    set.insert(ins.compile_pc_always_conjunction(a, n));
  }
  else if ((g_class == goal_sometime) && compile_away_plan_constraints) {
    index_set a;
    ((SimpleSequenceGoal*)this)->constraint->instantiate(ins, a, p, i);
    Name* n = (p ? (i != no_such_index ?
		    ...
		    (Name*)new StringName(p->print_name)) :
	       (Name*)0);
    set.insert(ins.compile_pc_sometime_conjunction(a, n));
  }
  else if ((g_class == goal_at_most_once) && compile_away_plan_constraints) {
    index_set a;
    ((SimpleSequenceGoal*)this)->constraint->instantiate(ins, a, p, i);
    Name* n = (p ? (i != no_such_index ?
		    ...
		    (Name*)new StringName(p->print_name)) :
	       (Name*)0);
    set.insert(ins.compile_pc_at_most_once_conjunction(a, n));
  }
  else if ((g_class == goal_sometime_before) && compile_away_plan_constraints) {
    index_set a;
    ...
    Name* n = (p ? (i != no_such_index ?
		    ...
		    (Name*)new StringName(p->print_name)) :
	       (Name*)0);
    set.insert(ins.compile_pc_sometime_before_cc(a, b, n));
  }
  else if ((warning_level > 0) || !best_effort) {
    std::cerr << "warning: goal ";
    print(std::cerr);
...
bool PDDL_Base::Goal::instantiate_as_atom_set
(Instance& ins, index_set& set, bool& is_dis)
{
  if (g_class == goal_pos_atom) {
    Instance::atom_ref p = ((AtomicGoal*)this)->atom->find_prop(ins, false, true);
    set.insert(p->index);
    ...
    is_dis = false;
    for (index_type k = 0; k < g->goals.length(); k++) {
      bool ok = g->goals[k]->instantiate_as_atom_set(ins, set, is_dis);
      if (!ok || is_dis) return false;
    }
    return true;
  }
  else if (g_class == goal_disjunction) {
    DisjunctiveGoal* g = (DisjunctiveGoal*)this;
    is_dis = true;
    for (index_type k = 0; k < g->goals.length(); k++) {
      bool ok = g->goals[k]->instantiate_as_atom_set(ins, set, is_dis);
      if (!ok || !is_dis) return false;
...
#endif // BUILD_WITH_SOFT

void PDDL_Base::Preference::unquantify
(index_type p, name_vec& nv, preference_vec& pv)
{
  if (p < param.size()) {
    for (index_type k = 0; k < param[p]->sym_types.n_elements(); k++) {
      param[p]->value = param[p]->sym_types.get_element(k);
      unquantify(p + 1, nv, pv);
    }
    param[p]->value = 0;    
  }
  else {
    PDDL_Name* pname = new PDDL_Name(name, param, false);
    nv.append(pname);
    Goal* pgoal = goal->instantiate_partially();
    pv.append(new Preference(name, pgoal));
  }
}
...
(index_type i, Instance& ins, const index_vec& map, Plan& p)
{
  ...
  p.add_trait(new SourceFileName(input_plans[i]->src));
  return true;
}
...
(const name_vec& names, index_set_vec& sets, name_vec* snames, index_type p)
{
  if (p < param.length()) {
    for (index_type k = 0; k < param[p]->sym_types.n_elements(); k++) {
      param[p]->value = param[p]->sym_types.get_element(k);
      if (Context::partial_eval() != p_false) {
	build(names, sets, snames, p + 1);
      }
    }
    param[p]->value = 0;
  }
    ...
    if (all_ok) {
      ...
      if (snames) {
	if (name) {
	  PDDL_Name* n = new PDDL_Name(name, param, false);
	  snames->append(n);
	}
	else {
	  snames->append(0);
...
bool PDDL_Base::SimpleReferenceSet::build_atom_sequence(index_vec& vec)
{
  index_type m = ref->find_atom();
  if (m != no_such_index) {
    if (write_info) {
      std::cerr << "info: reference ";
      print(std::cerr);
      std::cerr << " with binding ";
      print_assignment(std::cerr);
      std::cerr << " matched to atom #" << m << std::endl;
    }
    vec.append(m);
    return true;
  }
...
void PDDL_Base::ReferenceSet::build_atom_sequences
(lvector<index_vec>& seqs, name_vec* snames, index_type p)
{
  if (p < param.length()) {
    for (index_type k = 0; k < param[p]->sym_types.n_elements(); k++) {
      param[p]->value = param[p]->sym_types.get_element(k);
      if (Context::partial_eval() != p_false) {
	build_atom_sequences(seqs, snames, p + 1);
      }
    }
    param[p]->value = 0;
  }
    ...
    bool all_ok = true;
    for (index_type k = 0; (k < refs.length()) && all_ok; k++) {
      // all refs in the sequence must be ground!
      assert(refs[k]->param.length() == 0);
      bool ok = refs[k]->build_atom_sequence(v);
      if (!ok && (warning_level > 0)) {
	std::cerr << "warning: failed to build instance ";
	print_assignment(std::cerr);
	std::cerr << " of ";
	print(std::cerr);
	std::cerr << " (as sequence)" << std::endl;
      }
      all_ok = (all_ok && ok);
    }
    if (all_ok) {
...
void PDDL_Base::export_sets(const name_vec& names, index_set_vec& sets)
{
  for (index_type k = 0; k < input_sets.length(); k++) {
    input_sets[k]->build(names, sets, NULL, 0);
  }
}

void PDDL_Base::export_sets
(const name_vec& names, index_set_vec& sets, name_vec& snames)
{
  for (index_type k = 0; k < input_sets.length(); k++) {
    input_sets[k]->build(names, sets, &snames, 0);
  }
}

void PDDL_Base::export_atom_sequences
(lvector<index_vec>& seqs, name_vec& snames)
{
  for (index_type k = 0; k < input_sets.length(); k++) {
    input_sets[k]->build_atom_sequences(seqs, &snames, 0);
  }
}

void PDDL_Base::export_generators
(const Instance& ins, lvector<mapping>& maps)
{
  lvector<index_vec> seqs;
  name_vec names;
  export_atom_sequences(seqs, names);
  assert(seqs.size() == names.size());
  bool_vec rem(true, seqs.size());
  index_type next = rem.first(true);
  while (next != no_such_index) {
    assert(next < seqs.size());
    mapping m(ins.n_atoms());
    for (index_type k = 0; k < seqs.size(); k++)
      if (names[k]->equals(names[next])) {
	for (index_type i = 0; (i + 1) < seqs[k].size(); i++)
	  m[seqs[k][i]] = seqs[k][i + 1];
	m[seqs[k][seqs[k].size() - 1]] = seqs[k][0];
	rem[k] = false;
      }
      else {
	assert(k != next);
...
void PDDL_Base::ActionSymbol::collect_objects(symbol_vec& v)
{
  for (index_type i = 0; i < pos_pre.length(); i++)
    pos_pre[i]->collect_objects(v);
  for (index_type i = 0; i < neg_pre.length(); i++)
    neg_pre[i]->collect_objects(v);
  for (index_type i = 0; i < adds.length(); i++)
    adds[i]->collect_objects(v);
  for (index_type i = 0; i < dels.length(); i++)
    dels[i]->collect_objects(v);
  for (index_type i = 0; i < locks.length(); i++)
    locks[i]->collect_objects(v);
  for (index_type i = 0; i < enables.length(); i++)
    enables[i]->collect_objects(v);
  for (index_type i = 0; i < reqs.length(); i++)
    reqs[i]->collect_objects(v);
  for (index_type i = 0; i < incs.length(); i++)
    incs[i]->collect_objects(v);
  for (index_type i = 0; i < decs.length(); i++)
    decs[i]->collect_objects(v);
  for (index_type i = 0; i < fass.length(); i++)
    fass[i]->collect_objects(v);
  if (dmin)
    dmin->collect_objects(v);
  if (dmax)
    dmax->collect_objects(v);
}
...
PDDL_Base::Goal* PDDL_Base::Goal::instantiate_partially()
{
  switch (g_class) {
  case goal_pos_atom:
    return new AtomicGoal(((AtomicGoal*)this)->atom->instantiate_partially(),
			  false);
  case goal_neg_atom:
    return new AtomicGoal(((AtomicGoal*)this)->atom->instantiate_partially(),
			  true);
  case goal_relation:
    return
      new NumericGoal(new Relation(((NumericGoal*)this)->rel->rel,
				   ((NumericGoal*)this)->rel->at,
				   ((NumericGoal*)this)->rel->first
				   ->instantiate_partially(),
				   ((NumericGoal*)this)->rel->second
				   ->instantiate_partially()));
  case goal_conjunction:
    {
      ConjunctiveGoal* g0 = ((ConjunctiveGoal*)this);
      ConjunctiveGoal* g1 = new ConjunctiveGoal;
      for (index_type k = 0; k < g0->goals.size(); k++) {
	g1->goals.append(g0->goals[k]->instantiate_partially());
      }
      return g1;
    }
  case goal_disjunction:
    {
      DisjunctiveGoal* g0 = ((DisjunctiveGoal*)this);
      DisjunctiveGoal* g1 = new DisjunctiveGoal;
      for (index_type k = 0; k < g0->goals.size(); k++) {
	g1->goals.append(g0->goals[k]->instantiate_partially());
      }
      return g1;
  ...
  case goal_exists:
    {
      QuantifiedGoal* g0 = ((QuantifiedGoal*)this);
      QuantifiedGoal* g1 = new QuantifiedGoal(g0->g_class);
      g1->param = g0->param;
      g1->goal = g0->goal->instantiate_partially();
      return g1;
    }
  case goal_task:
    assert(0);
  ...
  case goal_at_most_once:
    return
      new SimpleSequenceGoal(((SimpleSequenceGoal*)this)->g_class,
			     ((SimpleSequenceGoal*)this)->constraint
			     ->instantiate_partially());
  case goal_within:
    return
      new DeadlineGoal(((DeadlineGoal*)this)->at,
		       ((DeadlineGoal*)this)->goal
		       ->instantiate_partially());
  case goal_always_within:
    return
      new TriggeredDeadlineGoal(((TriggeredDeadlineGoal*)this)->trigger
				->instantiate_partially(),
				((TriggeredDeadlineGoal*)this)->delay,
				((TriggeredDeadlineGoal*)this)->goal
				->instantiate_partially());
  case goal_sometime_before:
  case goal_sometime_after:
    return
      new TriggeredSequenceGoal(((TriggeredSequenceGoal*)this)->g_class,
				((TriggeredSequenceGoal*)this)->trigger
				->instantiate_partially(),
				((TriggeredSequenceGoal*)this)->constraint
				->instantiate_partially());
  default:
    std::cerr << "error: invalid goal class (" << g_class << ")" << std::endl;
    exit(255);
...
void PDDL_Base::Axiom::print(std::ostream& s)
{
  s << "(:derived " << std::endl;
  PDDL_Base::Context::print(s);
  s << "  :head ";
...
void PDDL_Base::print(std::ostream& s)
{
  ...
  for (index_type k = 0; k < dom_types.length(); k++)
    dom_types[k]->print(s);
  for (index_type k = 0; k < dom_predicates.length(); k++)
    dom_predicates[k]->print(s);
  for (index_type k = 0; k < dom_object_functions.length(); k++)
    dom_object_functions[k]->print(s);
  for (index_type k = 0; k < dom_functions.length(); k++)
    dom_functions[k]->print(s);
  for (index_type k = 0; k < dom_actions.length(); k++)
    dom_actions[k]->print(s);
  for (index_type k = 0; k < dom_axioms.length(); k++)
    dom_axioms[k]->print(s);
...
void PDDL_Base::write_set_precondition(std::ostream& s, SetOf* set)
{
  ...
  if (set->pos_con.length() +
      set->neg_con.length() +
      set->type_con.length() > 0) {
    s << " (imply";
    if (set->pos_con.length() +
	set->neg_con.length() +
	set->type_con.length() > 1)
      s << " (and";
  ...
  if (set->pos_con.length() +
      ...
      set->type_con.length() > 0)
    s << ")"; // end of (imply...
  if (set->param.length() > 0) s << ")"; // end of (forall...
  if (Instance::write_PDDL2) {
    switch (m) {
    case md_start:
    case md_end:
...
{
  bool is_durative = (Instance::write_PDDL2 && Instance::write_time &&
		      (act->dmin || act->dmax));
  ...
  index_type n_eff = (act->adds.length() + act->dels.length() +
		      act->set_eff.length() + act->cond_eff.length());
  if (Instance::write_PDDL2 || Instance::write_time || Instance::write_locks)
    n_eff += (2 * act->locks.length());
  if (Instance::write_PDDL2 || Instance::write_time)
    n_eff += ((2 * act->enables.length()) + (2 * act->reqs.length()) +
	      act->incs.length() + act->decs.length() + act->fass.length() +
	      act->qc_fass.length());
  ...
  if (n_eff > 0) {
    ...
    for (index_type i = 0; i < act->adds.length(); i++) {
      s << " ";
      // if (Instance::write_time && is_durative) {
      // 	switch (act->adds[i]->at) {
      // 	case md_start:
      // 	  s << "(at start ";
      // 	  break;
      // 	default:
    ...
    else if (Instance::write_locks) {
      for (index_type i = 0; i < act->locks.length(); i++) {
	s << " (not (" << act->locks[i]->pred->print_name;
	((PDDL_Base::AtomBase*)act->locks[i])->print(s);
	s << "))";
	s << " (" << act->locks[i]->pred->print_name;
	((PDDL_Base::AtomBase*)act->locks[i])->print(s);
	s << ")";
      }
    }
...
void PDDL_Base::write_init(std::ostream& s)
{
  ...
  if (Instance::write_PDDL2) {
    n_init += dom_fun_init.length();
    n_init += dom_pos_til.length();
    n_init += dom_neg_til.length();
  }
  ...
  if (n_init > 0) {
	...
	s << ") " << PRINT_NTYPE(dom_fun_init[k]->val) << ")";
      ...
      if (Instance::write_time) {
	for (index_type k = 0; k < dom_pos_til.length(); k++) {
	  s << " (at " << PRINT_NTYPE(dom_pos_til[k]->at_time)
	    << " (" << dom_pos_til[k]->pred->print_name;
	  ((AtomBase*)dom_pos_til[k])->print(s);
	  s << "))";
	}
	for (index_type k = 0; k < dom_neg_til.length(); k++) {
	  s << " (at " << PRINT_NTYPE(dom_neg_til[k]->at_time)
	    << " (not (" << dom_neg_til[k]->pred->print_name;
	  ((AtomBase*)dom_neg_til[k])->print(s);
	  s << ")))";
	}
      }
    }
...
bool PDDL_Name::silly_cap = false;
...
void PDDL_Name::write(std::ostream& s, unsigned int c) const
{
  if (context_is_instance(c)) {
    ...
    else if (silly_cap && neg) {
      char* tmp = uppercase_map.strdup_initial(sym->print_name);
      write_string_escaped(s, tmp, c);
      delete tmp;
    }
    else {
	...
	char* tmp = uppercase_map.strdup_initial(arg[k]->print_name);
	write_string_escaped(s, tmp, c);
	delete tmp;
      }
      else {
...
void PDDL_Name::write_for_downward(std::ostream& s) const
{
  s << sym->print_name;
  s << ' ' << arg.length();
  for (index_type k = 0; k < arg.length(); k++)
...
void PDDL_Name::write_FOL(std::ostream& s) const
{
  assert(!neg);
  s << sym->print_name;
  s << '(';
...
void PDDL_Name::write_without_parentheses(std::ostream& s) const
{
  s << sym->print_name;
  for (index_type k = 0; k < arg.length(); k++)
    s << ' ' << arg[k]->print_name;
...
const PlanTrait* SourceFileName::cast_to(const char* class_name) const
{
  if (strcmp(class_name, "SourceFileName") == 0)
    return this;
  else
    return 0;
}
