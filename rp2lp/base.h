...
class PDDL_Base {
 public:
  ...
  static bool     compile_away_disjunctive_goals;
  static bool     compile_away_object_functions;
  static bool     make_types_from_static_predicates;
  ...
  typedef lvector<TypeSymbol*> simple_type_vec;
  typedef lvector<Atom*> atom_vec;
  typedef std::pair<bool, Atom*> signed_atom;
  typedef lvector<signed_atom> signed_atom_vec;
  ...
  struct TypeSet : public simple_type_vec {
    TypeSet() : simple_type_vec(0, 0) { };
    TypeSet(const TypeSet& s) : simple_type_vec(s) { };
    TypeSet(TypeSymbol* t) : simple_type_vec(t, 1) { };
  ...
  typedef lvector<TypeSet> type_vec;
  ...
  struct Symbol {
    ...
    bool         visible;
    Symbol(symbol_class c, const char* n) : sym_class(c), print_name(n),
      defined_in_problem(false), visible(true) { };
    Symbol(char* n) : sym_class(sym_object), print_name(n),
      defined_in_problem(false), visible(true) { };
    void print(std::ostream& s) const;
  };
  ...
  typedef lvector<symbol_set> symbol_set_vec;
  ...
  struct TypeSymbol : public Symbol {
    symbol_vec  elements;
    bool        is_base_type;
    TypeSymbol(const char* n) : Symbol(sym_typename, n), elements(0, 0),
      is_base_type(false) { };
  ...
  struct VariableSymbol : public Symbol {
    Symbol*  value;
    FTerm*   binding;
    VariableSymbol(const char* n) : Symbol(sym_variable, n),
      value(0), binding(0) {};
  ...
  struct ParamSymbol : public Symbol {
    variable_vec param;
    ParamSymbol(symbol_class c, const char* n) : Symbol(c, n), param(0, 0) { };
    ListExpression* param_list();
    void print(std::ostream& s);
    void print_with_bindings(std::ostream& s);
  };
  ...
  struct PredicateSymbol : public ParamSymbol {
    ...
    PredicateSymbol(const char* n) : ParamSymbol(sym_predicate, n),
  ...
  struct FunctionSymbol : public ParamSymbol {
    ...
    bool  conditioned; // does the function appear in any condition exp?
    bool  increased;
    bool  decreased;
    bool  assigned;
    bool  borrowed;
    ptr_table  init;
    FunctionSymbol(const char* n) : ParamSymbol(sym_function, n),
  ...
  typedef lvector<FunctionSymbol*> function_vec;
  ...
  struct AtomBase {
    ...
    // make args of b from this:
    void fill_in_args(AtomBase* b);
    // make args of this from v:
    void fill_in_args(variable_vec& v, bool as_value);
    void collect_bound_variables(variable_vec& v);
    void collect_objects(symbol_vec& v);
  ...
  struct Atom : AtomBase {
    ...
    Atom(PredicateSymbol* p, AtomBase* b) : AtomBase(b), pred(p) { };
    ...
    bool check();
    Instance::atom_ref find_prop(Instance& ins, bool neg, bool create);
  ...
  struct FInitAtom : AtomBase {
    ...
    FInitAtom(FunctionSymbol* f, variable_vec& a, bool as_value, NTYPE v)
      : fun(f), val(v)
    {
      fill_in_args(a, as_value);
    };
  ...
  struct Expression {
    ...
    Expression* copy();
    Expression* simplify();
    Expression* instantiate_partially();
    void collect_constants(exp_vec& c);
    void collect_functions(exp_vec& c);
    void collect_variables(variable_vec& v);
    void collect_objects(symbol_vec& v);
  ...
  struct FChangeAtom : AtomBase {
    ...
    bool is_defined();
    Instance::resource_ref find_resource(Instance& ins);
    void print(std::ostream& s);
  };
  ...
  struct Context {
    ...
    // for each argument of atom a that is a variable and does not
    // appear in parameters of this context or in the parent context
    // (second argument vector), add that variable to params
    void add_parameters_for_atom(Atom* a, variable_vec& p_context);
  ...
  struct Axiom : public Context {
    Atom* head;
    Formula* body;
    Axiom(Atom* h, Formula* b) : head(h), body(b) { };
    void print(std::ostream& s);
  };

  typedef lvector<Axiom*> axiom_vec;
  ...
  struct DKEL_Item : public Context {
    ...
    bool       defined_in_problem;
    bool       included;
    DKEL_Item(const char* name) : item_name(name), name(0), 
  ...
  struct SetOf : public Context {
    ...
    bool is_static() const;
    index_type n_atoms() const
    { return (pos_atoms.length() + neg_atoms.length()); };
    void set_mode(mode_keyword m);
...
//  };
  ...
  struct SetSymbol : public Symbol {
    ...
    name_vec   names;
    SetSymbol(const char* n) : Symbol(sym_set, n), sets(0, 0), names(0, 0) { };
  };
  ...
  struct ActionSymbol : public ParamSymbol {
    ...
    ActionSymbol(const char* n) : ParamSymbol(sym_action, n),
      pos_pre(0, 0), neg_pre(0, 0), set_pre(0, 0), dis_pre(0, 0),
      num_pre(0, 0), adds(0, 0), dels(0, 0), set_eff(0, 0), cons(0, 0),
      cond_eff(0, 0), locks(0, 0), enables(0, 0), reqs(0, 0),
      incs(0, 0), decs(0, 0), fass(0, 0), qc_incs(0, 0), qc_decs(0, 0),
      qc_fass(0, 0), dmin(0), dmax(0), irr_ins(0, 0), refs(0, 0),
      exps(0, 0), part(0), assoc(0) { };
  ...
  typedef lvector<ActionSymbol*> action_vec;
  ...
  struct InputPlan {
    ...
    InputPlan() : name(0), is_opt(false), src(0), steps(0, 0) { };
  ...
  struct SimpleReferenceSet : public Context {
    ...
    bool build_atom_sequence(index_vec& vec);
    void print(std::ostream& s);
  };
  ...
  struct ReferenceSet : public Context {
    ...
    void build_atom_sequences(lvector<index_vec>& seqs,
			      name_vec* snames,
			      index_type p);
    void build(const name_vec& names,
	       index_set_vec& sets,
	       name_vec* snames,
	       index_type p);
    void print(std::ostream& s);
  ...
  struct CPG {
    ...
    symbol_vec args;
    type_vec arg_types;
    type_vec param_types;

    predicate_vec arg_range_pred;
    symbol_set_vec arg_range_set;

    CPG() : atoms(0, 0), neg(false, 0), args(0, 0) { };
    CPG(CPG& g, index_vec& s);
    void make_key(ptr_table::key_vec& key);
    void make_typed_key(ptr_table::key_vec& key);
    void make_key_no_negation(ptr_table::key_vec& key);
    ...
    // internals
    void compute_asserting_effects
    (index_type c_atom, ActionSymbol* act, index_type c_eff, bool_vec& sat,
     PredicateSymbol* p, bool p_val, PredicateSymbol* g, FunctionSymbol* f,
     Expression* f_val, symbol_pair_vec& eq, symbol_pair_vec& neq,
     atom_vec& pg, atom_vec& ng, bool strict, atom_set_vec& pe,
     qc_numeric_effect_vec& fe);

    void asserting_effects_satisfying_case
    (Atom* e_atom, index_type c_atom, ActionSymbol* act, index_type c_eff,
     bool_vec& sat, PredicateSymbol* p, bool p_val, PredicateSymbol* g,
     FunctionSymbol* f, Expression* f_val, symbol_pair_vec& eq,
     symbol_pair_vec& neq, atom_vec& pg, atom_vec& ng, bool strict,
     atom_set_vec& pe, qc_numeric_effect_vec& fe);

    void asserting_effects_falsifying_case
    (Atom* e_atom, index_type c_atom, ActionSymbol* act, index_type c_eff,
     bool_vec& sat, PredicateSymbol* p, bool p_val, PredicateSymbol* g,
     FunctionSymbol* f, Expression* f_val, symbol_pair_vec& eq,
     symbol_pair_vec& neq, atom_vec& pg, atom_vec& ng, bool strict,
    ...
    void asserting_effects_conditional_satisfying_case
    (Atom* e_atom, index_type c_atom, ActionSymbol* act, index_type c_eff,
     bool_vec& sat, atom_vec& pos_con, atom_vec& neg_con, PredicateSymbol* p,
     bool p_val, PredicateSymbol* g, FunctionSymbol* f, Expression* f_val,
     symbol_pair_vec& eq, symbol_pair_vec& neq, atom_vec& pg, atom_vec& ng,
     bool strict, atom_set_vec& pe, qc_numeric_effect_vec& fe);

    void asserting_effects_conditional_falsifying_case
    (Atom* e_atom, index_type c_atom, ActionSymbol* act, index_type c_eff,
     bool_vec& sat, atom_vec& pos_con, atom_vec& neg_con, PredicateSymbol* p,
     bool p_val, PredicateSymbol* g, FunctionSymbol* f, Expression* f_val,
     symbol_pair_vec& eq, symbol_pair_vec& neq, atom_vec& pg, atom_vec& ng,
    ...
    void add_fluent_effect(ActionSymbol* act,
			   ...
			   index_type c_atom, Atom* a_eff);
    void make_effect_conditions
    (Context* e, variable_vec& act_param, ParamSymbol* pf, bool_vec& sat,
     symbol_vec& subs, symbol_pair_vec& eq, symbol_pair_vec& neq,
     atom_vec& pg, atom_vec& ng);
  };
  ...
  struct Goal {
    ...
    // instantiate_as_atom_set is side-effec-free (i.e. does not change the
    // instance); returns false (but issues no warning/error) if the goal is
    // not a simple conjunction or disjunction of literals
    bool instantiate_as_atom_set(Instance& ins, index_set& set, bool& is_dis);
    Goal* instantiate_partially();
    void print(std::ostream& s);
  };
  ...
  struct  Preference; // forward declare preference_vec
  typedef lvector<Preference*> preference_vec;

  struct Preference : public Context {
    Symbol* name;
    Goal*   goal;
    ...
    void unquantify(index_type p, name_vec& nv, preference_vec& pv);
#ifdef BUILD_WITH_SOFT
  ...
  simple_type_vec    dom_types;
  simple_type_vec    dom_base_types;
  ...
  atom_vec      dom_init;
  atom_vec      dom_pos_til;
  atom_vec      dom_neg_til;
  ...
  FInitAtom* find_function_init(const char* fname, const symbol_vec& arg);
  bool  find_initial_fact(const char* pname, const symbol_vec& arg);
  // note: find_function_value assumes function is static
  ...
  void find_elements_satisfying(const symbol_vec& elements,
				...
				index_type element_arg_p,
				index_set& sats);
  signed_atom goal_to_atom(Goal* g);
  bool goal_to_atom_vec(Goal* g, signed_atom_vec& av, bool& is_disjunction);
  ...
  void set_type_type(simple_type_vec& vec, TypeSymbol* t);
  // obsolete // void set_constant_type(symbol_vec& vec, TypeSymbol* t);
  ...
  // last argument receives the result:
  void make_parameters(type_vec& t, const char* prefix, variable_vec& v);
  void make_parameters(variable_vec& p, const char* prefix, variable_vec& v);

  Atom* make_atom_from_prop(ptr_pair& src, bool& neg);
  void arg_list_to_vec(ListExpression* l, symbol_vec& v);
  ListExpression* arg_vec_to_list(variable_vec& v, bool as_value);

  void add_init_atom(Atom* a);
  void add_init_fun(FInitAtom* a);
  void add_goal_atom(Atom* a, bool neg);
  ...
  // NOTE: implementation of make_types is incorrect - don't use!!
  void make_types();
  ...
  bool instantiate_atom_set(Instance& ins, atom_vec& a, index_set& s);
  bool instantiate_atom_set(Instance& ins, signed_atom_vec& a, index_set& s);
  ...
  void export_sets(const name_vec& names,
		   index_set_vec& sets, name_vec& snames);
  void export_atom_sequences(lvector<index_vec>& seqs, name_vec& snames);
  void export_generators(const Instance& ins, lvector<mapping>& maps);
  ...
  void expand_quantified_preferences();

  void compile_preferences_1();
  void compile_preferences_2(bool ignore_tc_fun, bool use_shared_pred);
  ...
  void compile_negations();

  void compile_object_functions();
  void detil(bool force, NTYPE dur, action_vec& ea, cost_vec& tp);
  void compile_fluent_defined_check();
  void compile_static_fluent_expressions();
  ...
  Goal* compile_constraint_2(Goal* g, const Symbol* n,
			     ptr_table& acs, ptr_table& ecs,
			     ptr_table& ocs, ptr_table& bcs);
  Goal* compile_always_constraint(SimpleSequenceGoal* g,
				  const Symbol* n,
				  ptr_table& cs);
  Goal* compile_sometime_constraint(SimpleSequenceGoal* g,
				    const Symbol* n,
				    ptr_table& cs);
  ...
  void compile_set_conditions_and_effects(ActionSymbol* act,
					  ...
					  index_type p);
  void make_complementary_init(PredicateSymbol* pred,
			       PredicateSymbol* cpred,
			       Atom* a, index_type p);
  ...
  Expression* compile_static_fluent_expression(Expression* exp, index_type n);
  void init_static_fluent_expression(variable_vec& vs,
				     index_type p,
				     Expression* exp,
				     FunctionSymbol* f);

  bool quantified_atom_to_atom_vec(bool is_dis,
				   variable_vec& param,
				   index_type p,
				   atom_vec& pos_con,
				   atom_vec& neg_con,
...
class PDDL_Name : public Name {
 ...
 public:
  ...
  virtual ~PDDL_Name() { };
  virtual void write(std::ostream& s, unsigned int c) const;
  void write_for_downward(std::ostream& s) const;
  void write_FOL(std::ostream& s) const;
  void write_without_parentheses(std::ostream& s) const;
  virtual const Name* cast_to(const char* cname) const;
};
...
class SourceFileName : public PlanTrait {
  const char* the_file_name;
 public:
  SourceFileName(const char* fname) : the_file_name(fname) { };
  const char* source_file_name() { return the_file_name; };
