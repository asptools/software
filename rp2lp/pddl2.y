...
%token KW_REQS KW_CONSTANTS KW_PREDS KW_FUNS KW_TYPES KW_DEFINE KW_DOMAIN
       KW_ACTION KW_PROCESS KW_EVENT KW_ARGS KW_PRE KW_COND
       ...
       KW_TRUE KW_NUMBER KW_UNDEFINED KW_MINOP KW_MAXOP KW_DERIVED
...
%type <goal>  new_goal new_single_goal new_atomic_goal
...
| domain_elements domain_functions
| domain_elements domain_structure
| domain_elements reference_set
| domain_elements goal_spec
| /* empty */
;
...
{
  if (warning_level > 0) {
    std::cerr << "warning: redeclaration of action " << $1->text << std::endl;
...
typed_type_list primitive_type_list TK_HYPHEN TK_TYPE_SYMBOL
{
  ...
  for (HSPS::index_type k = 0; k < current_type_set.length(); k++) {
    if (current_type_set[k] == (TypeSymbol*)$4->val) {
	...
	current_type_set[k]->sym_types[0] = ((TypeSymbol*)$4->val);
      else 
	current_type_set[k]->sym_types.append((TypeSymbol*)$4->val);
    }
  }
  current_type_set.clear();
}
...
domain_constants:
TK_OPEN KW_CONSTANTS
{
  current_constant_decl.clear();
}
typed_constant_list TK_CLOSE
| TK_OPEN KW_OBJECTS
{
  current_constant_decl.clear();
}
...
ne_constant_sym_list TK_NEW_SYMBOL
{
  ...
  current_constant_decl.append((Symbol*)$2->val);
}
| TK_NEW_SYMBOL
{
  ...
  current_constant_decl.append((Symbol*)$1->val);
}
| ne_constant_sym_list TK_OBJ_SYMBOL
{
  if (warning_level > 0) {
    std::cerr << "warning: redeclaration of constant " << $2->text << std::endl;
  }
  current_constant_decl.append((Symbol*)$2->val);
}
| TK_OBJ_SYMBOL
{
  if (warning_level > 0) {
    std::cerr << "warning: redeclaration of constant " << $1->text << std::endl;
  }
  current_constant_decl.append((Symbol*)$1->val);
}
;
...
/*
| positive_prec_goal_atom
| negative_prec_goal_atom
*/
| set_condition
...
{
  $$ = new Relation($2, $3, $4);
  $3->mark_functions_in_condition();
  $4->mark_functions_in_condition();
}
| TK_OPEN timing_keyword TK_OPEN relation_keyword d_expression d_expression
  TK_CLOSE TK_CLOSE
{
  $$ = new Relation($4, $2, $5, $6);
  $5->mark_functions_in_condition();
  $6->mark_functions_in_condition();
}
;
...
| TK_OPEN KW_MINOP d_expression d_expression TK_CLOSE
{
  $$ = new BinaryExpression(exp_min, $3, $4);
}
| TK_OPEN KW_MAXOP d_expression d_expression TK_CLOSE
{
  $$ = new BinaryExpression(exp_max, $3, $4);
}
...
axiom_declaration:
TK_OPEN KW_DERIVED TK_OPEN TK_PRED_SYMBOL
{
  stored_n_param.append(current_param.length());
  current_atom = new Atom((PredicateSymbol*)$4->val);
  ((PredicateSymbol*)$4->val)->modded = true;
}
atom_schema_typed_arg_list TK_CLOSE
{
...
atom_schema_typed_arg_list:
atom_schema_typed_arg_list atom_schema_arg_list TK_HYPHEN TK_TYPE_SYMBOL
{
  set_variable_type(current_param, (TypeSymbol*)$4->val);
}
| atom_schema_typed_arg_list atom_schema_arg_list TK_HYPHEN TK_OPEN KW_EITHER
{
  current_type_set.clear();
}
non_empty_type_name_list TK_CLOSE
...
| atom_schema_typed_arg_list atom_schema_arg_list
{
  set_variable_type(current_param, dom_top_type);
}
| /* empty */
...
atom_schema_arg_list:
atom_schema_arg_list TK_NEW_VAR_SYMBOL
{
  $2->val = new VariableSymbol($2->text);
  current_param.append((VariableSymbol*)$2->val);
  if (trace_print_context) {
...
| atom_schema_arg_list TK_VAR_SYMBOL
{
  assert(current_atom != 0);
  current_atom->param.append((Symbol*)$2->val);
}
| atom_schema_arg_list TK_OBJ_SYMBOL
{
  assert(current_atom != 0);
  current_atom->param.append((Symbol*)$2->val);
}
| atom_schema_arg_list TK_NEW_SYMBOL
{
  $2->val = new Symbol($2->text);
  dom_constants.append((Symbol*)$2->val);
  assert(current_atom != 0);
...
| TK_NEW_VAR_SYMBOL
{
  ...
  assert(current_atom != 0);
  current_atom->param.append((Symbol*)$1->val);
}
| TK_VAR_SYMBOL
{
  assert(current_atom != 0);
...
TK_OPEN TK_PRED_SYMBOL
{
  current_atom = new Atom((PredicateSymbol*)$2->val, md_init);
}
atom_argument_list TK_CLOSE
{
  ((Atom*)current_atom)->check();
...
| TK_OPEN any_symbol numeric_value TK_OPEN TK_PRED_SYMBOL
{
  current_atom = new Atom((PredicateSymbol*)$5->val, md_init);
}
atom_argument_list TK_CLOSE TK_CLOSE
{
  ((Atom*)current_atom)->check();
  PredicateSymbol* p = (PredicateSymbol*)$5->val;
  if (p->param.length() != current_atom->param.length()) {
    log_error("wrong number of arguments for predicate in (:init ...");
  }
  ((Atom*)current_atom)->insert(p->init);
  current_atom->at_time = $3;
  p->added = true;
  p->modded = true;
  dom_pos_til.append((Atom*)current_atom);
}
| TK_OPEN any_symbol numeric_value TK_OPEN KW_NOT TK_OPEN TK_PRED_SYMBOL
{
  current_atom = new Atom((PredicateSymbol*)$7->val, md_init);
}
atom_argument_list TK_CLOSE TK_CLOSE TK_CLOSE
{
  ((Atom*)current_atom)->check();
  PredicateSymbol* p = (PredicateSymbol*)$7->val;
  if (p->param.length() != current_atom->param.length()) {
    log_error("wrong number of arguments for predicate in (:init ...");
  }
  ((Atom*)current_atom)->insert(p->init);
...
goal_spec_list:
goal_spec_list single_goal_spec
| /* empty */
;
...
typed_param_list TK_CLOSE TK_OPEN KW_PREFERENCE TK_NEW_SYMBOL new_goal TK_CLOSE TK_CLOSE
{
  Symbol* name = new Symbol(sym_preference, $9->text);
  $9->val = name;
  Preference* qp = new Preference(name, $10);
  assert(stored_n_param.length() > 0);
  for (HSPS::index_type k = stored_n_param[stored_n_param.length() - 1];
       k < current_param.length(); k++) {
...
| TK_OPEN KW_IMPLY new_atomic_goal new_atomic_goal TK_CLOSE
{
  assert(($3->g_class == goal_pos_atom) || ($3->g_class == goal_neg_atom));
  DisjunctiveGoal* dg = new DisjunctiveGoal();
  if ($3->g_class == goal_neg_atom) {
    $3->g_class = goal_pos_atom;
    ((AtomicGoal*)$3)->atom->pred->pos_pre = true;
  }
  else {
    $3->g_class = goal_neg_atom;
    ((AtomicGoal*)$3)->atom->pred->neg_pre = true;
  }
  dg->goals.append($3);
  dg->goals.append($4);
  $$ = dg;
}
| TK_OPEN KW_IMPLY TK_OPEN KW_AND
{
  current_goal.append(new DisjunctiveGoal());
}
new_atomic_goal_list TK_CLOSE
{
  assert(current_goal.length() > 0);
  DisjunctiveGoal* dg =
...
typed_param_list TK_CLOSE new_goal TK_CLOSE
{
...
new_atomic_goal_list:
new_atomic_goal new_atomic_goal_list
{
  assert(current_goal.length() > 0);
  ConjunctiveGoal* cg = current_goal[current_goal.length() - 1];
  assert(cg != 0);
