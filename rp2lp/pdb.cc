...
(std::ostream& s, const partial_state& g, const index_set& e,
 bool mark_increasing, bool mark_decreasing, bool write_inf)
{
  CompleteStatePerfectHashFunction h(instance, variable_set);
  if (h.overflow()) {
  ...
  while (more) {
	...
	assign(res, act.post);
	index_type i_res = h.index(res);
	if ((FINITE(eval(se.current_state())) && FINITE(eval(res)))
	    || write_inf) {
	  s << "S" << h.index(se.current_state())
	    << " -> S" << h.index(res)
	    << " [label=\"" << act.name
...
#ifdef TRACE_PRINT_LOTS
  std::cerr << "is_final: state = " << state
	    << "vars = " << sa.variables()
	    << "goal = " << instance.goal_state << std::endl;
#endif
...
(const partial_state& s, NTYPE b)
{
  partial_state a(s, variables());
#ifdef TRACE_PRINT_LOTS
  std::cerr << "solving s = " << s << " -> " << a << " = ";
  instance.write_partial_state(std::cerr, a);
  std::cerr << " using A*..." << std::endl;
  std::cerr << "abstraction variables: " << variables() << std::endl;
  partial_state ag(instance.goal_state, variables());
  std::cerr << "abstract goal state: " << ag << " = ";
  instance.write_partial_state(std::cerr, ag);
  std::cerr << std::endl;
#endif
  ASHProgState s0(instance, *this, sa_acts, cost, h_search, sinc, inc, s, val);
  ASHProgState::cut_at_solved = false;
...
{
#ifdef TRACE_PRINT_LOTS
  std::cerr << "ProgressionASH::eval(" << s << ")..." << std::endl;
#endif
  partial_state a(s, variables());
...
{
#ifdef TRACE_PRINT_LOTS
  std::cerr << "ProgressionASH::eval_to_bound(" << s << ", " << b
	    << ")..." << std::endl;
#endif
  partial_state a(s, variables());
  partial_state_map::iterator p = val.find(a);
...
#ifdef ASH_SOLVE_USE_BFS
  //HValue v2 = solveIDA(s, b, 0);
  HValue v = solveBFS(s, b);
  // if (v.val > v2.val) {
  //   std::cerr << "error: state = " << s
  // 	      << ", v = " << v
  // 	      << ", v2 = " << v2 << std::endl;
  //   exit(1);
  // }
...
bool PDBCollection::has_complete_PDB()
{
  // if (update_flag) compute_canonical_heuristic();
  for (index_type k = 0; k < this->size(); k++)
    if ((*this)[k]->variables().size() == instance.n_variables())
      return true;
  return false;
}
 ...
 Statistics& stats)
{
  ...
  for (index_type k = 0; k < exts.length(); k++) {
    ...
    // std::cerr << "evaluating ";
    // s_ins.write_variable_set(std::cerr, col[exts[k].pattern]->variables());
    // std::cerr << " + ";
    // s_ins.write_variable_set(std::cerr, exts[k].v_new);
    // std::cerr << std::endl;
	...
	calculate_normal_ci(exts[k].wscore, exts[k].sq_wscore,
      ...
      exts[k].p_upper = N_TO_D(POS_INF);
      exts[k].p_lower = N_TO_D(NEG_INF);
    }
#endif
  }
}
 ...
 Statistics& stats)
{
  ...
  while (!done && (l < s_max)) {
    ...
    for (index_type k = 0; k < col->size(); k++)
      if ((*col)[k]->variables() == vars)
    	has_complete = true;
    if (has_complete && (best.length() > 0)) {
      std::cerr << "collection has a complete PDB, yet can be improved!"
    		<< std::endl;
    }
    assert(!has_complete || (best.length() == 0));
