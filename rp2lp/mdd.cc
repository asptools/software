...
MDDNode* makeMDD(StaticMutex* mx, const partial_state* map, index_type mapl)
{
  MDDNode* root = new MDDNode();
  for (index_type i = 0; i < mapl; i++) {
    if (mx->unreachable(i)) {
      root->insert(map[i]);
    }
    else {
      for (index_type j = i + 1; j < mapl; j++)
	if (!mx->unreachable(j) && mx->mutex(i, j)) {
	  partial_state s(map[i]);
	  s.insert(map[j]);
	  if (s.consistent())
	    root->insert(s);
	}
