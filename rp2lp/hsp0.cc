...
int main(int argc, char *argv[]) {
  ...
  for (int k = 1; k < argc; k++) {
    ...
    else if (strcmp(argv[k],"-lmc") == 0) {
      opt_lmc = true;
    }
    else if (strcmp(argv[k],"-lmcP2") == 0) {
      opt_lmcP2 = true;
    }
    ...
    else if (strcmp(argv[k],"-Rlmc") == 0) {
      opt_Rlmc = true;
    }
    else if (strcmp(argv[k],"-RlmcP2") == 0) {
      opt_RlmcP2 = true;
    }
  ...
  if (stack_limit > 0) stats.enable_stack_limit(stack_limit);
  ...
  if (!stats.break_signal_raised()) {
	...
	HSPS::MaxBinPDB* h_bin =
	  new HSPS::MaxBinPDB(*sas_instance, h_stats);
	if (opt_optimal)
	...
	HSPS::MaxCGPDB* h_cg = new HSPS::MaxCGPDB(*sas_instance, h_stats);
	...
	HSPS::MaxInterferencePDB* h_int =
	  new HSPS::MaxInterferencePDB(*sas_instance, h_stats);
	if (opt_pdb_reduced)
	...
	HSPS::IndependentPairPDB* h_pair =
	  new HSPS::IndependentPairPDB(*sas_instance, h_stats);
	h_pair->compute_sets();
	if (verbose_level > 1) {
	...
	HSPS::IndependentBinPDB* h_ibin =
	  new HSPS::IndependentBinPDB(*sas_instance, h_stats);
	if (opt_optimal)
...
// 	    new HSPS::WeightedBinPDB(*sas_instance, h_stats, *svf_sum);
...
// 	}
	...
	HSPS::WeightedIndependentBinPDB* h_wbin =
	  new HSPS::WeightedIndependentBinPDB(*sas_instance, h_stats, *sv_sum);
	if (opt_optimal)
	...
	new HSPS::AdditiveIncrementalRegressionPDB(*sas_instance, h_stats);
      HSPS::ACF* cost = (opt_cost ?
			 (HSPS::ACF*)new HSPS::SASCostACF(*sas_instance) :
			 (HSPS::ACF*)new HSPS::UnitACF());
      HSPS::Heuristic* inc = (opt_pdb_inc ? prep.inconsistency() : 0);
	...
	new HSPS::AdditiveIncrementalRegressionPDB(*sas_instance, h_stats);
      ...
      HSPS::MDDNode* sinc = (opt_pdb_inc ?
			     makeMDD(prep.inconsistency(),
				     sas_instance->atom_map_defined(),
				     sas_instance->atom_map_n()) : 0);
      HSPS::CostTable* h_cw = new HSPS::CostTable(instance, h_stats);
      h_cw->compute_H1(*cost);
      std::cerr << "computing Additive Incremental PDB:s..." << std::endl;
      h_ai->compute_pdbs(sas_instance->goal_state, opt_pdb_size, opt_optimal,
			 opt_pdb_spanning_subset, opt_pdb_collapse, false,
			 *cost, sinc, inc, 0); // h_cw);
      // note: additive groups are already computed by compute_pdbs
      if (verbose_level > 1) {
	std::cerr << "PDB sets/Additive groups: ";
	h_ai->write_additive_groups(std::cerr);
	...
	exit(0);
      ...
      HSPS::AH* h = new HSPS::AH(instance, h_stats);
	...
	if (opt_cost) {
	  h->compute_bottom_up_2
	    (HSPS::CostACF(instance), instance.goal_atoms,
	     (HSPS::index_type)CEIL_TO_INT(bu2_ratio * instance.n_atoms()),
	     !opt_H1);
	...
	if (opt_cost) {
	  ...
	  // implicit: h->compute_max(HSPS::UnitACF(), true);
	}
	else {
	...
	cost_est = h;
      ...
      std::cerr << "initialising lm-cut(P^2)..." << std::endl;
      HSPS::Instance* p2_ins = new HSPS::Instance(instance.name);
      HSPS::s2index* p2_atom_map = new HSPS::s2index();
      HSPS::index_vec* p2_act_map = new HSPS::index_vec();
      p2_ins->makeP2(instance, prep.inconsistency(), *p2_atom_map, *p2_act_map);
      p2_ins->cross_reference();
      HSPS::ACF* c = (opt_cost ?
		      (HSPS::ACF*)new HSPS::CostACF(instance) :
		      (HSPS::ACF*)new HSPS::UnitACF());
      cost_est = new HSPS::RegressionLMCutP2(*p2_ins, *c, instance,
					     *p2_atom_map, *p2_act_map,
					     h_stats); 
    }

    else if (opt_lmc) {
      std::cerr << "initialising lm-cut(P)..." << std::endl;
      HSPS::ACF* c = (opt_cost ?
		      (HSPS::ACF*)new HSPS::CostACF(instance) :
		      (HSPS::ACF*)new HSPS::UnitACF());
      cost_est = new HSPS::RegressionLMCut(instance, *c, h_stats); 
    }
    ...
    h_stats.stop();
    if (!stats.break_signal_raised()) {
      std::cerr << "heuristic computed in " << h_stats.total_time()
		<< " seconds" << std::endl;
    }
	...
	(opt_rpdb && opt_pdb_inc ? prep.inconsistency() : 0);
      HSPS::MDDNode* sinc = (opt_rpdb && opt_pdb_inc ?
			     makeMDD(prep.inconsistency(),
				     sas_instance->atom_map_defined(),
				     sas_instance->atom_map_n()) : 0);
      HSPS::Instance* p2ins = 0;
      HSPS::s2index* p2_atom_map = 0;
      HSPS::index_vec* p2_act_map = 0;
      if (opt_RlmcP2) {
	p2ins = new HSPS::Instance(instance.name);
	p2_atom_map = new HSPS::s2index();
	p2_act_map = new HSPS::index_vec();
	p2ins->makeP2(instance, prep.inconsistency(), *p2_atom_map, *p2_act_map);
	p2ins->cross_reference();
      }
	...
	if (opt_rpdb) {
	  ...
	  HSPS::ACF* c = new HSPS::ResourceConsACF(instance, k);
	  HSPS::Heuristic* hP2 =
	    new HSPS::RegressionLMCutP2(*p2ins, *c, instance,
					*p2_atom_map, *p2_act_map, h_stats);
	  resource_est[k] = hP2;
	  // HSPS::Heuristic* hP =
	  //   new HSPS::RegressionLMCut(instance, *c, h_stats);
	  // resource_est[k] = new HSPS::CompareEval(instance, *hP2, *hP);
	  // resource_est[k]->set_trace_level(1);
	}
	else if (opt_Rlmc) {
	  HSPS::ACF* c = new HSPS::ResourceConsACF(instance, k);
	  resource_est[k] = new HSPS::RegressionLMCut(instance, *c, h_stats);
	}
	else if (opt_RAH) {
	  HSPS::AH* rce = new HSPS::AH(instance, h_stats);
	  if (opt_load_partition) {
	    HSPS::name_vec pnames(0, 0);
	    HSPS::index_set_vec partition;
	    reader->export_action_partitions(pnames, partition);
	    ...
	    instance.remap_sets(partition, prep.action_map);
	    rce->compute_additive(HSPS::ResourceConsACF(instance, k), partition, opt_R2);
	    rce->compute_max(HSPS::ResourceConsACF(instance, k), opt_R2);
	  }
	  else if (opt_ia1_partition) {
	    rce->compute_with_iterative_assignment_1
	      (HSPS::ResourceConsACF(instance, k), instance.goal_atoms,
	       opt_R2, opt_fpia, opt_optimal, instance.goal_atoms);
	  }
	  else if (opt_ia2_partition) {
	    HSPS::index_set a;
	    if (opt_iaa_partition)
	      ...
	      a.assign_copy(instance.goal_atoms);
	    rce->compute_with_iterative_assignment_2
	      (HSPS::ResourceConsACF(instance, k), a,
	       opt_R2, opt_fpia, instance.goal_atoms);
	  }
	  else if (opt_new_decomposition) {
	    rce->compute_with_new_decomposition
	      (HSPS::ResourceConsACF(instance, k), instance.goal_atoms, !opt_R2);
	    if (opt_AH_max)
	      rce->compute_max(HSPS::ResourceConsACF(instance, k), !opt_R2);
	  }
	  else if (opt_layered_partition) {
	    rce->compute_with_layered_partitioning
	      (HSPS::ResourceConsACF(instance, k), instance.goal_atoms);
	  }
	  // else if (opt_rnd_relevance_partition) {
	  ...
	  // }
	    ...
	    rce->compute_with_relevance_partitioning
	      (HSPS::ResourceConsACF(instance, k), instance.goal_atoms);
	  }
	  resource_est[k] = rce;
	}
	...
	if (opt_R2) {
	  HSPS::CostTable* rce = new HSPS::CostTable(instance, h_stats);
      ...
      if (!stats.break_signal_raised()) {
	std::cerr << "resource estimators computed in "
		  << h_stats.time() << " seconds" << std::endl;
      }
      h_stats.stop();
    }
    ...
    if (!stats.break_signal_raised() && opt_weighted_h) {
      std::cerr << "using h = " << h_weight << " * heuristic..." << std::endl;
      HSPS::WeightedHeuristic* wh =
	new HSPS::WeightedHeuristic(instance, *cost_est, h_weight);
      cost_est = wh;
    }
	...
	HSPS::IDAO* idao_search = new HSPS::IDAO(search_stats, *result, tt);
	...
	tt = new HSPS::HashTable(opt_tt_size);
	HSPS::IDA* ida_search = new HSPS::IDA(search_stats, *result, tt);
	...
	HSPS::IDA* ida_search = new HSPS::IDA(search_stats, *result);
	ida_search->set_cycle_check(opt_cc);
	// if (iteration_limit > 0)
	//  ida_search->set_iteration_limit(iteration_limit);
	search = ida_search;
	...
	// enumartion of paths from a fully explored state space
	// not implemented
	assert(0);
	...
	//   HSPS::DFS s(search_stats, *result);
	//   if (opt_find_all) {
	...
	// }
      ...
      if (!stats.break_signal_raised()) {
	std::cerr << "search complete (" << search_stats << ")" << std::endl;
      }
	...
	if (!stats.break_signal_raised()) {
	  ...
	  store.set_stop_condition(HSPS::Result::stop_at_all_optimal);
	  std::cerr << "searching..." << std::endl;
	  HSPS::DFS_BB op_search(search_stats, result);
  ...
  if (opt_validate) {
	...
	plan->write(std::cerr);
      ...
      plan->trace_level = verbose_level;
  ...
  if (opt_ipc || (verbose_level == 0)) {
    ...
    if (solved) {
      std::cout << "solution cost: " << PRINT_NTYPE(solution_cost);
      if (optimally)
	std::cout << " (optimal)";
      else
    ...
    std::cerr << "highest lower bound " << PRINT_NTYPE(stats.max_lower_bound())
