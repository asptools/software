...
Transition* SeqRegState::transition()
{
  if (act != no_such_index) {
    return new SeqRegTrans(act, instance.actions[act].dur);
  }
  else {
    return 0;
  }
...
#endif
#ifdef EVAL_ACTION_PRECONDITION_IN_SEQUENTIAL_REGRESSION
...
int SeqRegState::compare(const State& s)
{
  int c = AtomSetState::compare(s);
  if (c != 0) return c;
  SeqRegState& ss = (SeqRegState&)s;
  if (res) {
    if (ss.res)
...
{
  SeqCRegState& ss = (SeqCRegState&)s;
