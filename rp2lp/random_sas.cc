...
int main(int argc, char *argv[]) {
  ...
  bool       cg_polytree = false;
  bool       cg_tree = false;
  bool       cg_star = false;
  HSPS::index_type cg_star_nucleus_size = 1;
  bool       cg_clockwork = false;
  HSPS::index_type cg_clockwork_conc = 1;
  HSPS::index_type cg_clockwork_depth = 2;
  bool       cg_clockwork_asym = false;
  ...
  for (int k = 1; k < argc; k++) {
    ...
    else if (strcmp(argv[k],"-polytree-cg") == 0) {
      cg_polytree = true;
    }
...
//   }
  ...
  while (i < n_instances) {
	...
	base_file_name << ".pddl";
      ...
      if (opt_automata) {
	if (verbose_level > 0)
	  std::cerr << "writing automata representation to "
		    << base_file_name.str() << "..."
		    << std::endl;
	std::ofstream save_file(base_file_name.str().c_str());
	sas_ins->write_automata(save_file);
	save_file.close();
      }

      else if (opt_single_file) {
	if (verbose_level > 0)
