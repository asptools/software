...
class ConcatenatedName : public Name {
 ...
 public:
  ...
  ConcatenatedName(const Name* n1, const Name* n2)
    : elements((Name*)0, 0), catc('+')
  {
    elements.append(n1);
    elements.append(n2);
  };
...
class CopyName : public Name {
  ...
  index_type  _num;
 public:
  CopyName(const Name* n)
   : _prefix("copy"), _name(n), _num(no_such_index) { };
  CopyName(const Name* n, index_type m)
    : _prefix("copy"), _name(n), _num(m) { };
  CopyName(const char* p, const Name* n, index_type m)
    : _prefix(p), _name(n), _num(m) { };
  virtual ~CopyName() { };
  virtual void write(::std::ostream& s, unsigned int c = NC_DEFAULT) const;
};
...
class EnumName : public Name {
  ...
  index_vec   index;
 public:
  EnumName(const char* p, index_type i)
    : prefix(p), index(i, 1) { };
  EnumName(const char* p, index_type i1, index_type i2)
    : prefix(p), index(0, 2) { index[0] = i1; index[1] = i2; };
  EnumName(const char* p, index_type i1, index_type i2, index_type i3)
    : prefix(p), index(0, 3) { index[0] = i1; index[1] = i2; index[2] = i3; };
  virtual ~EnumName() { };
  virtual void write(::std::ostream& s, unsigned int c = NC_DEFAULT) const;
};
